...
class BalancedCrosswordGenerator:
                ...
                if row < len(grid) and col < len(grid[row]):
                    ...
                    if cell == "#":
                        ...
                        if numbers and (row, col) in numbers:
                            ...
                            c.drawString(
                                x + 2, y + CELL_SIZE - 9, str(numbers[(row, col)])
                            )
                        ...
                        if show_solution and cell not in ["#", ".", " "]:
                            ...
                            c.drawCentredString(
                                x + CELL_SIZE / 2, y + CELL_SIZE / 2 - 3, cell
                            )
    ...
    def create_complete_book(self):
        ...
        for book_type in ["paperback", "hardcover"]:
            ...
            c.drawString(
                MARGIN,
                PAGE_HEIGHT - MARGIN - 1 * inch,
                "Copyright © 2025 KindleMint Press",
            )
            c.drawString(
                MARGIN, PAGE_HEIGHT - MARGIN - 1.3 * inch, "ISBN: 9798289681881"
            )
            c.showPage()
            ...
            c.drawCentredString(
                PAGE_WIDTH / 2, PAGE_HEIGHT - MARGIN - 1 * inch, "Table of Contents"
            )
            ...
            c.setFont("Helvetica-Bold", 18)
            c.drawCentredString(
                PAGE_WIDTH / 2, PAGE_HEIGHT - MARGIN - 1 * inch, "Introduction"
            )
            ...
            c.setFont("Helvetica-Bold", 18)
            c.drawCentredString(
                PAGE_WIDTH / 2, PAGE_HEIGHT - MARGIN - 1 * inch, "How to Solve"
            )
            c.setFont("Helvetica", 11)
            y = PAGE_HEIGHT - MARGIN - 2 * inch
            tips = [
            ...
            for puzzle in puzzles:
                ...
                c.drawCentredString(
                    PAGE_WIDTH / 2, PAGE_HEIGHT - MARGIN - 0.4 * inch, f"Puzzle {num}"
                )
                ...
                self.draw_grid(
                    c, x_offset, y_offset, empty_grid, numbers, show_solution=False
                )
                c.showPage()
                ...
                for clue_num in sorted(puzzle["across"].keys()):
                    if y > MARGIN + 0.5 * inch:
                        c.drawString(
                            MARGIN, y, f"{clue_num}. {puzzle['across'][clue_num]}"
                        )
                        y -= 0.25 * inch
                        across_count += 1
                ...
                c.setFont("Helvetica-Bold", 12)
                c.drawString(
                    PAGE_WIDTH / 2 + 0.1 * inch, PAGE_HEIGHT - MARGIN - 1 * inch, "DOWN"
                )
                c.setFont("Helvetica", 10)
                y = PAGE_HEIGHT - MARGIN - 1.3 * inch
                ...
                for clue_num in sorted(puzzle["down"].keys()):
                    if y > MARGIN + 0.5 * inch:
                        c.drawString(
                            ...
                            y,
                            f"{clue_num}. {puzzle['down'][clue_num]}",
                        )
                        y -= 0.25 * inch
                        down_count += 1
            ...
            for puzzle in puzzles:
                ...
                c.drawCentredString(
                    PAGE_WIDTH / 2,
                    PAGE_HEIGHT - MARGIN - 0.5 * inch,
                    f"Puzzle {num} - Solution",
                )
                    ...
                    for col in range(15):
                        ...
                        if row < len(grid) and col < len(grid[row]):
                            ...
                            if cell == "#":
                                c.setFillColor(colors.black)
                                c.rect(
                                    x, y, solution_cell, solution_cell, fill=1, stroke=0
                                )
                            else:
                                c.setFillColor(colors.white)
                                c.setStrokeColor(colors.black)
                                c.rect(
                                    x, y, solution_cell, solution_cell, fill=1, stroke=1
                                )
