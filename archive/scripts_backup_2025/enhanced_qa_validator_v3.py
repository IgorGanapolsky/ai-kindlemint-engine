#!/usr/bin/env python3
"""
Enhanced Content-Aware QA Validator **v3**

This advanced validator focuses on **content quality** rather than just file
structure.  It performs the following domain-specific crossword-book checks:

1. 📐 Grid pattern analysis & 180-degree symmetry verification
2. 🔍 Clue ↔︎ Answer relationship validation (format & dictionary look-up)
3. 🧩 Solution-grid completeness & letter-validity checks
4. 🎭 Theme consistency and difficulty-progression analysis across the book
5. 🏆 KDP-readiness assessment combining content and PDF validations

The validator still supports all v2 features, but surfaces far richer,
actionable feedback for puzzle-book creators.
"""
...
logger = logging.getLogger("EnhancedQAValidatorV3")
...
class EnhancedQAValidatorV2:
    ...
    def __init__(self, book_dir, output_dir=None, word_list_path=None):
        ...
        if not self.metadata_dir.exists():
            raise FileNotFoundError(
                f"Metadata directory not found: {self.metadata_dir}"
            )
    ...
    def validate_book(self):
        ...
        if not puzzle_ids:
            self.report["global_issues"].append(
                "CRITICAL: No puzzles listed in collection.json."
            )
            self.finalize_report()
            return self.report
    ...
    def validate_puzzle(self, puzzle_id):
        ...
        if not puzzle_file.exists():
            self._add_result(
                puzzle_id,
                "critical",
                f"Puzzle metadata file not found: {puzzle_file.name}",
            )
            return
        ...
        with open(puzzle_file, "r") as f:
            try:
                ...
                self._add_result(
                    puzzle_id, "critical", "Invalid JSON in metadata file."
                )
                return
        ...
        grid, reconstruction_ok = self._validate_intersections_and_reconstruct_grid(
        ...
        if reconstruction_ok:
            ...
            self._add_result(
                puzzle_id,
                "critical",
                "Skipping connectivity check due to grid reconstruction failure.",
            )
    ...
    def _validate_metadata_completeness(self, puzzle_id, data):
        ...
        if missing_keys:
            ...
            self._add_result(
                puzzle_id, "pass", "All required metadata keys are present."
            )
    ...
    def _validate_word_content(self, puzzle_id, data):
        ...
        for clue_data in all_words:
            if len(clue_data) < 3:
                self._add_result(
                    puzzle_id, "critical", f"Malformed clue data: {clue_data}"
                )
                continue
            word = clue_data[2]
        ...
        if invalid_words:
            self._add_result(
                puzzle_id,
                "critical",
                f"Contains invalid/unknown words: {', '.join(invalid_words)}",
            )
        else:
    ...
    def _validate_word_balance_and_count(self, puzzle_id, data):
        ...
        if total_count < 20:
            self._add_result(
                puzzle_id,
                "critical",
                f"Too few words in puzzle: {total_count} (min 20)",
            )
        else:
        ...
        if total_count > 0:
            ...
            if across_ratio < 0.3 or down_ratio < 0.3:
                self._add_result(
                    puzzle_id,
                    "warning",
                    f"Poor word balance: {across_count} across, {down_count} down",
                )
            else:
    ...
    def _validate_for_duplicate_words(self, puzzle_id, data):
        ...
        all_words = [
            item[2]
            for item in clues.get("across", []) + clues.get("down", [])
            if len(item) > 2
        ]
        ...
        if duplicates:
            self._add_result(
                puzzle_id, "critical", f"Duplicate words found: {', '.join(duplicates)}"
            )
        else:
    ...
    def _validate_intersections_and_reconstruct_grid(self, puzzle_id, data):
        ...
        if not clues or not clue_positions:
            self._add_result(
                puzzle_id,
                "critical",
                "Missing clues or clue_positions for intersection check.",
            )
            return None, False
        ...
        for direction, (num, _, word) in all_clues:
            ...
            if not pos_str:
                intersection_errors.append(
                    f"No position found for clue {direction} {num}"
                )
                continue
            ...
            for i, letter in enumerate(word):
                ...
                if r >= grid_size or c >= grid_size:
                    intersection_errors.append(
                        f"Word '{word}' ({direction} {num}) goes out of bounds."
                    )
                    break
                ...
                if grid[r][c] == "":
                    ...
                    error_msg = (
                        f"Intersection conflict at ({r},{c}) for word '{word}' ({direction} {num}). "
    ...
    def _validate_grid_connectivity(self, puzzle_id, grid):
        ...
        if not white_squares:
            self._add_result(
                puzzle_id, "warning", "Grid appears to be empty or all black squares."
            )
            return
        ...
        if len(visited) != len(white_squares):
            ...
            self._add_result(
                puzzle_id,
                "critical",
                f"Grid has {unreachable_count} unreachable/isolated squares.",
            )
        else:
    ...
    def finalize_report(self):
        ...
        if (
            self.report["summary"]["critical_issues_count"] > 0
            or self.report["global_issues"]
        ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced Content-Aware QA Validator v3"
    )
    parser.add_argument(
        "book_dir", help="Path to the book directory containing the metadata folder."
    )
    parser.add_argument(
        "--output-dir", help="Directory to save the QA report (defaults to book_dir)."
    )
    parser.add_argument(
        "--word-list", help="Path to a custom word list file for validation."
    )
    parser.add_argument(
        "--log-level",
        default="info",
    ...
    try:
        ...
        print(
            f"  - Critical Issues: {report['summary']['puzzles_with_critical_issues']}"
        )
        print("-" * 36)
