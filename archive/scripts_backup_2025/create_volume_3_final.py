...
class FinalCrosswordGenerator:
    ...
    def create_complete_book(self):
        ...
        for book_type in ["paperback", "hardcover"]:
            ...
            c.drawCentredString(
                PAGE_WIDTH / 2, PAGE_HEIGHT - 5.2 * inch, "50 Unique Crossword Puzzles"
            )
            c.drawCentredString(
                PAGE_WIDTH / 2, PAGE_HEIGHT - 5.7 * inch, "Large Print Edition"
            )
            c.showPage()
            ...
            c.drawString(
                MARGIN, PAGE_HEIGHT - MARGIN - 1.3 * inch, "All rights reserved."
            )
            c.drawString(
                MARGIN, PAGE_HEIGHT - MARGIN - 1.6 * inch, "ISBN: 9798289681881"
            )
            c.showPage()
                ...
                for row in range(15):
                    for col in range(15):
                        ...
                        if puzzle["grid"][row][col] == "#":
                            ...
                            c.drawCentredString(
                                x + solution_cell / 2, y + solution_cell / 2 - 3, letter
                            )
