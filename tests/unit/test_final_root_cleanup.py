...
class TestFinalRootCleanup:
    ...
    def test_successful_file_moves(self, mock_cwd, temp_project_dir, capsys):
        ...
        assert (temp_project_dir / "reports" / "hygiene_report_20250630_172720.json").exists()
        assert (temp_project_dir / "docs" / "scripts_to_archive.txt").exists()
        assert (temp_project_dir / "config" / ".roomodes").exists()
        assert (temp_project_dir / "deployment" / "Dockerfile.scheduler").exists()
        
        # Check that original files no longer exist in root
        assert not (temp_project_dir / "hygiene_report_20250630_172720.json").exists()
    ...
    def test_nonexistent_files_skipped(self, mock_cwd, temp_project_dir, capsys):
        ...
        assert not (temp_project_dir / "reports" / "hygiene_report_20250630_172720.json").exists()
        assert not (temp_project_dir / "docs" / "scripts_to_archive.txt").exists()
        assert (temp_project_dir / "config" / ".roomodes").exists()
        assert (temp_project_dir / "deployment" / "Dockerfile.scheduler").exists()
    ...
    @patch('scripts.final_root_cleanup.Path.cwd')
    def test_essential_files_check_all_essential(self, mock_cwd, temp_project_dir, capsys):
