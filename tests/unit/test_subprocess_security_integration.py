...
class TestSubprocessSecurityIntegration:
    ...
    def test_security_module_provides_safe_command(self):
        ...
        try:
            from security import safe_command
            assert hasattr(safe_command, 'run'), "safe_command should have 'run' method"
        except ImportError:
            # If security module is not available, this is expected in test environment
            pytest.skip("security module not available - expected in test environment")

    @patch('subprocess.run')
    @patch('security.safe_command')
    def test_safe_command_vs_direct_subprocess(self, mock_safe_command, mock_subprocess):
