...
class TestClaudeCostsSecurity:
    ...
    def test_badge_command_prints_stdout(
        self, mock_print, mock_tracker, mock_safe_command
    ):
    ...
    @patch("sys.argv", ["claude_costs.py", "badge"])
    @patch("builtins.print")
    def test_badge_command_handles_called_process_error(
        self, mock_print, mock_tracker, mock_safe_command
    ):
        ...
        mock_print.assert_called_with(
            "❌ Badge generation failed: Error generating badge"
        )
    ...
    @patch("security.safe_command")
    @patch("kindlemint.utils.cost_tracker.ClaudeCostTracker")
    def test_badge_command_not_called_for_other_commands(
        ...
        mock_tracker_instance = Mock()
        mock_tracker_instance.track_commit.return_value = {
            "status": "tracked",
            "cost": 0.01,
            "tokens": 100,
            "files": 2,
    ...
    def test_security_module_structure(self):
        ...
        try:
            ...
            assert hasattr(
                safe_command, "run"
            ), "safe_command should have a 'run' method"
            assert callable(
