...
class TestClaudeCostsSafeCommandIntegration:
    ...
    def test_badge_command_uses_safe_command_run_success(self, mock_tracker, mock_safe_command):
        ...
        assert call_args[0][0] == subprocess.run  # First arg should be subprocess.run
        assert len(call_args[0][1]) == 2  # Command should have 2 elements [python, script_path]
        assert str(call_args[0][1][0]) == sys.executable  # First element should be python executable
        assert call_args[0][1][1].endswith('generate_cost_badge.py')  # Second should be script path
    ...
    @patch('scripts.claude_costs.ClaudeCostTracker')
    @patch('sys.argv', ['claude_costs.py', 'badge'])
    def test_badge_command_uses_safe_command_run_failure(self, mock_tracker, mock_safe_command):
        """Test that badge command handles failures properly when using safe_command.run"""
        # Setup mocks for failure scenario
        error = subprocess.CalledProcessError(1, ['python', 'generate_cost_badge.py'])
        ...
        mock_print.assert_called_with("❌ Badge generation failed: Badge generation failed: File not found")

    @patch('scripts.claude_costs.safe_command')
    @patch('scripts.claude_costs.ClaudeCostTracker')
    @patch('sys.argv', ['claude_costs.py', 'badge'])
    def test_badge_command_script_path_resolution(self, mock_tracker, mock_safe_command):
    ...
    def test_safe_command_import_failure_handling(self, mock_safe_command):
        ...
        mock_safe_command.side_effect = ImportError("security module not found")
    ...
    def test_other_commands_not_affected_by_safe_command(self, mock_tracker, mock_safe_command):
        ...
        assert any("Claude Cost Tracking Status" in str(call) for call in mock_print.call_args_list)

    @patch('scripts.claude_costs.safe_command')
    @patch('scripts.claude_costs.ClaudeCostTracker')
    @patch('sys.argv', ['claude_costs.py', 'badge'])
    @patch('scripts.claude_costs.Path')
    def test_badge_script_path_construction(self, mock_path, mock_tracker, mock_safe_command):
    ...
    @patch('scripts.claude_costs.ClaudeCostTracker')
    @patch('sys.argv', ['claude_costs.py', 'badge'])
    def test_badge_command_preserves_all_subprocess_parameters(self, mock_tracker, mock_safe_command):
