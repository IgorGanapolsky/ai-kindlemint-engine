...
class TestSetupPySecurityDependency:
    ...
    def test_security_dependency_in_install_requires(self):
        ...
        if setup_py_path.exists():
            ...
            assert (
                '"security==1.3.1"' in setup_content
            ), "security==1.3.1 should be in install_requires"

            # Check that it's in the install_requires section
            assert (
                "install_requires=" in setup_content
            ...
            assert (
                security_line_found
            ), "security==1.3.1 should be found in install_requires section"
    ...
    def test_security_dependency_version_pinned(self):
        ...
        if setup_py_path.exists():
            ...
            assert (
                '"security==1.3.1"' in setup_content
            ), "security should be pinned to version 1.3.1"

            # Should not have version ranges
            assert (
                '"security>=' not in setup_content
            ), "security should not use >= version specifier"
            assert (
                '"security~=' not in setup_content
            ), "security should not use ~= version specifier"
            assert (
    ...
    def test_security_dependency_in_alphabetical_order(self):
        ...
        if setup_py_path.exists():
            ...
            assert (
                "security" in install_requires_lines
            ), "security should be in install_requires"

            # Check alphabetical order (should be sorted)
            sorted_packages = sorted(install_requires_lines)
            assert (
                install_requires_lines == sorted_packages
            ), f"install_requires should be in alphabetical order. Expected: {sorted_packages}, Got: {install_requires_lines}"
