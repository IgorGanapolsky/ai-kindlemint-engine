...
class TestCursorBugbotValidator(unittest.TestCase):
    ...
    def test_check_cursorignore_missing_critical_patterns(self, mock_exists, mock_file):
        ...
        warning_found = any("⚠️  .cursorignore missing critical patterns:" in warning 
    ...
    def test_check_cursorignore_all_patterns_present(self, mock_exists, mock_file):
        ...
        self.assertTrue(result)
        self.assertIn("✅ .cursorignore properly configured with security patterns", 
    ...
    def test_check_github_workflow_not_found(self, mock_exists):
        ...
        self.assertFalse(result)
        self.assertIn("❌ GitHub workflow cursor-bugbot.yml not found", 
    ...
    def test_check_github_workflow_exists(self, mock_exists):
        ...
        self.assertTrue(result)
        self.assertIn("✅ GitHub workflow for automated Bugbot triggering exists", 
                     self.validator.successes)
    ...
    def test_check_readme_documentation_no_bugbot_mention(self, mock_exists, mock_file):
        ...
        mock_exists.return_value = True
        mock_file.return_value.read.return_value = "# My Project\nSome content without bugbot mention"
        
        result = self.validator.check_readme_documentation()
        
        self.assertFalse(result)
        self.assertIn("⚠️  README.md doesn't mention Cursor Bugbot", 
                     self.validator.warnings)

    @patch("builtins.open", new_callable=mock_open)
    @patch("pathlib.Path.exists")
    def test_check_readme_documentation_with_bugbot_mention(self, mock_exists, mock_file):
        ...
        self.assertTrue(result)
        self.assertIn("✅ README.md includes Cursor Bugbot badge", 
                     self.validator.successes)
    ...
    def test_generate_setup_instructions(self):
        ...
        self.assertIn("https://www.cursor.com/dashboard?tab=integrations", instructions)
    ...
    def test_validate_all_successful(self, mock_print, mock_readme, mock_workflow, mock_cursorignore):
        ...
        self.assertIn("✅ Local configuration is ready for Cursor Bugbot!", report)
        self.assertIn("⚠️  Remember: You must still enable Bugbot in the Cursor dashboard", report)
        ...
        mock_print.assert_called_once_with("🔍 Validating Cursor Bugbot Configuration...\n")

    @patch.object(CursorBugbotValidator, "check_cursorignore")
    @patch.object(CursorBugbotValidator, "check_github_workflow")
    @patch.object(CursorBugbotValidator, "check_readme_documentation")
    @patch("builtins.print")
    def test_validate_with_issues(self, mock_print, mock_readme, mock_workflow, mock_cursorignore):
        ...
        self.assertIn("❌ Configuration issues found. Please fix them before proceeding.", report)
        self.assertIn("🤖 Cursor Bugbot Setup Instructions", report)
    ...
    @patch.object(CursorBugbotValidator, "check_readme_documentation")
    @patch("builtins.print")
    def test_validate_with_warnings(self, mock_print, mock_readme, mock_workflow, mock_cursorignore):
...
class TestStringFormatting(unittest.TestCase):
    ...
    def test_workflow_path_formatting(self, mock_exists):
        ...
        expected_path = self.validator.repo_root / ".github" / "workflows" / "cursor-bugbot.yml"
        mock_exists.assert_called_with()
