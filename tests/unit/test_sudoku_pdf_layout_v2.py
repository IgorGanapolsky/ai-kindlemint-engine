...
class TestSudokuPDFLayoutV2(unittest.TestCase):
    ...
    def test_get_puzzle_insight_uses_secrets_choice(self, mock_file, mock_json_load, mock_path):
    ...
    @patch('scripts.sudoku_pdf_layout_v2.json.load')
    @patch('builtins.open', new_callable=mock_open, read_data='{"puzzles": [1]}')
    def test_get_puzzle_insight_easy_difficulty(self, mock_file, mock_json_load, mock_path):
        ...
        with patch('secrets.choice') as mock_secrets_choice:
            ...
            easy_content_found = any("3×3 boxes" in insight or "rows or columns" in insight for insight in call_args)
            self.assertTrue(easy_content_found, "Easy difficulty insights should contain characteristic content")

    @patch('scripts.sudoku_pdf_layout_v2.Path')
    @patch('scripts.sudoku_pdf_layout_v2.json.load')
    @patch('builtins.open', new_callable=mock_open, read_data='{"puzzles": [1]}')
    def test_get_puzzle_insight_hard_difficulty(self, mock_file, mock_json_load, mock_path):
        ...
        with patch('secrets.choice') as mock_secrets_choice:
            ...
            hard_content_found = any("patience" in insight or "sparse" in insight or "systematic" in insight for insight in call_args)
            self.assertTrue(hard_content_found, "Hard difficulty insights should contain characteristic content")
