...
class TestKindleMintCLI:
    ...
    def test_import_warnings_daily_tasks(self, capsys):
        ...
        with patch.dict('sys.modules', {'scripts.daily_tasks': None}):
            with patch('builtins.__import__', side_effect=ImportError("Module not found")):
                ...
                assert "Warning: Could not import run_daily_tasks from scripts.daily_tasks:" in captured.out
                assert "This function will not be available via kindlemint.cli.run_daily_tasks." in captured.out
    ...
    def test_import_warnings_book_layout_engine(self, capsys):
        ...
        with patch.dict('sys.modules', {'scripts.book_layout_bot': None}):
            with patch('builtins.__import__', side_effect=ImportError("Module not found")):
                ...
                assert "Warning: Could not import BookLayoutEngine from scripts.book_layout_bot:" in captured.out
                assert "This class will not be available via kindlemint.cli.BookLayoutEngine." in captured.out
    ...
    def test_import_warnings_crossword_engine(self, capsys):
        ...
        with patch.dict('sys.modules', {'scripts.crossword_engine_v2': None}):
            with patch('builtins.__import__', side_effect=ImportError("Module not found")):
                ...
                assert "Warning: Could not import CrosswordEngine from scripts.crossword_engine_v2:" in captured.out
                assert "This class will not be available via kindlemint.cli.CrosswordEngineV2." in captured.out
    ...
    def test_import_warnings_sudoku_generator(self, capsys):
        ...
        with patch.dict('sys.modules', {'scripts.sudoku_generator': None}):
            with patch('builtins.__import__', side_effect=ImportError("Module not found")):
                ...
                assert "Warning: Could not import SudokuGenerator from scripts.sudoku_generator:" in captured.out
                assert "This class will not be available via kindlemint.cli.SudokuGeneratorCLI." in captured.out
    ...
    def test_validate_metadata_wrapper_creation(self):
        ...
        with patch.dict('sys.modules', {'scripts.critical_metadata_qa': Mock(CriticalMetadataQA=mock_qa_class)}):
            with patch('builtins.__import__', side_effect=[ImportError("Direct function import failed"), None]):
                if 'kindlemint.cli' in sys.modules:
                    del sys.modules['kindlemint.cli']
                ...
                mock_qa_instance.validate_metadata.assert_called_once_with({"test": "data"})
    ...
    def test_validate_metadata_import_failure(self, capsys):
        ...
        with patch.dict('sys.modules', {'scripts.critical_metadata_qa': None}):
            with patch('builtins.__import__', side_effect=ImportError("Module not found")):
                ...
                assert "Warning: Could not import validate_metadata from scripts.critical_metadata_qa:" in captured.out
                assert "This function will not be available via kindlemint.cli.validate_metadata." in captured.out
    ...
    def test_legacy_cli_import_warnings(self, capsys):
        ...
        with patch.dict('sys.modules', {'scripts.cli.main': None}):
            with patch('builtins.__import__', side_effect=ImportError("Module not found")):
                ...
                assert "Warning: Could not import cli or FORMATTERS from scripts.cli.main:" in captured.out
                assert "The click command-group interface will not be available via kindlemint.cli.cli." in captured.out
    ...
    def test_is_legacy_cli_available(self):
        ...
        with patch.object(kindlemint.cli, 'globals', return_value={'test_function': Mock()}):
            assert kindlemint.cli.is_legacy_cli_available('test_function') is True
        
        with patch.object(kindlemint.cli, 'globals', return_value={'test_function': None}):
            assert kindlemint.cli.is_legacy_cli_available('test_function') is False
        
        with patch.object(kindlemint.cli, 'globals', return_value={}):
            assert kindlemint.cli.is_legacy_cli_available('nonexistent_function') is False
    ...
    def test_enhance_seo_command_success(self, mock_cli):
        ...
        mock_path = Mock()
        mock_path.open.return_value.__enter__.return_value.read.return_value = json.dumps(test_book_data)
        ...
        with patch('pathlib.Path') as mock_path_class:
            ...
            with patch('kindlemint.marketing.seo_engine_2025.SEOOptimizedMarketing') as mock_seo_class:
                ...
                with patch('json.load', return_value=test_book_data):
                    with patch('json.dump') as mock_json_dump:
                        with patch('builtins.open', mock_open()):
                            ...
                            runner = CliRunner()
                            result = runner.invoke(enhance_seo, ['--input', 'test_book.json'])
                            
                            assert result.exit_code == 0
                            assert "Enhanced metadata written to" in result.output
                            mock_seo_engine.enhance_book_marketing.assert_called_once_with(test_book_data)
        ...
        with patch('pathlib.Path') as mock_path_class:
            ...
            with patch('json.load', side_effect=json.JSONDecodeError("Invalid JSON", "doc", 0)):
                with patch('builtins.open', mock_open(read_data="invalid json")):
                    ...
                    runner = CliRunner()
                    result = runner.invoke(enhance_seo, ['--input', 'invalid_book.json'])
