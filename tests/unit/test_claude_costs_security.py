...
class TestClaudeCostsSecurityIntegration:
    ...
    def test_badge_command_uses_safe_command(self, mock_safe_command):
        ...
        assert call_args[0][0] == subprocess.run  # First argument should be subprocess.run
        assert sys.executable in call_args[0][1]  # Should contain python executable
        assert str(Path(__file__).parent.parent.parent / "scripts" / "generate_cost_badges.py") in str(call_args[0][1])
        ...
        mock_print.assert_called_with("✅ Badge generation completed successfully")
    ...
    def test_badge_command_handles_import_error(self, mock_safe_command):
        ...
        with patch('builtins.__import__', side_effect=ImportError("No module named 'security'")):
    ...
    def test_safe_command_import_success(self, mock_safe_command):
        ...
        try:
            ...
            with patch.dict('sys.modules', {'security': Mock(), 'security.safe_command': mock_safe_command}):
                from security import safe_command
                assert safe_command is not None
    ...
    def test_badge_command_output_handling(self, mock_safe_command):
        ...
        mock_print.assert_called_with("Line 1\nLine 2\nBadge generation complete")
