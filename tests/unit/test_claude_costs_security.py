...
class TestClaudeCostsSecurity(unittest.TestCase):
    ...
    @patch("scripts.claude_costs.safe_command")
    def test_badge_generation_uses_safe_command(
        ...
        with patch("scripts.claude_costs.Path") as mock_path:
            mock_path.return_value.__truediv__.return_value = (
                self.temp_dir / "generate_cost_badge.py"
            )
            ...
            with patch("sys.argv", ["claude_costs.py", "badge"]):
                with patch("builtins.print") as mock_print:
                    ...
                    mock_print.assert_any_call(
                        "❌ Badge generation failed: Error occurred"
                    )
    ...
    def test_security_dependency_available(self):
        ...
        try:
            ...
            self.skipTest(
                "Security package not installed - this is expected in test environment"
            )
    ...
    def test_safe_command_parameters(self, mock_safe_command):
        ...
        self.assertTrue(str(command_list[1]).endswith(
