...
class TestClaudeCostsBadgeGenerationIntegration(unittest.TestCase):
    ...
    def test_complete_badge_generation_workflow(self, mock_path_class, mock_safe_command):
        ...
        mock_script_path.__truediv__ = Mock(return_value=self.temp_dir / "generate_cost_badge.py")
        ...
        with patch('scripts.claude_costs.Path') as mock_path_class:
            ...
            with patch('sys.argv', ['claude_costs.py', 'badge']):
                with patch('builtins.print') as mock_print:
                    ...
                    mock_print.assert_any_call("❌ Badge generation failed: Security check failed")
    ...
    def test_security_wrapper_preserves_original_functionality(self, mock_safe_command):
        ...
        result = mock_safe_command.run(subprocess.run, ['echo', 'test'], capture_output=True)
