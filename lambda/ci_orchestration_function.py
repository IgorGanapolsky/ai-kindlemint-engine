...
class CIOrchestrationEngine:
    ...
    def calculate_fix_confidence(self, failure: Dict[str, Any], category: str) -> float:
        ...
        created_at = datetime.fromisoformat(failure['created_at'].replace('Z', '+00:00'))
        ...
        for fix in analysis['high_confidence_fixes'][:max_fixes]:
            if fix['confidence'] >= confidence_threshold:
                try:
                    ...
                    if success:
                        fixes_applied += 1
                        logger.info(f"Applied high-confidence fix for {fix['category']}")
                except Exception as e:
        ...
        for fix in analysis['medium_confidence_fixes'][:remaining_fixes]:
            if fix['confidence'] >= confidence_threshold:
                try:
                    ...
                    if success:
                        fixes_applied += 1
                        logger.info(f"Applied medium-confidence fix for {fix['category']}")
                except Exception as e:
                    logger.error(f"Failed to apply fix for {fix['run_id']}: {e}")
    ...
    def send_slack_notification(self, result: Dict[str, Any]) -> None:
        ...
        message = {
            ...
            "attachments": [{
                "color": color,
                "fields": [
                    {"title": "Failures Detected", "value": str(result['failures_detected']), "short": True},
                    {"title": "Fixes Applied", "value": str(result['fixes_applied']), "short": True},
                    {"title": "Run Type", "value": result.get('run_type', 'unknown'), "short": True},
