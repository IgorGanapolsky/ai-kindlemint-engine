...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if 'Records' in event:
            ...
            return create_error_response(400, "Invalid event format. Expected S3 trigger or manual CSV content.")
...
def process_kdp_report(csv_content: str, report_date: str) -> Dict[str, Any]:
    ...
    try:
            ...
            try:
                ...
                if book_data:
                    ...
                    if success:
                        ...
                        logger.info(f"Updated book {book_data['book_id']}: {book_data['sales_count']} sales, {book_data['pages_read']} pages")
                    else:
                        failed_books.append({'book_id': book_data['book_id'], 'error': 'DynamoDB update failed'})
                else:
        ...
        logger.info(f"KDP report processing complete: {len(processed_books)} books processed, {len(failed_books)} failed")
        return result
...
def extract_book_data_from_csv_row(row: Dict[str, str]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        book_id = asin if asin else (isbn if isbn else generate_book_id_from_title(title))
        ...
        pages_read = safe_int_conversion(row.get('Pages Read', row.get('KENP Read', '0')))
        royalties = safe_decimal_conversion(row.get('Royalties', row.get('Net Earnings', '0.0')))
...
def update_book_memory(book_data: Dict[str, Any], report_date: str) -> bool:
    ...
    try:
        ...
        estimated_cost = Decimal('50.0')  # Base cost for book creation (adjust as needed)
        actual_revenue = royalties
        calculated_roi = (actual_revenue - estimated_cost) / estimated_cost if estimated_cost > 0 else Decimal('0')
        ...
        logger.info(f"Updated memory for book {book_id}: ROI {calculated_roi:.2%}, Revenue ${actual_revenue:.2f}")
        return True
