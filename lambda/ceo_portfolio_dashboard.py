...
def generate_ceo_report(portfolio_summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        executive_summary = {
            ...
            'monthly_projection': round(portfolio_summary['total_daily_revenue'] * 30, 2)
        }
        ...
        awaiting_approval = [s for s in full_portfolio if s['status'] == 'AWAITING_APPROVAL']
        low_performers = [s for s in full_portfolio 
                         if float(s.get('daily_revenue', 0)) < 10 and s['status'] not in ['AWAITING_APPROVAL', 'CANCELLED']]
