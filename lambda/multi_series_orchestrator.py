...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"📋 SCAN RESULT: Found {len(pending_actions)} series requiring action"
        )
        ...
        for series_data in pending_actions:
            try:
                ...
                logger.error(
                    f"Failed to process series {series_data.get('series_id', 'Unknown')}: {e}"
                )
        ...
        logger.info(
            f"✅ Portfolio orchestration completed: {successful_actions} success, {failed_actions} failed"
        )

        return create_success_response(
            execution_results, f"Processed {len(execution_results)} series actions"
        )
...
def process_series_action(
    series_data: Dict[str, Any], portfolio: PortfolioManager
) -> Dict[str, Any]:
...
def start_next_volume_production(
    series_data: Dict[str, Any], portfolio: PortfolioManager, volume_num: int
) -> Dict[str, Any]:
