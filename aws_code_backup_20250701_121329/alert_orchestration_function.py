...
def __init__(self):
    ...
    def check_lambda_error_rate(self) -> List[Dict[str, Any]]:
        ...
        start_time = end_time - timedelta(
            minutes=self.config["monitoring"]["lookback_minutes"]
        )
    ...
    def calculate_alert_priority(self, alert: Dict[str, Any]) -> float:
        ...
        if alert.get("last_seen"):
            last_seen = datetime.fromisoformat(
                alert["last_seen"].replace("Z", "+00:00")
            )
            hours_old = (
                datetime.utcnow().replace(tzinfo=last_seen.tzinfo) - last_seen
            ).total_seconds() / 3600
    ...
    def is_auto_resolvable(self, alert: Dict[str, Any]) -> bool:
        ...
        return any(
            resolvable_type in alert_type for resolvable_type in auto_resolvable_types
        )
                    ...
                    "fields": [
                        {
                            ...
                            "value": str(result["alerts_processed"]),
                            "short": True,
                        },
                        {
                            "title": "Auto-Resolutions",
                            "value": str(result["resolutions_applied"]),
                            "short": True,
                        },
                        {
                            "title": "Escalations",
                            "value": str(result.get("escalations_created", 0)),
                            "short": True,
                        },
                        {
                            "title": "Duration",
