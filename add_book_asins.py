...
def add_asins():
    ...
    if not active_books_file.exists():
        print("❌ No active books file found. Please run the orchestration system first.")
        return
    ...
    for series, book_list in series_books.items():
        ...
        print("-" * 40)
        for book_id, book_info in sorted(book_list, key=lambda x: x[1].get('volume', '')):
    ...
    for series in ['Large_Print_Crossword_Masters', 'Large_Print_Sudoku_Masters']:
        ...
        for book_id, book_info in sorted(series_books[series], key=lambda x: x[1].get('volume', '')):
            volume = book_info.get('volume', '').replace('volume_', 'Volume ')
            ...
            asin = input(f"\nEnter ASIN for {volume} (or press Enter to skip): ").strip()
            ...
            if asin:
                ...
                if len(asin) == 10 and asin.startswith('B'):
                    ...
                    print(f"⚠️  Invalid ASIN format. ASINs should be 10 characters starting with 'B'")
