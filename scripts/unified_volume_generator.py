...
class BookLayout:
    ...
    def _set_page_number(self, page_num):
        ...
        self.canvas.drawCentredString(self.page_width / 2, self.bottom_margin / 2, str(page_num))

    def _create_title_page(self):
        self.canvas.setFont(self.title_font, self.font_sizes.get('large_title', 20) + 10)
        self.canvas.drawCentredString(self.page_width / 2, self.page_height - 2.5 * inch, self.config['series_name'])
        self.canvas.setFont(self.title_font, self.font_sizes.get('title', 18) + 6)
        self.canvas.drawCentredString(self.page_width / 2, self.page_height - 3.5 * inch, f"Volume {self.config['volume_num']}")
        self.canvas.setFont(self.body_font, self.font_sizes.get('subtitle', 14))
        self.canvas.drawCentredString(self.page_width / 2, self.page_height - 4.5 * inch, f"{len(self.puzzle_data)} {self.config['difficulty'].title()} Puzzles")
    ...
    def _create_copyright_page(self):
        ...
        self.canvas.drawString(self.gutter_margin, self.page_height - 2.5 * inch, "No part of this publication may be reproduced, distributed, or transmitted in any form.")
        self.canvas.showPage()
    ...
    def _create_intro_page(self, title, text):
        ...
        self.canvas.drawCentredString(self.page_width / 2, self.page_height - 1.5 * inch, title)
        self.canvas.setFont(self.body_font, self.font_sizes.get('body', 12))
        text_obj = self.canvas.beginText(self.gutter_margin, self.page_height - 2.5 * inch)
            ...
            for c in range(grid_size):
                ...
                if cell_content == '#':
                    ...
                    for pos, num in grid_data.get('clue_positions', {}).items():
                        ...
                        if r == row and c == col:
                            self.canvas.setFont(self.body_font, self.font_sizes.get('grid_numbers', 9))
                    ...
                    if solution_mode and 'solution_grid' in grid_data:
                        ...
                        if letter.isalpha():
                            ...
                            self.canvas.drawCentredString(x + cell_size / 2, y + cell_size / 2 - 5, letter)
    ...
    def _create_puzzle_pages(self):
        ...
        for puzzle in self.puzzle_data:
            ...
            self.canvas.drawCentredString(self.page_width / 2, self.page_height - 1.5 * inch, f"Puzzle {puzzle_id}")
            grid_total_size = 15 * config.get('puzzle_generation.crossword.cell_size_points', 18.72)
            ...
            # Right page: Clues
            self.canvas.setFont(self.title_font, self.font_sizes.get('title', 18))
            self.canvas.drawCentredString(self.page_width / 2, self.page_height - 1.5 * inch, f"Clues for Puzzle {puzzle_id}")
    ...
    def _create_solution_pages(self):
        ...
        for i in range(0, len(self.puzzle_data), puzzles_per_page):
            ...
            for idx, puzzle in enumerate(chunk):
                ...
                self._draw_grid(x_offset, y_offset - 0.2 * inch, puzzle, solution_mode=True)
...
class UnifiedVolumeGenerator:
    ...
    def __init__(self, series_name: str, volume_nums: List[int], puzzle_type: str, difficulty: str, book_format: str, skip_qa: bool):
    ...
    def _run_puzzle_engine(self, volume_dir: Path, vol_num: int) -> List[Dict[str, Any]]:
        ...
        cmd = [sys.executable, str(self.engine_script), '--output', str(volume_dir), '--count', str(puzzle_count), '--difficulty', self.difficulty]
    ...
    def _generate_book_format(self, volume_dir: Path, vol_num: int, fmt: str, puzzle_data: List[Dict[str, Any]]):
        ...
        if not kdp_specs: raise ValueError(f"No KDP specifications for format '{fmt}'.")
        volume_config = {'series_name': self.series_name, 'volume_num': vol_num, 'year': datetime.now().year, 'pdf_path': pdf_path, 'puzzle_data': puzzle_data, **kdp_specs}
    ...
    def _run_qa_validation(self, volume_dir: Path):
        ...
        result = subprocess.run(cmd, capture_output=True, text=True, cwd=project_root)
        if result.returncode != 0:
            logger.warning(f"QA validation reported issues for {volume_dir.name}:\n{result.stdout}")
        else:
...
def _parse_volume_range(vol_str: str) -> List[int]:
    ...
    for part in vol_str.split(','):
        if '-' in part:
            ...
            except ValueError: raise argparse.ArgumentTypeError(f"Invalid range: '{part}'")
        else:
            try: volumes.add(int(part))
            except ValueError: raise argparse.ArgumentTypeError(f"Invalid volume: '{part}'")
    return sorted(list(volumes))

def main():
    """Main entry point for the unified volume generator script."""
    parser = argparse.ArgumentParser(description="Unified Puzzle Book Volume Generator.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--series', required=True, help="Name of the book series.")
    parser.add_argument('--volumes', required=True, type=_parse_volume_range, help="Volume numbers (e.g., '1-5' or '1,3,5').")
    parser.add_argument('--puzzle-type', default='crossword', choices=['crossword'], help="Type of puzzle.")
    parser.add_argument('--difficulty', default='mixed', choices=['easy', 'medium', 'hard', 'mixed'], help="Puzzle difficulty.")
    parser.add_argument('--format', default='paperback', choices=['paperback', 'hardcover', 'both'], help="Book format(s).")
    parser.add_argument('--skip-qa', action='store_true', help="Skip the final QA validation step.")
    
    args = parser.parse_args()
    logger.info("🚀 Starting Unified Volume Generator...")
    generator = UnifiedVolumeGenerator(args.series, args.volumes, args.puzzle_type, args.difficulty, args.format, args.skip_qa)
