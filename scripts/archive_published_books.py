...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Archive published books to GitHub LFS"
    )
    parser.add_argument(
        "--list", action="store_true", help="List all books in active production"
    )
    parser.add_argument("--series", help="Archive all volumes in a series")
    parser.add_argument(
        "--book", nargs=2, metavar=("SERIES", "VOLUME"), help="Archive a specific book"
    )
    parser.add_argument(
        "--keep-puzzles",
        action="store_true",
    ...
    parser.add_argument(
        "--commit", action="store_true", help="Commit archives to Git after archiving"
    )
    ...
    if args.list:
        ...
        print("\n💡 Usage examples:")
        print(
            "  Archive a series: python scripts/archive_published_books.py --series Large_Print_Sudoku_Masters"
        )
        print(
            "  Archive one book: python scripts/archive_published_books.py --book Large_Print_Sudoku_Masters volume_1"
        )
        print(
            "  Keep puzzle images: python scripts/archive_published_books.py --series Large_Print_Sudoku_Masters --keep-puzzles"
        )
