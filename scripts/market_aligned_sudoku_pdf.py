...
class MarketAlignedSudokuPDF:
    ...
    def create_puzzle_page(self, story, puzzle_data, puzzle_number):
        ...
        # Add VARIED instructions for customers - CRITICAL for usability
        instructions = self.get_varied_instructions(difficulty.lower(), puzzle_number)
        story.append(Paragraph(instructions, self.styles["Instructions"]))

        # Add VARIED solving tip based on difficulty
        tip = self.get_varied_tips(difficulty.lower(), puzzle_number)
        story.append(Paragraph(tip, self.styles["LargeBody"]))
        story.append(Spacer(1, 0.3 * inch))
