...
def freeze_current_environment():
    ...
    if result.returncode == 0:
        ...
        with open(frozen_file, "w") as f:
            f.write("# Frozen requirements - " + subprocess.run(
...
def main():
    ...
    freeze_parser = subparsers.add_parser(
        "freeze", 
        help="Freeze current environment to requirements-frozen.txt"
    )
    ...
    check_parser = subparsers.add_parser(
        "check",
        help="Check installed versions against pinned requirements"
    )
