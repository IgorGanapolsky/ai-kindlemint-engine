...
class EmergencyVisualValidator:
            ...
            for page_num in puzzle_pages:
                if page_num < len(pdf):
                    ...
                    if issues:
                        ...
                        print(f"❌ Found {len(issues)} critical issues on page {page_num}")
                        for issue in issues:
                            print(f"  • {issue}")
    ...
    def _analyze_puzzle_page(self, pdf: fitz.Document, page_num: int) -> List[str]:
        ...
        try:
            ...
            h, w = img_gray.shape
            grid_area = img_gray[int(h*0.2):int(h*0.8), int(w*0.2):int(w*0.8)]
            ...
            if filled_cells > 45:  # More than half the grid filled
                issues.append(f"Too many numbers visible ({filled_cells}/81 cells) - appears to show solution")
            
            if empty_cells < 30:  # Too few empty cells
                issues.append(f"Too few empty cells ({empty_cells}/81) - puzzle not solvable")
            ...
            if filled_cells > 0:
                ...
                if not has_distinction:
                    issues.append("No visual distinction between clues and empty cells - all numbers look identical")
            ...
            if filled_cells == 81:
                issues.append("Complete grid detected - showing full solution instead of puzzle")
            ...
            for col in range(9):
                ...
                if cell.size > 0:
                    ...
                    if dark_pixels > cell.size * 0.05:  # Has a number
                        ...
                        thickness_ratio = very_dark_pixels / dark_pixels if dark_pixels > 0 else 0
