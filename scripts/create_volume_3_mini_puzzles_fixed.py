...
def create_mini_puzzles():
    ...
    for puzzle_num, puzzle in enumerate(mini_puzzles, 1):
        ...
        c.drawCentredString(
            ...
            f"Bonus Mini Puzzle {puzzle_num}",
        )
                ...
                if (row, col) in puzzle["black"]:
                    ...
                    if col == 0 or (row, col - 1) in puzzle["black"]:
                        if (
                            ...
                            and (row, col + 1) not in puzzle["black"]
                        ):
                            needs_number = True
                    if row == 0 or (row - 1, col) in puzzle["black"]:
                        if (
                            row < mini_grid_size - 1
                            and (row + 1, col) not in puzzle["black"]
                        ):
                            needs_number = True
...
def replace_mini_puzzles():
    ...
    # Read existing PDF
    input_path = Path(
        "books/active_production/Large_Print_Crossword_Masters/volume_3/paperback/crossword_book_volume_3_FINAL.pdf"
    )
    output_path = Path(
        "books/active_production/Large_Print_Crossword_Masters/volume_3/paperback/crossword_book_volume_3_FINAL_FIXED.pdf"
    )
    ...
    # Also fix hardcover
    hardcover_input = Path(
        "books/active_production/Large_Print_Crossword_Masters/volume_3/hardcover/crossword_book_volume_3_FINAL.pdf"
    )
    hardcover_output = Path(
        "books/active_production/Large_Print_Crossword_Masters/volume_3/hardcover/crossword_book_volume_3_FINAL_FIXED.pdf"
    )
