...
class SudokuClueRenderingFixer:
    ...
    def create_proper_puzzle_image(self, initial_grid, puzzle_id):
        ...
        for font_path in font_paths:
            try:
                clue_font = ImageFont.truetype(font_path, 42)  # Larger, bold font for clues
                normal_font = ImageFont.truetype(font_path, 36)  # Smaller font if needed
                break
            except:
                continue
        ...
        print(f"✅ Fixed Puzzle {puzzle_id}: {clues_drawn} clues rendered (empty cells left blank)")
    ...
    def fix_all_puzzles(self):
        ...
        for metadata_file in metadata_files:
            ...
            fixed_path, clues_drawn = self.create_proper_puzzle_image(initial_grid, puzzle_id)
            ...
            if clues_drawn == expected_clues:
                ...
                if original_path.exists():
                    # Backup original
                    backup_path = self.puzzles_dir / f"sudoku_puzzle_{puzzle_id:03d}_OLD.png"
                ...
                print(f"❌ Puzzle {puzzle_id}: Clue count mismatch ({clues_drawn} vs {expected_clues})")
...
def main():
    if len(sys.argv) != 2:
        ...
        print("Example: python fix_sudoku_clue_rendering.py books/active_production/Large_Print_Sudoku_Masters/volume_1")
        sys.exit(1)
