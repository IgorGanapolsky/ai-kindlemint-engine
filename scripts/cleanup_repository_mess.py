...
def move_documentation_files(root):
    ...
    file_mappings = {
        "docs/architecture": [
            "*ARCHITECTURE*", "*DESIGN*", "*SYSTEM*", "*UNIFIED*"
        ],
        "docs/infrastructure": [
            "*INFRASTRUCTURE*", "*DEPLOYMENT*", "*DEVOPS*", "*CODECOV*", 
            "*SONAR*", "*CI*", "*CD*"
        ],
        "docs/guides": [
            "*GUIDE*", "*MANUAL*", "*SETUP*", "*GETTING*", "*QUICK*"
        ],
        "docs/analysis": [
            "*ANALYSIS*", "*REPORT*", "*SUMMARY*", "*MIGRATION*", 
            "*QA*", "*FAILURE*", "*TECHNICAL*"
        ],
        "docs/api": [
    ...
    for md_file in md_files:
        ...
        for subdir, patterns in file_mappings.items():
            if any(pattern.replace("*", "").upper() in md_file.name.upper() 
...
def organize_config_files(root):
    ...
    keep_in_root = {
        ".gitignore", ".deepsource.toml", "pyproject.toml", 
    ...
    for pattern in config_patterns:
        for config_file in root.glob(pattern):
            if (config_file.is_file() and 
                config_file.name not in keep_in_root and
                not config_file.name.startswith('.')):
