...
class SudokuRenderingFixer:
    ...
    def diagnose_puzzle(self, puzzle_id):
        ...
        if json_clues != declared_clues:
            issue["issue"] = f"JSON mismatch: actual {json_clues} vs declared {declared_clues}"
        elif png_clues != -1 and abs(png_clues - json_clues) > 5:  # Allow some detection error
    ...
    def fix_all_issues(self):
        ...
        for puzzle_id in range(1, 11):
            ...
            if "issue" in result:
                ...
                print(f"✅ Puzzle {puzzle_id}: OK ({result['json_actual_clues']} clues)")
