...
class MarketToPublishOrchestrator:
    ...
    async def _run_market_research(self):
        ...
        try:
            ...
            result = await self._run_subprocess([
                'python', 'scripts/synthetic_market_research.py',
                '--output', 'research',
                '--niches', '20'
            ])
    ...
    async def _analyze_opportunities(self):
        ...
        try:
            ...
            result = await self._run_subprocess([
                'python', 'scripts/market_research_auto_reviewer.py',
                '--auto'
            ])
            ...
            if tasks_file.exists():
                with open(tasks_file, 'r') as f:
                    ...
                    self.pipeline_state['selected_niches'] = [
                        task['niche'] for task in tasks 
                        if task.get('status') == 'pending'
                    ]
            
            stage_data['selected_count'] = len(self.pipeline_state['selected_niches'])
            stage_data['status'] = 'completed'
            print(f"✅ Selected {stage_data['selected_count']} niches for content generation")
    ...
    async def _plan_content(self):
        ...
        try:
            for niche in self.pipeline_state['selected_niches'][:3]:  # Limit to 3 for demo
    ...
    async def _generate_content(self):
        ...
        try:
            content_plans = self.pipeline_state['stages']['content_planning']['content_plans']
    ...
    async def _prepare_for_publishing(self):
        ...
        try:
            qa_results = self.pipeline_state['stages']['quality_assurance']['qa_results']
            ...
            stage_data['status'] = 'completed'
            print(f"✅ Prepared {len(stage_data['prepared_books'])} books for publishing")
    ...
    async def _final_review(self):
        ...
        try:
            prepared_books = self.pipeline_state['stages']['publishing_preparation']['prepared_books']
            ...
            stage_data['status'] = 'completed'
            print(f"✅ {len(stage_data['approved_for_publishing'])} books approved for publishing")
