...
try:
    from scripts.sentry_config import init_sentry, add_breadcrumb, capture_kdp_error
    ...
    def capture_kdp_error(*args, **kwargs): pass
...
try:
    ...
    logger.error("Critical QA dependencies missing. Run: pip install PyPDF2 Pillow PyMuPDF")
    sys.exit(1)
...
try:
    ...
    OPENAI_VALIDATION_KEY = os.getenv("OPENAI_VALIDATION_KEY", os.getenv("OPENAI_API_KEY"))
    if OPENAI_VALIDATION_KEY:
        ...
        logger.warning("OpenAI API not available for validation. Check OPENAI_VALIDATION_KEY.")
...
try:
    ...
    if GEMINI_API_KEY:
        ...
        logger.warning("Gemini API not available for validation. Check GEMINI_API_KEY or GEMINI_BACKUP_KEY.")
except Exception as e:
...
class EnhancedQAValidator:
    ...
    def __init__(self):
        ...
        if not OPENAI_AVAILABLE and not GEMINI_AVAILABLE:
            logger.warning("No LLM validation services available. QA will be limited to basic checks.")
            self._add_issue("No LLM validation services available", "QA_NO_LLM", 
                           "Configure OPENAI_VALIDATION_KEY or GEMINI_BACKUP_KEY for full validation.", 
                           level="WARNING")

    def _add_issue(self, issue_type: str, code: str, description: str, level: str = "CRITICAL"):
        ...
        if SENTRY_AVAILABLE:
            add_breadcrumb(description, category="qa_issue", level=level.lower(), data={"code": code})
    ...
    def run_enhanced_qa(self, pdf_path: str) -> Dict:
        ...
        try:
            ...
            self._add_issue("QA_PROCESS_ERROR", "QA process failed", f"QA process failed: {e}", level="CRITICAL")
            if SENTRY_AVAILABLE:
                capture_kdp_error(e, {"qa_file": str(pdf_path), "stage": "overall_process"})
    ...
    def _check_file_properties(self, pdf_path: Path):
        ...
        if not pdf_path.exists():
            self._add_issue("FILE_NOT_FOUND", "File not found", "PDF file does not exist", level="CRITICAL")
            return
        ...
        if file_size < 10000:
            self._add_issue("FILE_TOO_SMALL", "File too small", 
    ...
    def _check_pdf_structure(self, pdf_path: Path):
        ...
        try:
            with open(pdf_path, 'rb') as file:
                ...
                if num_pages < 5:
                    self._add_issue("LOW_PAGE_COUNT", "Low page count", 
                ...
                if pdf_reader.is_encrypted:
                    self._add_issue("PDF_ENCRYPTED", "PDF encrypted", 
            ...
            self._add_issue("PDF_CORRUPT", "PDF corrupt", 
                           f"Cannot read PDF file: {e}", 
                           level="CRITICAL")
            if SENTRY_AVAILABLE:
                capture_kdp_error(e, {"qa_file": str(pdf_path), "stage": "pdf_structure_check"})
            ...
            for page_num in range(len(doc)):
                ...
                for font in fonts:
                    ...
                    if not is_embedded:
                        ...
                        self._add_issue("FONT_NOT_EMBEDDED", "Font not embedded", 
                                       f"Font '{font_name}' on page {page_num+1} is not embedded", 
                                       level="CRITICAL")
            
            if font_info["all_embedded"]:
                print(f"  ✅ All fonts properly embedded ({len(font_info['fonts'])} fonts)")
            else:
                print(f"  ❌ Some fonts are not embedded - KDP will reject")
                
        except Exception as e:
            self._add_issue("FONT_CHECK_ERROR", "Font check error", 
    ...
    def _extract_text_from_pdf(self, pdf_path: Path) -> str:
        ...
        try:
            ...
            self._add_issue("TEXT_EXTRACTION_FAILED", "Text extraction failed", 
                           f"Could not extract text: {e}", 
                           level="WARNING")
            if SENTRY_AVAILABLE:
                capture_kdp_error(e, {"qa_file": str(pdf_path), "stage": "text_extraction"})
    ...
    def _check_content_quality(self, text_content: str):
        ...
        if len(text_content) < 1000:
            self._add_issue("INSUFFICIENT_CONTENT", "Insufficient content", 
        ...
        if lines:
            ...
            if duplicate_ratio > self.qa_criteria["duplicate_content_threshold"]:
                self._add_issue("HIGH_DUPLICATION", "High duplication", 
    ...
    def _check_puzzle_integrity(self, text_content: str):
        ...
        if clues:
            ...
            if invalid_clues:
                self._add_issue("INVALID_CLUES", "Invalid clues", 
    ...
    def _check_visual_layout(self, pdf_path: Path):
        ...
        try:
            ...
            for page_num in range(page_count):
                ...
                white_pixels = sum(1 for pixel in img_gray.getdata() if pixel > white_threshold)
                ...
                if whitespace_ratio > self.qa_criteria["whitespace_ratio_threshold"]:
                    checks["pages_with_high_whitespace"] = checks.get("pages_with_high_whitespace", 0) + 1
                    print(f"  ⚠️  WARNING: Page {page_num + 1}: {whitespace_ratio*100:.1f}% white - may lack content")
                ...
                for block in text_blocks:
                    for line in block.get("lines", []):
                        for span in line.get("spans", []):
                            ...
                            if (bbox[0] < margin_threshold or 
                                bbox[1] < margin_threshold or
                                bbox[2] > page_rect.width - margin_threshold or
                                bbox[3] > page_rect.height - margin_threshold):
                                
                                if not cutoff_detected:  # Report only once per page
                                    self._add_issue("TEXT_CUTOFF_RISK", "Text cutoff risk", 
                                                  f"Page {page_num + 1}: Text '{text[:20]}...' may be cut off", 
                                                  level="CRITICAL")
                                    checks["text_cutoff_instances"] = checks.get("text_cutoff_instances", 0) + 1
            ...
            whitespace_compliance = 1 - (checks["pages_with_high_whitespace"] / page_count)
            checks["whitespace_compliance"] = whitespace_compliance
            
            if whitespace_compliance < self.qa_criteria["whitespace_page_compliance"]:
                self._add_issue("EXCESSIVE_WHITESPACE", "Excessive whitespace", 
            ...
            if checks["text_cutoff_instances"] == 0:
                ...
                print(f"  ❌ {checks['text_cutoff_instances']} text cut-off issues detected")
                
        except Exception as e:
            self._add_issue("VISUAL_CHECK_ERROR", "Visual check error", 
    ...
    def _check_crossword_grid(self, page, page_num):
        ...
        try:
            ...
            if len(text_blocks) > 0:
                print(f"  ✅ Page {page_num + 1}: Crossword numbers appear properly positioned")
        except Exception as e:
            logger.warning(f"Could not check crossword grid on page {page_num + 1}: {e}")
    ...
    def _llm_content_validation(self, text_content: str, pdf_path: Path):
        ...
        if len(text_content) < 100:
            self._add_issue("INSUFFICIENT_TEXT_FOR_LLM", "Insufficient text for LLM", 
        ...
        if len(text_content) > 3000:
            text_sample = (text_content[:1000] + "\n...\n" + 
                          text_content[len(text_content)//2-500:len(text_content)//2+500] + 
                          "\n...\n" + text_content[-1000:])
        else:
        ...
        if OPENAI_AVAILABLE:
            ...
            if gpt_result:
                ...
                print(f"  ✅ GPT-4o validation complete: {gpt_result.get('quality_score', 'N/A')}/100")
        ...
        if GEMINI_AVAILABLE:
            ...
            if gemini_result:
                ...
                print(f"  ✅ Gemini validation complete: {gemini_result.get('quality_score', 'N/A')}/100")
        
        # Calculate consensus score if we have multiple models
        if len(validation_results) > 1:
            # Simple average for now - could be weighted in the future
            quality_scores = [r.get("quality_score", 0) for r in validation_results.values()]
            ...
            for model, result in validation_results.items():
                for issue in result.get("issues", []):
                    self._add_issue(f"LLM_{model.upper()}_ISSUE", f"LLM {model} issue", 
                                   issue, level="CRITICAL")
                
                for warning in result.get("warnings", []):
                    self._add_issue(f"LLM_{model.upper()}_WARNING", f"LLM {model} warning", 
                                   warning, level="WARNING")
        elif len(validation_results) == 1:
            # Use the single model's score
            model = list(validation_results.keys())[0]
            checks["consensus_score"] = validation_results[model].get("quality_score", 0)
            ...
            self._add_issue("NO_LLM_VALIDATION", "No LLM validation", 
...
"""
            ...
            response = OPENAI_VALIDATION_CLIENT.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"Here is a sample from a book to validate:\n\n{file_info}\n\nCONTENT SAMPLE:\n{text_sample}"}
                ],
...
"""
            ...
            try:
                ...
                if json_match:
                    ...
                    score_match = re.search(r'quality_score"?\s*:\s*(\d+)', text_response)
                    if score_match:
                        result["quality_score"] = int(score_match.group(1))
                    
                    # Look for issues
                    issues_match = re.search(r'issues"?\s*:\s*\[(.*?)\]', text_response, re.DOTALL)
                    if issues_match:
                    ...
                    # Look for warnings
                    warnings_match = re.search(r'warnings"?\s*:\s*\[(.*?)\]', text_response, re.DOTALL)
                    if warnings_match:
                    ...
                    # Look for feedback
                    feedback_match = re.search(r'feedback"?\s*:\s*"(.*?)"', text_response, re.DOTALL)
                    if feedback_match:
    ...
    def _check_kdp_compliance(self, pdf_path: Path):
        ...
        try:
            ...
            if file_size_mb > 650:
                self._add_issue("FILE_TOO_LARGE", "File too large", 
            ...
            if pdf_version < 1.3 or pdf_version > 1.7:
                self._add_issue("INVALID_PDF_VERSION", "Invalid PDF version", 
            ...
            if has_transparency:
                self._add_issue("PDF_HAS_TRANSPARENCY", "PDF has transparency", 
                               "PDF contains transparency which may cause issues with KDP", 
                               level="WARNING")
            
        except Exception as e:
            self._add_issue("KDP_CHECK_ERROR", "KDP check error", 
    ...
    def _calculate_overall_score(self):
        ...
        llm_score = self.qa_results["checks"]["llm_content_validation"].get("consensus_score")
        ...
        print(f"{'✅ READY' if self.qa_results['publish_ready'] else '❌ NOT READY'}: {'Book meets quality standards' if self.qa_results['publish_ready'] else 'Fix all critical issues before publishing'}")
        print("\n" + "=" * 70)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Enhanced QA Validator - Multi-Model AI Content Quality Assurance")
    parser.add_argument("pdf_path", help="Path to the PDF file to validate")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose output")
    parser.add_argument("--output", help="Path to save the QA report (default: same directory as PDF)")
