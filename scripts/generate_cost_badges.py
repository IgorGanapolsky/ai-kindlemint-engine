...
def update_readme_badges(badges: dict, analytics: dict):
    ...
    primary_badges.append(f"[![Claude Monthly Cost]({badges['monthly']})](./CLAUDE_COSTS.md)")
    primary_badges.append(f"[![Budget Status]({badges['budget']})](./CLAUDE_COSTS.md#budget)")
    primary_badges.append(f"[![Cost Trend]({badges['trend']})](./CLAUDE_COSTS.md#trends)")
    
    # Secondary row: Daily, Weekly, YTD
    secondary_badges.append(f"[![Daily Average]({badges['daily_avg']})](./CLAUDE_COSTS.md#daily)")
    secondary_badges.append(f"[![Weekly Cost]({badges['weekly']})](./CLAUDE_COSTS.md#weekly)")
    secondary_badges.append(f"[![YTD Cost]({badges['ytd']})](./CLAUDE_COSTS.md#yearly)")
    ...
    if re.search(section_pattern, content):
        ...
        content = re.sub(old_badge_pattern, badge_section.strip(), content, count=1)
...
def main():
    ...
    print(f"  Budget Status: {(cost_data['periods']['month']['cost'] / 50.0 * 100):.0f}% of $50")
