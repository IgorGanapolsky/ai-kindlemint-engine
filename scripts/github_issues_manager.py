...
from kindlemint.agents.task_system import Task, TaskType, TaskPriority
    ...
    async def process_all_issues(self) -> Dict[str, Any]:
        ...
        with track_kdp_operation("github_issues_processing"):
            ...
            if not result.success:
                self._send_error_notification("Failed to generate issues report", result.error_message)
        ...
        for pr in report.get("pull_requests", []):
            if pr["author"]["login"].lower() == "pixeebot" and pr["state"] == "OPEN":
                ...
                results.append({
                    "pr_number": pr["number"],
                    "title": pr["title"],
                    "result": result.output_data if result.success else {"error": result.error_message}
                })
        
        # Process Pixeebot dashboard issues
        for issue in report.get("issues", []):
            if issue["author"]["login"].lower() == "pixeebot" and "Dashboard" in issue.get("title", ""):
                task = Task(
...
async def main():
    ...
    parser.add_argument("--repo", default="IgorGanapolsky/ai-kindlemint-engine", help="GitHub repository")
    parser.add_argument("--process-all", action="store_true", help="Process all open issues and PRs")
