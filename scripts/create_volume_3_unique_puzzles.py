        ...
        for row in range(GRID_SIZE):
            for col in range(GRID_SIZE):
                if solution[row][col] != "#":
                    ...
                    starts_across = (
                        (col == 0 or solution[row][col - 1] == "#")
                        and col + 1 < GRID_SIZE
                        and solution[row][col + 1] != "#"
                    )
                    starts_down = (
                        (row == 0 or solution[row - 1][col] == "#")
                        and row + 1 < GRID_SIZE
                        and solution[row + 1][col] != "#"
                    )
                    ...
                    if starts_across or starts_down:
                        ...
                        for word_info in placed_words:
                            if word_info["row"] == row and word_info["col"] == col:
                                clue = self.get_unique_clue(
                                    word_info["word"], puzzle_num
                                )
...
def create_pdf(puzzles, output_path):
    ...
    for puzzle_num, puzzle in enumerate(puzzles, 1):
        ...
        c.drawCentredString(
            PAGE_WIDTH / 2,
            PAGE_HEIGHT - 72,
            f"Puzzle {puzzle_num}: {puzzle['theme']} Theme",
        )
