...
class MultiAgentBatchProcessor:
    ...
    async def _process_books(self, books_config: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _process_book_multi_agent(self, book_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not puzzle_result or not puzzle_result.success:
                raise RuntimeError(f"Puzzle generation failed: {puzzle_result.error_message if puzzle_result else 'Unknown error'}")
            ...
            pdf_task = Task(
                ...
                input_data={
                    ...
                    "output_dir": puzzle_result.artifacts["puzzles_dir"].replace("/puzzles", "/paperback"),
                    "puzzle_type": book_config.get("puzzle_type", "crossword"),
                    "book_id": book_id,
                },
                priority=TaskPriority.HIGH,
            ...
            if not pdf_result or not pdf_result.success:
                raise RuntimeError(f"PDF layout failed: {pdf_result.error_message if pdf_result else 'Unknown error'}")
            ...
            if not qa_result or not qa_result.success:
                raise RuntimeError(f"QA validation failed: {qa_result.error_message if qa_result else 'Unknown error'}")
            ...
            book_result["publish_ready"] = qa_result.output_data.get("publish_ready", False)
            ...
            book_result["duration_seconds"] = (datetime.now() - start_time).total_seconds()
    ...
    async def _generate_batch_report(
        ...
        self.logger.info(f"📊 Batch processing completed: {successful}/{len(results)} books successful")
        return batch_report
...
async def main():
    ...
    try:
        ...
        print(f"   📚 Books: {ma_results['books_succeeded']}/{ma_results['books_processed']}")
        ...
        print(f"   📚 Books: {legacy_results['books_succeeded']}/{legacy_results['books_processed']}")
        ...
        if legacy_results['total_duration_seconds'] > 0:
            improvement = (
                (legacy_results['total_duration_seconds'] - ma_results['total_duration_seconds']) 
                / legacy_results['total_duration_seconds'] * 100
            )
