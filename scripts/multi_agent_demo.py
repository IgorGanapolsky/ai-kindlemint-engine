...
class MultiAgentBookGenerator:
    ...
    async def generate_book_individual_tasks(self, book_config: Dict) -> bool:
        ...
        """
        self.logger.info(f"📚 Generating book with individual tasks: {book_config['title']}")
        ...
        try:
            ...
            self.logger.info(f"   🎯 QA Score: {qa_result.output_data['overall_score']}/100")
            self.logger.info(f"   ✅ Publish Ready: {qa_result.output_data['publish_ready']}")
    ...
    async def run_performance_demo(self) -> None:
        ...
        self.demo_metrics["books_failed"] = len(books) - self.demo_metrics["books_succeeded"]
    ...
    async def _create_agents(self) -> None:
        ...
        pdf_agent = PDFLayoutAgent(
            agent_id="pdf_layout_01",
            max_concurrent_tasks=1
        )
        
        epub_agent = EPUBGeneratorAgent(
            agent_id="epub_generator_01",
            max_concurrent_tasks=2
        )
        
        qa_agent = QualityAssuranceAgent(
            agent_id="qa_validator_01",
    ...
    async def _monitor_workflow_progress(self, execution_id: str, timeout: int = 600) -> None:
        ...
        while (datetime.now() - start_time).total_seconds() < timeout:
            ...
            if status != last_status:
                self.logger.info(f"   📋 Workflow status: {status.value if status else 'unknown'}")
                last_status = status
        ...
        raise TimeoutError(f"Workflow {execution_id} did not complete within {timeout} seconds")
    ...
    async def _display_system_status(self) -> None:
        ...
        self.logger.info(f"✅ Completed Tasks: {coordination_metrics['completed_tasks']}")
        self.logger.info("="*60)
    
    async def _display_performance_summary(self) -> None:
        """Display performance demonstration summary"""
        duration = (self.demo_metrics["end_time"] - self.demo_metrics["start_time"]).total_seconds()
        ...
        self.logger.info(f"📈 Books per Minute: {(self.demo_metrics['books_processed'] / duration * 60):.1f}")
        
        success_rate = (self.demo_metrics['books_succeeded'] / self.demo_metrics['books_processed'] * 100)
