...
class SmartCIMonitor:
    ...
    def detect_cascades(self, patterns):
        ...
        for workflow, events in patterns.items():
            ...
            cascade_count = sum(events.get(trigger, 0) for trigger in self.cascade_patterns)
        ...
        return sorted(cascading_workflows, key=lambda x: x['total_failures'], reverse=True)
    ...
    def auto_fix_cascades(self, cascading_workflows):
        ...
        for cascade in cascading_workflows:
            ...
            if cascade['total_failures'] > 20:
                subprocess.run(f'gh workflow disable "{workflow_name}"', shell=True)
...
"""
                subprocess.run(f"""gh issue create --title "CI: {workflow_name} auto-disabled" --body '{issue_body}' --label "ci-health,auto-generated" """, shell=True)
    ...
    def generate_report(self):
        ...
        if cascades:
            ...
            for cascade in cascades:
                print(f"   - {cascade['workflow']}: {cascade['total_failures']} failures")
