...
class CIMonitor:
    ...
    def categorize_failure(self, job: Dict, logs: str) -> Dict:
        ...
        if logs:
            ...
            if 'modulenotfounderror' in logs_lower or 'importerror' in logs_lower:
                ...
                # Extract module name
                import_match = self._extract_error_pattern(logs, r"No module named '([^']+)'")
                if import_match:
                    failure_info['error_message'] = f"Missing module: {import_match}"
                    failure_info['suggested_fix'] = f"Add missing import for {import_match}"
            
            # Test failures
            elif 'failed' in logs_lower and ('test' in logs_lower or 'pytest' in logs_lower):
                failure_info['failure_type'] = 'test_failure'
                failure_info['error_message'] = "Unit or integration tests failed"
                failure_info['suggested_fix'] = "Fix failing test assertions or update test data"
            ...
            # Linting errors
            elif 'flake8' in logs_lower or 'black' in logs_lower or 'isort' in logs_lower:
                failure_info['failure_type'] = 'linting_error'
                failure_info['error_message'] = "Code style or linting violations"
                failure_info['suggested_fix'] = "Run formatters and fix style violations"
            ...
            # Type errors
                ...
                if type_match:
                    ...
                    failure_info['suggested_fix'] = "Fix type annotations or type mismatches"
            ...
            # Dependency errors
                ...
                failure_info['suggested_fix'] = "Update requirements.txt or resolve conflicts"
            
            # Path/File errors
            elif 'filenotfounderror' in logs_lower or 'no such file' in logs_lower:
                failure_info['failure_type'] = 'path_error'
                path_match = self._extract_error_pattern(logs, r"No such file or directory: '([^']+)'")
                if path_match:
                    failure_info['error_message'] = f"Missing file: {path_match}"
                    failure_info['suggested_fix'] = f"Create missing file or fix path to {path_match}"
    ...
    def save_failure_report(self, failures: List[Dict], output_path: str = 'ci_failures.json'):
...
def main():
    ...
    if args.watch:
        ...
        logger.info(f"Starting continuous monitoring (checking every {args.interval} seconds)")
        while True:
            try:
