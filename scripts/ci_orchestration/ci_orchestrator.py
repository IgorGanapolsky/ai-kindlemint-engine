...
class CIOrchestrator:
    ...
    def run_single_cycle(self) -> Dict:
        ...
        try:
            ...
            if (not self.dry_run and 
                results['fixes_applied'] > 0 and 
                self.config['fixing']['enable_auto_commit']):
            ...
            logger.info(f"Orchestration cycle completed in {results['cycle_duration']:.2f} seconds")
    ...
    def _apply_fixes(self, analysis_results: Dict) -> Dict:
        ...
        for analyzed_failure in analysis_results.get('analyzed_failures', []):
            ...
            for strategy in strategies:
                ...
                if (strategy.get('confidence', 0) >= confidence_threshold and 
                    strategy.get('auto_fixable', False)):
                    
                    logger.info(f"Applying fix: {strategy.get('description')}")
                    
                    if self.fixer.apply_fix_strategy(strategy):
                        fixes_applied += 1
                        logger.info(f"Successfully applied fix {fixes_applied}/{max_fixes}")
                    else:
                        logger.warning(f"Failed to apply fix: {strategy.get('description')}")
    ...
    def _validate_fixes(self) -> Dict:
        ...
        try:
            ...
            import_cmd = "python -c 'import src.kindlemint; print(\"Import successful\")'"
...
"""
            ...
            if success:
                ...
                pr_url_match = re.search(r'https://github\.com/[^/]+/[^/]+/pull/(\d+)', stdout)
                if pr_url_match:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='CI Orchestration - Automated CI failure detection and fixing')
    
    # Repository settings
    parser.add_argument('--owner', default='igorganapolsky', help='Repository owner')
    parser.add_argument('--repo', default='ai-kindlemint-engine', help='Repository name')
    ...
    # Overrides
    parser.add_argument('--lookback-minutes', type=int, help='Minutes to look back for failures')
    parser.add_argument('--max-fixes', type=int, help='Maximum fixes to apply per cycle')
    parser.add_argument('--confidence-threshold', type=float, help='Minimum confidence for auto-fix')
    ...
    if args.confidence_threshold:
        orchestrator.config['fixing']['auto_fix_confidence_threshold'] = args.confidence_threshold
