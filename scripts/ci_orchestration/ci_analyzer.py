...
class CIAnalyzer:
        ...
        if failure_type in self.error_patterns:
            ...
            for pattern in pattern_info['patterns']:
                ...
                if matches:
                    ...
                    if strategy:
                        strategies.extend(strategy if isinstance(strategy, list) else [strategy])
    ...
    def _analyze_import_error(self, matches: List, failure_info: Dict) -> List[FixStrategy]:
    ...
    def _analyze_test_failure(self, matches: List, failure_info: Dict) -> List[FixStrategy]:
    ...
    def _analyze_syntax_error(self, matches: List, failure_info: Dict) -> List[FixStrategy]:
    ...
    def _analyze_linting_error(self, matches: List, failure_info: Dict) -> List[FixStrategy]:
        ...
        if flake8_files:
            strategies.append(FixStrategy(
                ...
                commands=['autopep8 --in-place --aggressive ' + ' '.join(flake8_files)],
                estimated_complexity='low',
                auto_fixable=True
            ))
    ...
    def _analyze_type_error(self, matches: List, failure_info: Dict) -> List[FixStrategy]:
    ...
    def _analyze_dependency_error(self, matches: List, failure_info: Dict) -> List[FixStrategy]:
        ...
        for match in matches:
            ...
            strategies.append(FixStrategy(
                ...
                confidence=0.8,
                files_to_modify=['requirements.txt'],
                commands=[f"pip install {package_name}", "pip freeze > requirements-temp.txt"],
                estimated_complexity='low',
                auto_fixable=True
            ))
    ...
    def _analyze_path_error(self, matches: List, failure_info: Dict) -> List[FixStrategy]:
    ...
    def _is_local_module(self, module_name: str) -> bool:
        ...
        return any(module_name.startswith(prefix) for prefix in local_prefixes) or '.' not in module_name
    ...
    def prioritize_strategies(self, strategies: List[FixStrategy]) -> List[FixStrategy]:
        ...
        return sorted(
            strategies,
            key=lambda s: (-s.confidence, complexity_order.get(s.estimated_complexity, 3))
        )
    ...
    def save_analysis(self, analysis_results: Dict, output_path: str = 'ci_analysis.json'):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Analyze CI failures and suggest fixes')
    parser.add_argument('--input', default='ci_failures.json', help='Input failure report')
    parser.add_argument('--output', default='ci_analysis.json', help='Output analysis file')
