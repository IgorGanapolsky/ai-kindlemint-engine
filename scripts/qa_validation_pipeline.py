...
class QAValidationPipeline:
    ...
    def validate_pdf(self, pdf_path: Path, book_type: str = "crossword") -> QAResult:
        ...
        if book_type == "crossword":
            ...
            criteria_results["puzzle_integrity"] = {
                ...
                "passed": puzzle_score >= self.qa_criteria["puzzle_integrity"]["threshold"],
    ...
    def _check_duplicate_content(self, pdf_path: Path) -> Tuple[float, List[Dict]]:
        ...
        try:
            ...
            if result.returncode != 0:
                return 0.0, [{"type": "error", "message": "Could not extract text from PDF"}]
            ...
            return 0.0, [{"type": "error", "message": f"Duplicate check failed: {str(e)}"}]
    ...
    def _check_white_space(self, pdf_path: Path) -> Tuple[float, List[Dict]]:
        ...
        try:
            ...
            if result.returncode != 0:
                return 0.0, [{"type": "error", "message": "Could not analyze white space"}]
            ...
            for i, page in enumerate(pages):
                if page.strip():
                    ...
                    ws_ratio = (white_chars / total_chars) * 100 if total_chars > 0 else 100
            ...
            avg_ratio = sum(p["ratio"] for p in high_whitespace_pages) / len(high_whitespace_pages) if high_whitespace_pages else 0
            
            return avg_ratio, high_whitespace_pages[:5]
            
        except Exception as e:
            return 0.0, [{"type": "error", "message": f"White space check failed: {str(e)}"}]
    ...
    def _check_font_embedding(self, pdf_path: Path) -> Tuple[float, List[Dict]]:
        ...
        try:
            ...
            embedded_percentage = ((len(lines) - len(unembedded)) / len(lines) * 100) if lines else 100
            ...
            if criterion in weights:
                ...
                if result["passed"]:
                    ...
                    if criterion in ["duplicate_content", "white_space_ratio"]:
                        # Lower is better
                        score = max(0, 100 - (result["score"] - result["threshold"]) * 5)
                    else:
                        # Higher is better
    ...
    def _generate_recommendations(self, criteria_results: Dict, issues: List) -> List[str]:
        ...
        if not criteria_results.get("white_space_ratio", {}).get("passed", True):
            recommendations.append("Add more content or adjust layout to reduce white space")
    ...
    def _print_qa_summary(self, result: QAResult):
        ...
        for criterion, details in result.criteria.items():
            ...
            print(f"{status} {criterion}: {details['score']} (threshold: {details['threshold']})")
        
        # Issues summary
        if result.issues_found:
            print(f"\n⚠️  Issues Found ({len(result.issues_found)}):")
            for issue in result.issues_found[:5]:
                print(f"  - {issue.get('type', 'Unknown')}: {issue.get('message', issue)}")
    ...
    def _save_qa_report(self, result: QAResult, pdf_path: Path):
        ...
        report_path = qa_dir / f"qa_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
