...
class OrchestrationDaemon:
    ...
    async def _run_with_recovery(self):
        ...
        while self.running:
            try:
                ...
                self.logger.info(f"Starting orchestration system (attempt #{self.restart_count + 1})")
                ...
                self.pr_processor_task = asyncio.create_task(self._process_pr_backlog())
                ...
                wait_time = min(60 * (2 ** min(self.restart_count - 1, 5)), 300)
        ...
        while self.running:
            try:
                ...
                if self.orchestration:
                    ...
                    active_agents = sum(1 for agent in self.orchestration.agents 
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    for pr in open_prs:
                        ...
                        if 'sentry-ai-completed' in labels and 'sentry-ai-reviewed' in labels:
    ...
    async def _send_critical_alert(self, message: str):
        ...
        # Write to alert file
        alert_file = Path(__file__).parent.parent / "logs" / "critical_alerts.log"
        ...
        try:
            subprocess.run(
                ["gh", "issue", "create", "--title", f"Critical Alert: {message}",
                 "--body", f"Orchestration system alert:\n\n{message}\n\nTime: {datetime.now()}",
                 "--label", "critical,orchestration"],
                timeout=30
            )
        except:
            pass
