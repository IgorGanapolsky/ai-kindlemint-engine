...
class PROrchestrator Dashboard:
    ...
    def get_orchestrator_metrics(self) -> Dict:
        ...
        if metrics["total_prs_analyzed"] > 0:
            metrics["success_rate"] = (metrics["auto_merged"] / metrics["total_prs_analyzed"]) * 100
    ...
    def create_metrics_panel(self, metrics: Dict) -> Panel:
        ...
        table.add_row("Auto-Merged", f"{metrics['auto_merged']} ({metrics['success_rate']:.1f}%)")
    ...
    def create_activity_log(self) -> Panel:
        ...
        text = "\n".join(activities[:10]) if activities else "No recent activity"
    ...
    def create_pr_type_distribution(self, prs: List[Dict]) -> Panel:
        ...
        for pr_type, count in sorted(type_counts.items(), key=lambda x: x[1], reverse=True):
            ...
            table.add_row(pr_type.capitalize(), str(count), f"{bar} {percentage:.1f}%")
    ...
    def create_dashboard_layout(self) -> Layout:
        ...
        # Header
        header_text = Text("🤖 PR Orchestrator Dashboard", style="bold white on blue")
        header_text.append(f"\n{self.repo_name}", style="dim")
        header_text.append(f" | Last Update: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", style="dim")
        layout["header"].update(Panel(header_text, style="blue"))
        
        # Components
        layout["metrics"].update(self.create_metrics_panel(self.cache["metrics"]))
        layout["distribution"].update(self.create_pr_type_distribution(self.cache["prs"]))
