...
async def handle_openhands_notification(pr_number: int, notification_type: str = "ci_cd_failure"):
    ...
    try:
        ...
        if result and result.status == TaskStatus.COMPLETED:
            ...
            print(f"   Critical Failures: {output.get('critical_failures', 0)}")
            print(f"   Actions Taken: {', '.join(output.get('actions_taken', []))}")
