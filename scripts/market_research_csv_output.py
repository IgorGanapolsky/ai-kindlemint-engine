...
class MarketResearchEngine:
    ...
    def save_to_csv(self, data_points: List[MarketDataPoint], filename: str = "market_analysis.csv"):
        """Save data points to CSV with proper schema"""
        csv_path = self.output_dir / filename
        
        # Define CSV schema
        fieldnames = ['date', 'keyword', 'amazon_rank', 'avg_price', 'est_sales', 
                      'competition_level', 'reviews_avg']
    ...
    def run_research(self, keywords: Optional[List[str]] = None) -> Path:
        ...
        add_breadcrumb("Market research started", category="research", 
    ...
    def generate_summary(self, data_points: List[MarketDataPoint]):
        ...
        for keyword, points in keyword_groups.items():
            ...
            avg_competition = sum(1 if p.competition_level == "low" else 2 if p.competition_level == "medium" else 3 
        ...
        summary = {
            ...
            "market_insights": {
                "avg_book_price": round(sum(p.avg_price for p in data_points) / len(data_points), 2),
                "high_competition_keywords": sum(1 for p in data_points if p.competition_level == "high"),
                "low_competition_keywords": sum(1 for p in data_points if p.competition_level == "low")
            }
        }
