...
class SlackBot:
    ...
    def send_alert(self, 
        ...
        """
        # Create alert blocks
        blocks = self._create_alert_blocks(alert_data, alert_type, severity, include_actions)
    ...
    def _create_action_buttons(self, 
        ...
        actions = [
            {
                ...
                "value": json.dumps({"action": "investigate", "alert_data": alert_data})
            },
            {
                "type": "button", 
                "text": {"type": "plain_text", "text": "✅ Acknowledge"},
                "action_id": "acknowledge_alert",
                "value": json.dumps({"action": "acknowledge", "alert_data": alert_data})
            }
        ]
        ...
        if severity in ['critical', 'high']:
            actions.append({
                ...
                "value": json.dumps({"action": "escalate", "alert_data": alert_data})
            })
        ...
        if alert_type == "error":
            actions.append({
                ...
                "value": json.dumps({"action": "autofix", "alert_data": alert_data})
            })
            ...
            actions.append({
                ...
                "value": json.dumps({"action": "optimize", "alert_data": alert_data})
            })
        ...
        blocks = [
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": f"📞 *Alert escalated by {user}*"
                }
            },
            {
                "type": "section",
...
class SlackWebhookHandler:
    ...
    async def _process_command(self, command: str, text: str, user: str, channel: str) -> Dict:
        """Process slash commands"""
        if command == '/alert-status':
            # Return status of active alerts
            active_count = len([a for a in self.slack_bot.active_alerts.values() if a.status == 'new'])
            return {
