...
class SentryAIOrchestrator:
    ...
    async def check_pr_status(self, pr_number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for comment in comments:
                if comment.user.type == 'Bot' and 'sentry' in comment.user.login.lower():
            ...
            return {
                ...
                'latest_sentry_comment': sentry_comments[0] if sentry_comments else None,
                'labels': labels
            }
    ...
    async def trigger_sentry_ai(self, pr_number: int, command: str = 'both') -> Dict[str, Any]:
        ...
        try:
            ...
            for comment in comments:
                if comment.user.type == 'Bot' and 'sentry' in comment.user.login.lower():
                    ...
                    if 'issue' in body_lower or 'error' in body_lower or 'bug' in body_lower:
                        analysis['issues_found'] += body_lower.count('issue') + body_lower.count('error') + body_lower.count('bug')
                    ...
                    if 'critical' in body_lower or 'security' in body_lower or 'vulnerability' in body_lower:
