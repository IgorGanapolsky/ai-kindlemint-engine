...
try:
    ...
    from .sentry_ai_orchestrator import SentryAIAgent, SentryAIOrchestrator
...
def __init__(self, config_file: Optional[str] = None):
    ...
    def _load_config(self, config_file: Optional[str]) -> OrchestrationConfig:
        ...
        env_config = {
            ...
            "auto_resolution_enabled": os.getenv(
...
def start_orchestration(self):
        ...
        try:
            ...
            await asyncio.gather(
                monitoring_task, resolution_task, metrics_task, return_exceptions=True
            )
        ...
        while not self.shutdown_requested:
            try:
                ...
                errors = self.sentry_monitor.fetch_errors(
                    since=last_fetch_time, limit=100
                )
            ...
            try:
                ...
                if self.sentry_ai_orchestrator:
                    ...
                    for pr_status in pr_statuses:
                        ...
                        if (
                            not pr_status.get("has_sentry_review")
                            and not pr_status.get("is_processing")
                            and "sentry-ai-skip" not in pr_status.get("labels", [])
                        ):

                            logger.info(f"PR #{pr_number} needs Sentry AI review")

                            # Process through Sentry AI agent
                            task = {"type": "check_pr", "pr_number": pr_number}
...
def _process_single_error(self, error: SentryError):
    ...
    def _should_send_alert(
        ...
        if error.environment.lower() in ["production", "prod"] and error.level in [
            "error",
            "fatal",
        ]:
            return True
    ...
    def _should_attempt_resolution(
        self, error: SentryError, classification: ErrorClassification
    ) -> bool:
        """Determine if auto-resolution should be attempted"""
        ...
        if (
            ...
            in ["performance", "network", "authentication"]
            and classification.confidence_score >= 0.8
        ):
            return True

        # Auto-resolve critical errors with very high confidence
        if (
            classification.resolution_urgency == "critical"
            and classification.confidence_score >= 0.9
        ):
            return True
    ...
    def _should_escalate(
        ...
        if (
            len(recent_errors)
            > self.config.escalation_thresholds["critical_errors_per_hour"]
        ):
            return True
        ...
        if (
            failed_resolutions
            > self.config.escalation_thresholds["failed_resolutions_per_hour"]
        ):
            return True
...
def _attempt_resolution(self, resolution_item: Dict):
        ...
        try:
            ...
            # Attempt resolution
            result = await self.auto_resolver.resolve_error(
                error_data, asdict(classification)
            )
        ...
        try:
            ...
            if success:
                notification_data = {
                    ...
                    "resolution_details": (
                        result.output if result else "No details available"
                    ),
                    "execution_time": (
                        f"{result.execution_time:.2f}s" if result else "Unknown"
                    ),
                }
                severity = "low"
            else:
...
def _report_metrics(self):
        ...
        try:
            ...
            if self.metrics.auto_resolutions_attempted > 0:
                success_rate = (
                    self.metrics.auto_resolutions_successful
                    / self.metrics.auto_resolutions_attempted
                ) * 100
            ...
            logger.info(
                f"Orchestration metrics: {json.dumps(metrics_report, indent=2)}"
            )
