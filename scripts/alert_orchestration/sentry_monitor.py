...
class SentryError:
    ...
    def is_critical(self) -> bool:
        ...
        return (
            ...
            self.count > 10 or
            any(count > 5 for _, count in self.frequency[-24:])  # High frequency in last 24 hours
        )
    ...
    def category(self) -> str:
        ...
        # Performance issues
        if any(keyword in message_lower for keyword in ['timeout', 'slow', 'memory', 'cpu']):
            return 'performance'
        
        # Application errors
        if any(keyword in message_lower for keyword in ['import', 'module', 'syntax', 'attribute']):
            return 'application'
        
        # Infrastructure issues
        if any(keyword in message_lower for keyword in ['connection', 'network', 'service', 'rate limit']):
            return 'infrastructure'
        
        # Security issues
        if any(keyword in message_lower for keyword in ['unauthorized', 'permission', 'authentication']):
            return 'security'
        
        # Database issues
        if any(keyword in message_lower for keyword in ['database', 'query', 'transaction', 'deadlock']):
            return 'database'
...
class SentryMonitor:
    ...
    def __init__(self, auth_token: Optional[str] = None, organization: Optional[str] = None):
    ...
    def fetch_errors(self, 
                    ...
                    limit: int = 100,
                    query: Optional[str] = None) -> List[SentryError]:
        """
        ...
        for project in projects:
            try:
                ...
                issues_data = self._make_request(
                    f"/projects/{self.organization}/{project}/issues/",
                    params=params
                )
    ...
    def _create_sentry_error(self, issue: Dict, events: List[Dict]) -> Optional[SentryError]:
        ...
        try:
            ...
            return SentryError(
                ...
                message=issue.get('metadata', {}).get('value', '') or issue.get('culprit', ''),
    ...
    def analyze_error_patterns(self, errors: List[SentryError]) -> Dict[str, Any]:
        ...
        new_errors = [
            ...
            if error.id not in self.error_cache or 
            error.timestamp > (self.last_fetch_time or datetime.min.replace(tzinfo=timezone.utc))
        ]
    ...
    def _find_error_correlations(self, errors: List[SentryError]) -> List[Dict[str, Any]]:
            ...
            if len(window_errors) > 1:
                ...
                if len(unique_types) > 1:
                    correlations.append({
                        ...
                        'potential_root_cause': self._identify_root_cause(window_errors)
                    })
        ...
        while True:
            try:
                ...
                errors = self.fetch_errors(
                    since=datetime.now(timezone.utc) - timedelta(seconds=interval_seconds * 2)
                )
