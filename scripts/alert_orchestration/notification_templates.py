...
def error_alert_blocks(data: Dict[str, Any]) -> List[Dict]:
    ...
    if data.get("suggested_actions"):
        actions_text = "\n".join([f"• {action}" for action in data["suggested_actions"][:3]])
        blocks.append({
            "type": "section",
            "text": {
                "type": "mrkdwn",
    ...
    return [
        {
            ...
            "text": {
                "type": "plain_text",
                "text": "✅ Auto-Resolution Successful"
            }
        },
        {
            "type": "section",
...
def escalation_blocks(data: Dict[str, Any]) -> List[Dict]:
    ...
    if data.get("suggested_actions"):
        suggestions_text = "\n".join([f"• {action}" for action in data["suggested_actions"]])
        blocks.append({
            "type": "section",
            "text": {
                "type": "mrkdwn",
        ...
        "elements": [
            {
                ...
                "value": json.dumps({"error_id": data.get("error_id"), "escalation_level": escalation_level})
            },
            {
                "type": "button",
...
class TemplateRegistry:
    ...
    def _register_default_templates(self):
        ...
        self.register_template(SlackBlockTemplate("resolution_success", resolution_success_blocks))
        self.register_template(SlackBlockTemplate("resolution_failure", resolution_failure_blocks))
        self.register_template(SlackBlockTemplate("escalation", escalation_blocks))
        self.register_template(SlackBlockTemplate("metrics_report", metrics_report_blocks))
        self.register_template(SlackBlockTemplate("system_health", system_health_blocks))
        
        # Simple text templates
        self.register_template(SlackTextTemplate(
            "simple_error",
    ...
    def render_template(self, name: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
...
def render_notification(template_name: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
...
def build_resolution_data(error, result, strategy_name) -> Dict[str, Any]:
    ...
    return {
        ...
        "failure_reason": result.error_message if result and not result.success else None,
        "actions_attempted": result.actions_taken if result and not result.success else [],
        "fallback_text": f"Resolution {'successful' if result and result.success else 'failed'}: {error.title}"
    }

def build_escalation_data(error, classification, escalation_level, reason, context=None) -> Dict[str, Any]:
    ...
    return {
        ...
        "business_impact": classification.business_impact if classification else "unknown",
        "suggested_actions": classification.suggested_actions if classification else [],
