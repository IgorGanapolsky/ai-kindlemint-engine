...
class BranchCleanupAgent:
    ...
    async def get_branches(self) -> Dict[str, List[str]]:
        ...
        result = subprocess.run(
            ["git", "branch"], capture_output=True, text=True, cwd=Path.cwd()
        )
        local_branches = [
            b.strip().replace("* ", "")
        ...
        # Get remote branches
        result = subprocess.run(
            ["git", "branch", "-r"], capture_output=True, text=True, cwd=Path.cwd()
        )
        remote_branches = [
    ...
    def should_delete_branch(self, branch_name: str) -> bool:
        ...
        return any(
            re.match(pattern, branch_name) for pattern in self.auto_delete_patterns
        )
    ...
    async def delete_local_branch(self, branch_name: str) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"❌ Error deleting local branch {branch_name}: {e}")
            return False
    ...
    async def delete_remote_branch(self, branch_name: str) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"❌ Error deleting remote branch {branch_name}: {e}")
            return False
    ...
    async def prune_remote_tracking(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.error(
                    f"❌ Failed to prune remote tracking: {result.stderr}"
                )
                return False
        except Exception as e:
    ...
    async def run_full_cleanup(self) -> Dict[str, any]:
        ...
        initial_count = len(
            initial_branches["local"]) + len(initial_branches["remote"])

        print(
            f"📊 Initial state: {len(initial_branches['local'])} local, {len(initial_branches['remote'])} remote branches"
        )
        ...
        final_count = len(final_branches["local"]) + \
            len(final_branches["remote"])
        ...
        print(
            f"✅ Cleanup complete! Deleted {results['total_deleted']} branches")
        print(
            f"📊 Final state: {results['final_local_count']} local, {results['final_remote_count']} remote branches"
        )
...
async def main():
    ...
    try:
        ...
        print("\n📋 Cleanup Summary:")
        print(
            f"   • Merged branches deleted: {results.get('merged_branches_deleted', 0)}"
        )
        print(
            f"   • Local branches deleted: {results.get('local_deleted', 0)}")
        print(
            f"   • Remote branches deleted: {results.get('remote_deleted', 0)}")
        print(
            f"   • Total branches deleted: {results.get('total_deleted', 0)}")
        print(
            f"   • Remaining: {results.get('final_local_count', 0)} local, {results.get('final_remote_count', 0)} remote"
        )
