...
class SentryKDPPublisher:
    ...
    def __init__(self):
        ...
        if self.sentry_enabled:
            ...
            sentry_sdk.set_context("kdp_automation_context", {
    ...
    def simulate_kdp_upload_process(self, book_metadata: dict):
        ...
        with SentryKDPOperation("kdp_book_upload_full_cycle", book_metadata) as transaction:
            
            try:
                # PHASE 1: Initialize Browser (Most failure-prone)
                with SentryKDPOperation("browser_initialization") as browser_tx:
                    add_breadcrumb("Starting browser initialization", 
                                 category="browser_automation",
                                 data={"browser": "playwright", "headless": True})
                    
                    # Simulate common browser failures
                    if os.getenv('SIMULATE_BROWSER_FAILURE') == 'true':
                        raise Exception("Browser failed to start - common Fargate issue")
                    
                    add_breadcrumb("Browser initialized successfully", category="browser_automation")
                ...
                with SentryKDPOperation("kdp_authentication", {"email": "masked"}) as auth_tx:
                    add_breadcrumb("Attempting KDP login", 
                    ...
                    if not kdp_email or not kdp_password:
                        raise ValueError("KDP credentials not found - critical revenue blocker")
                    
                    # Simulate authentication challenges
                    if os.getenv('SIMULATE_AUTH_FAILURE') == 'true':
                        raise Exception("KDP authentication failed - possible CAPTCHA or rate limit")
                    
                    add_breadcrumb("KDP authentication successful", category="authentication")
                ...
                with SentryKDPOperation("book_file_upload", book_metadata) as upload_tx:
                    add_breadcrumb("Starting book file upload", 
                    ...
                    if os.getenv('SIMULATE_UPLOAD_FAILURE') == 'true':
                        raise Exception("File upload timeout - network or file size issue")
                    
                    # Simulate successful upload with realistic timing
                    time.sleep(2)  # Simulate upload time
                    
                    add_breadcrumb("Book file uploaded successfully", 
                                 category="file_upload",
                ...
                with SentryKDPOperation("metadata_entry", book_metadata) as metadata_tx:
                    add_breadcrumb("Entering book metadata", 
                    ...
                    if os.getenv('SIMULATE_METADATA_FAILURE') == 'true':
                        raise Exception("Form validation failed - Amazon changed form structure")
                ...
                with SentryKDPOperation("book_publishing", book_metadata) as publish_tx:
                    add_breadcrumb("Publishing book to KDP", 
                                 category="publishing",
                                 data={
                                     "title": book_metadata.get('title', 'Unknown')[:30],
                    ...
                    add_breadcrumb("Book published successfully", 
                                 category="publishing",
                                 data={
                                     "asin": book_asin,
                                     "status": "live",
                    ...
                    print(f"✅ Published: {book_metadata.get('title')} - ASIN: {book_asin}")
                ...
                add_breadcrumb("KDP upload process failed", 
                             level="error",
...
def run_sentry_kdp_automation():
    ...
    with SentryKDPOperation("daily_book_publishing_cycle") as daily_cycle:
        ...
        add_breadcrumb("Starting daily publishing cycle", 
        ...
        print(f"✅ Sentry Monitoring: {'Active' if publisher.sentry_enabled else 'Disabled'}")
        print(f"📚 Book Processing: {result.get('title', 'Unknown')}")
        print(f"🎯 Automation Result: {'SUCCESS' if result.get('success') else 'FAILED'}")
        print(f"💰 Revenue Status: {result.get('revenue_status', result.get('revenue_impact', 'Unknown'))}")
        ...
        if result.get('success'):
            add_breadcrumb("Daily publishing cycle completed successfully", 
                         category="business_automation",
                         data={"revenue_generating": True})
        else:
            add_breadcrumb("Daily publishing cycle failed - revenue impact", 
                         level="error",
                         category="business_automation", 
