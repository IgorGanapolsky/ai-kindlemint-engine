...
def validate_source_images():
    ...
    puzzles_dir = Path("books/active_production/Large_Print_Sudoku_Masters/volume_1/puzzles/puzzles")
    ...
    for i in [1, 27, 50, 100]:  # Test sample images
        ...
        if puzzle_img.exists() and solution_img.exists():
            ...
            if puzzle_size >= solution_size:
                errors.append(f"Puzzle {i} size ({puzzle_size}) >= solution size ({solution_size}) - likely wrong content")
...
def main():
    ...
    paperback_dir = Path("books/active_production/Large_Print_Sudoku_Masters/volume_1/paperback")
