    ...
    def validate(self) -> Dict:
        ...
        if pdf_files:
            for pdf_file in pdf_files:
                if "interior" in pdf_file.name.lower() or "manuscript" in pdf_file.name.lower():
        ...
        for item, item_type in required_items:
            if item_type == "directory":
                if not (self.book_dir / item).is_dir():
                    self.report["critical_issues"].append(f"Missing required directory: {item}")
                else:
                    self.report["passed_checks"].append(f"✅ Found {item} directory")
            elif item_type == "file":
                if not (self.book_dir / item).exists():
                    self.report["critical_issues"].append(f"Missing required file: {item}")
                else:
    ...
    def _validate_metadata(self):
        ...
        for puzzle_id in puzzle_ids:
            ...
            if not puzzle_file.exists():
                self.report["critical_issues"].append(f"Missing metadata for puzzle {puzzle_id}")
                continue
        ...
        self.report["metadata_validation"]["puzzles_with_solutions"] = puzzles_with_solutions
        
        if valid_puzzles == len(puzzle_ids):
            self.report["passed_checks"].append(f"✅ All {valid_puzzles} puzzles have valid metadata")
        else:
            self.report["critical_issues"].append(
        ...
        try:
            ...
            with open(pdf_path, 'rb') as file:
                ...
                if page_count == 156:
                    ...
                    self.report["critical_issues"].append(f"Wrong page count: {page_count} (expected 156)")
                ...
                if "test" in sample_text.lower() or "lorem ipsum" in sample_text.lower():
                    self.report["critical_issues"].append("Test or placeholder content found")
            ...
            if file_size_mb > 400:
                self.report["critical_issues"].append(f"File too large: {file_size_mb:.2f} MB")
            elif file_size_mb < 0.1:
                self.report["critical_issues"].append(f"File too small: {file_size_mb:.2f} MB")
            else:
                self.report["passed_checks"].append(f"✅ File size OK: {file_size_mb:.2f} MB")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='Comprehensive QA Validator for KDP Books')
    parser.add_argument('book_dir', help='Path to book directory')
    parser.add_argument('--strict', action='store_true', help='Use strict validation (fail on warnings)')
