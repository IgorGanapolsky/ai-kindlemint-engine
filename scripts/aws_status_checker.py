...
def generate_status_summary() -> Dict[str, Any]:
    ...
    for stack_name in cf_stacks:
        status['services'][f'cloudformation_{stack_name}'] = check_cloudformation_stack(stack_name, region)
    ...
    healthy_services = sum(1 for s in all_services.values() if s.get('status') == 'available')
    ...
    status['overall'] = {
        ...
        'status': 'healthy' if healthy_services == total_services else 'degraded' if healthy_services > 0 else 'down'
    }
...
def main():
    ...
    try:
        ...
        print(f"🔧 Services: {overall.get('healthy_services', 0)}/{overall.get('total_services', 0)}")
        
        # Print service details
        for service_name, service_status in status.get('services', {}).items():
            status_emoji = '✅' if service_status.get('status') == 'available' else '❌'
            print(f"  {status_emoji} {service_name}: {service_status.get('status', 'unknown')}")
