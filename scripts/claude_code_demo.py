...
async def main():
    ...
    kdp_integration = await orchestrator.integrate_service(
        service_name="KDP Publishing API",
    ...
    print(f"✅ Generated {tests['total_tests']} tests with {tests['estimated_coverage']*100:.0f}% coverage")
    ...
    print(f"✅ Performance optimizations: {len(perf_optimization['performance']['optimizations'])} improvements found")
    
    # Run security audit
    security_optimization = await orchestrator.optimize_codebase("security")
    print(f"✅ Security audit: {len(security_optimization['security']['optimizations'])} issues addressed")
