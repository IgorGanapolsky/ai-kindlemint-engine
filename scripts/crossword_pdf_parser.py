...
class CrosswordPDFParser:
            ...
            for puzzle_num in sorted(puzzle_pages.keys()):
                if puzzle_num <= 50:  # We expect 50 puzzles
                    puzzle = ExtractedPuzzle(
                        number=puzzle_num, grid_text="", clues_text="", solution_text=""
                    )
                    ...
                    if puzzle_num in solution_pages:
                        ...
                        solution = self._extract_grid_visual(
                            pdf_path, page_num, is_solution=True
                        )
                        if solution:
    ...
    def _extract_grid_visual(
        self, pdf_path: str, page_num: int, is_solution: bool = False
    ...
    def validate_extracted_puzzle(self, puzzle: ExtractedPuzzle) -> Dict[str, bool]:
        ...
        if puzzle.grid_array:
            validation["grid_size_correct"] = len(
                puzzle.grid_array
            ) == self.grid_size and all(
                len(row) == self.grid_size for row in puzzle.grid_array
            )
