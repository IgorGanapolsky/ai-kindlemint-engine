...
class ClaudeCostSlackNotifier:
    ...
    def send_efficiency_report(self) -> bool:
        ...
        # Calculate efficiency metrics
        tokens_per_dollar = summary_7d["total_tokens"] / max(summary_7d["total_cost"], 0.01)
        commits_per_dollar = summary_7d["commit_count"] / max(summary_7d["total_cost"], 0.01)
        ...
        optimizations = self._generate_optimization_recommendations(summary_7d, summary_30d)
        if optimizations:
            blocks.append({
                "type": "section",
                "text": {
    ...
    def _calculate_daily_costs(self, days: int) -> List[Tuple[str, float]]:
        ...
        return [(day, daily_costs.get(day, 0)) for day in days_order if day in daily_costs]
    ...
    def _generate_daily_recommendations(self, summary: Dict) -> List[str]:
        ...
        if summary["total_cost"] > 5.0:
            recommendations.append("• Consider batching similar changes to reduce API calls")
        ...
        if summary["commit_count"] > 20:
            recommendations.append("• High commit volume - consider grouping related changes")
        
        return recommendations
    
    def _generate_weekly_insights(self, current_week: Dict, previous_week: Dict) -> List[str]:
        """Generate insights comparing weeks"""
        insights = []
        
        # Token efficiency
        current_tpd = current_week["total_tokens"] / max(current_week["total_cost"], 0.01)
        insights.append(f"• Token efficiency: {current_tpd:,.0f} tokens per dollar")
        
        # Cost trends
        if current_week["total_cost"] > 10:
            insights.append("• 💰 Consider setting daily budget limits to control costs")
        
        # Commit patterns
        if current_week["commit_count"] > 50:
            insights.append("• 📈 High commit frequency - explore batch processing options")
        
        # Model recommendations
        if current_week["average_cost_per_commit"] > 0.30:
            insights.append("• 🤖 Switch to Claude Haiku for routine tasks (80% cost reduction)")
    ...
    def _analyze_expensive_patterns(self, commits: List[Dict]) -> List[str]:
        ...
        for commit in commits:
            if commit["cost"] > 0.10:  # Focus on expensive commits
                ...
                if "test" in msg:
                    file_costs["test files"] = file_costs.get("test files", 0) + commit["cost"]
                elif "doc" in msg:
                    file_costs["documentation"] = file_costs.get("documentation", 0) + commit["cost"]
                elif "config" in msg:
                    file_costs["configuration"] = file_costs.get("configuration", 0) + commit["cost"]
    ...
    def _analyze_time_patterns(self, commits: List[Dict]) -> List[str]:
        ...
        if hour_costs:
            ...
            patterns.append(f"• Peak usage hour: {peak_hour[0]:02d}:00 (${peak_hour[1]:.2f})")
    ...
    def _generate_optimization_recommendations(self, week_data: Dict, month_data: Dict) -> List[str]:
        ...
        if avg_cost > 0.20:
            savings = (avg_cost - 0.05) * week_data["commit_count"] * 4  # Monthly savings
            recommendations.append(f"• Switch to Haiku for routine tasks: Save ~${savings:.2f}/month")
        
        # Batching optimization
        if week_data["commit_count"] > 30:
            recommendations.append("• Implement change batching: Reduce costs by 30-40%")
        ...
        if week_data["total_tokens"] > 500000:
            recommendations.append("• Use .claudeignore to exclude large generated files")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Send Claude cost notifications to Slack")
    ...
    parser.add_argument("--budget-limit", type=float, help="Budget limit for budget alerts")
    parser.add_argument("--budget-period", choices=["daily", "weekly", "monthly"], 
