...
from reportlab.platypus import SimpleDocTemplate, PageBreak, Spacer, Table, TableStyle, Paragraph, Image, PageTemplate, Frame
...
class EnhancedSudokuPDFLayout:
    ...
    def __init__(self, input_dir, output_dir, title, author, subtitle=None, 
                 page_size="letter", include_solutions=True, isbn=None):
    ...
    def create_title_page(self, story):
        ...
        # Volume indicator
        story.append(Paragraph(f"Volume {self.volume_number}", self.styles['BookSubtitle']))
    ...
    def create_instructions_page(self, story):
        ...
        instructions = [
            ...
            ("beginners learning the basics." if self.puzzles[0]['difficulty'] == 'easy' else "improving your skills."),
            "",
    ...
    def create_puzzle_page(self, story, puzzle_data, puzzle_number):
        ...
        story.append(Paragraph(f"Difficulty: {puzzle_data['difficulty'].title()}", self.styles['Difficulty']))
        ...
        if image_path.exists():
            ...
            story.append(Paragraph(f"[Puzzle image not found: {image_path}]", self.styles['Normal']))
    ...
    def create_solutions_section(self, story):
        ...
        for i, puzzle_data in enumerate(self.puzzles):
            solution_path = self.input_dir / "puzzles" / f"sudoku_solution_{puzzle_data['id']:03d}.png"
            if not solution_path.exists():
                solution_path = self.input_dir.parent / "puzzles" / f"sudoku_solution_{puzzle_data['id']:03d}.png"
    ...
    def add_solutions_page(self, story, puzzle_solutions):
        ...
        if len(puzzle_solutions) <= 2:
            ...
            table_data = [
                [puzzle_solutions[0], puzzle_solutions[1] if len(puzzle_solutions) > 1 else []],
    ...
    def generate_pdf(self):
        ...
        print(f"📄 Total pages: {len(self.puzzles) + 4}")  # puzzles + front matter + solutions
        return output_file


def main():
    """Main entry point for enhanced Sudoku PDF layout generator."""
    parser = argparse.ArgumentParser(description='Generate professional PDF layout for Sudoku puzzle books')
    parser.add_argument('--input', required=True, help='Input directory with puzzle images')
    ...
    parser.add_argument('--page-size', choices=['letter', 'a4'], default='letter', help='Page size')
    parser.add_argument('--include-solutions', action='store_true', help='Include solutions section')
