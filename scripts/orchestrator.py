...
class KindleMintOrchestrator:
    ...
    def generate_book(self, spec: BookSpec, max_retries: int = 3) -> GenerationResult:
        ...
        for attempt in range(max_retries):
            ...
            logger.warning(f"QA score {qa_result['score']} below threshold {self.qa_threshold}")
    ...
    def _generate_puzzles(self, spec: BookSpec) -> GenerationResult:
        ...
        if not worker_info:
            return GenerationResult(
                success=False, 
                issues=[f"No worker available for {spec.puzzle_type}"]
            )
        ...
        try:
            ...
            return GenerationResult(
                success=False,
                issues=[f"Worker error: {e.stderr}"]
            )
...
def main():
    ...
    parser.add_argument("--type", choices=["crossword", "sudoku", "word_search"], required=True)
