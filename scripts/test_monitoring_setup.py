...
def test_slack_notification():
    ...
    try:
        ...
        if not webhook_url:
            ...
            print("   Please add: SLACK_WEBHOOK_URL=https://hooks.slack.com/services/YOUR/WEBHOOK/URL")
            return False
        ...
        blocks = [
            {
                ...
                "text": {
                    "type": "plain_text",
                    "text": "🎉 KindleMint Monitoring Test"
                }
            },
            {
                "type": "section",
...
def test_sentry_monitoring():
    ...
    try:
        import sentry_sdk
        from scripts.sentry_config import init_sentry, capture_kdp_error, add_breadcrumb
...
def main():
    ...
    for service, status in results.items():
        icon = "✅" if status else "❌"
        print(f"{icon} {service.capitalize()}: {'Working' if status else 'Not configured'}")
