...
"""
    ...
    for py_file in scripts_dir.glob("*.py"):
        ...
        try:
            ...
            has_font_issues = any(
                font in content for font in font_replacements.keys())
            ...
            if "pdfmetrics.registerFont" not in content:
                ...
                if import_match:
                    ...
                    content = (
                        content[:insert_pos] +
                        font_registration + content[insert_pos:]
                    )
            ...
            for old_font, new_font in font_replacements.items():
                ...
                content = re.sub(
                    rf'["\']?{re.escape(old_font)}["\']?', f'"{new_font}"', content
                )
...
def fix_metadata_compliance():
    ...
    backup_file = Path(
        "books/active_production/_backup/books/active_production/Large_Print_Sudoku_Masters/volume_2/hardcover/amazon_kdp_metadata.json"
    )
