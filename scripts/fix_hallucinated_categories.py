...
def fix_markdown_files():
    ...
    for root, dirs, files in os.walk(base_path):
        for file in files:
            if file.endswith('.md') and ('template' in file.lower() or 'checklist' in file.lower()):
            ...
            for hallucinated in HALLUCINATED_CATEGORIES:
                if hallucinated in content:
                    ...
                    if 'sudoku' in str(md_file).lower():
                        content = content.replace(hallucinated, "Crafts, Hobbies & Home")
                    elif 'crossword' in str(md_file).lower():
                        content = content.replace(hallucinated, "Crafts, Hobbies & Home")
            
            # Fix category count mentions
            content = content.replace("Choose 2)", "Choose 3 - KDP ALLOWS 3 CATEGORIES!)")
            content = content.replace("(Choose 2", "(Choose 3 - KDP ALLOWS 3 CATEGORIES!")
