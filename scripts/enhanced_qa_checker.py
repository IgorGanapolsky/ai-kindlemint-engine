...
class EnhancedQAChecker:
    ...
    def check_visual_layout(self, pdf_path):
        ...
        try:
            ...
            for page_num in range(len(doc)):
                            ...
                            for line in block["lines"]:
                                if "spans" in line:
                                    for span in line["spans"]:
                                        ...
                                        if (bbox[0] < margin_threshold or  # Left edge
                                            bbox[1] < margin_threshold or  # Top edge  
                                            bbox[2] > page_rect.width - margin_threshold or  # Right edge
                                            bbox[3] > page_rect.height - margin_threshold):  # Bottom edge
                ...
                if edge_text_issues:
                    ...
                    for issue in edge_text_issues:
                        self.add_critical_issue("TEXT_NEAR_EDGE", 
                ...
                if black_ratio > 0.5:
                    self.add_critical_issue("EXCESSIVE_BLACK", 
                                          f"Page {page_num + 1}: {black_ratio*100:.1f}% black - may have rendering issues")
                
                if white_ratio > 0.9:
                    self.add_warning("MOSTLY_WHITE", 
            ...
            self.add_critical_issue("VISUAL_ANALYSIS_FAILED", f"Could not perform visual analysis: {e}")
    ...
    def check_crossword_grid_numbers(self, page, page_num, checks):
        ...
        if number_issues:
            ...
            for issue in number_issues:
                self.add_critical_issue("CROSSWORD_NUMBER_ISSUE", 
    ...
    def check_pdf_structure(self, pdf_path):
        ...
        try:
            with open(pdf_path, 'rb') as file:
                ...
                if num_pages < 5:
                    self.add_warning("LOW_PAGE_COUNT", f"Only {num_pages} pages - may seem short")
                else:
        ...
        try:
            with open(pdf_path, 'rb') as file:
                ...
                if len(all_text) < 1000:
                    self.add_critical_issue("INSUFFICIENT_CONTENT", f"Only {len(all_text)} characters - too short")
                else:
    ...
    def check_amazon_kdp_compliance(self, pdf_path):
        ...
        if file_size_mb <= 650:
            ...
            self.add_critical_issue("EXCEEDS_KDP_LIMIT", f"File {file_size_mb:.1f}MB exceeds KDP limit")
    ...
    def calculate_overall_score(self):
        ...
        for section in ["file_checks", "content_checks", "visual_checks", "amazon_kdp_checks"]:
    ...
    def generate_enhanced_report(self, pdf_path):
        ...
        self.qa_results["publish_ready"] = (critical_issues == 0 and self.qa_results["overall_score"] >= 85)
