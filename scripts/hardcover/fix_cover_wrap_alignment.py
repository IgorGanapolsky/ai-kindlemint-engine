...
class CoverWrapAlignmentFixer:
    ...
    def create_corrected_cover_wrap(self, cover_source_path, output_dir):
        ...
        front_scaled = cover_source.resize(
            (scaled_width, scaled_height), Image.Resampling.LANCZOS
        )

        # Center the front cover in the front cover area
        front_paste_x = (
            self.front_cover_x + (self.front_cover_width - scaled_width) // 2
        )
        ...
        spine_background = spine_sample.resize(
            (self.spine_width_px, self.cover_height), Image.Resampling.LANCZOS
        )
        ...
        back_background = back_sample.resize(
            (self.back_cover_width, self.cover_height), Image.Resampling.LANCZOS
        )

        # Apply darker overlay for text readability
        overlay = Image.new(
            "RGBA", (self.back_cover_width, self.cover_height), (0, 0, 0, 80)
        )
        back_with_overlay = Image.alpha_composite(
            back_background.convert("RGBA"), overlay
        ).convert("RGB")
    ...
    def add_text_overlays(self, canvas):
        ...
        barcode_x = (
            self.back_cover_x + self.back_cover_width - barcode_width - self.safe_margin
        )
...
def main():
    ...
    output_dir = (
        "books/active_production/Large_Print_Crossword_Masters/volume_1/hardcover"
    )
