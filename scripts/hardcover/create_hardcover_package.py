...
class HardcoverProducer:
    ...
    def generate_metadata(self, output_dir):
        ...
        metadata = {
            ...
            .replace("paperback", "hardcover")
            .replace('8.5\\" × 11\\"', '6\\" × 9\\"'),
            "keywords": [
                kw.replace("paperback", "hardcover") for kw in self.config["keywords"]
            ]
            + ["hardcover", "premium", "gift"],
...
"""
    ...
    def create_cover_wrap(self, output_dir):
        ...
        template_path = (
            self.template_dir
            / "kdp_case_laminate"
            / f"6x9_{self.config['pages']}pages_template.png"
        )
        if not template_path.exists():
            # Use the default template we have
            template_path = (
                self.template_dir / "kdp_case_laminate" / "6x9_103pages_template.png"
            )

        template = Image.open(template_path).convert("RGBA")
        template = template.resize(
            (self.template_width, self.template_height), Image.Resampling.LANCZOS
        )
        ...
        back_background = back_sample.resize(
            (back_width, back_height), Image.Resampling.LANCZOS
        )
        canvas.paste(back_background, (back_x, front_y))

        # Create spine background
        spine_sample = front_cover.crop(
            (
                front_cover.width // 2 - 50,
                0,
                front_cover.width // 2 + 50,
                front_cover.height,
            )
        )
        spine_background = spine_sample.resize(
            (spine_width, front_height), Image.Resampling.LANCZOS
        )
        canvas.paste(spine_background, (spine_x, front_y))

        # Add text overlays
        self._add_text_overlays(
            canvas, spine_x, spine_width, front_y, front_height, back_x, back_width
        )
    ...
    def _add_text_overlays(
        ...
        for line in lines:
            if line.strip():
                draw.text(
                    (back_x + 60, current_y), line.strip(), font=desc_font, fill="white"
                )
    ...
    def export_pdf_x1a(self, output_dir):
        ...
        try:
            ...
            c = pdf_canvas.Canvas(
                str(output_file), pagesize=(width_points, height_points)
            )
