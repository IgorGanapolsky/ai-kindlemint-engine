...
"""
    ...
    def create_cover_wrap_template(self, output_dir):
        ...
        # Create base image
        img = Image.new('CMYK', (self.template_width, self.template_height), (100, 100, 100, 0))
        ...
        # Back cover
        draw.rectangle([bleed, bleed, bleed + back_width, self.template_height - bleed], 
                      outline=(0, 0, 0, 100), width=3)
        draw.text((bleed + 50, self.template_height//2), "BACK COVER", fill=(0, 0, 0, 100))
        
        # Spine
        spine_x = bleed + back_width
        draw.rectangle([spine_x, bleed, spine_x + spine_width_px, self.template_height - bleed], 
                      outline=(0, 0, 0, 100), width=3)
        draw.text((spine_x + 10, self.template_height//2), "SPINE", fill=(0, 0, 0, 100))
        
        # Front cover
        front_x = spine_x + spine_width_px
        draw.rectangle([front_x, bleed, front_x + front_width, self.template_height - bleed], 
                      outline=(0, 0, 0, 100), width=3)
        draw.text((front_x + 50, self.template_height//2), "FRONT COVER", fill=(0, 0, 0, 100))
        
        # Wrap area
        wrap_x = front_x + front_width
        draw.rectangle([wrap_x, bleed, wrap_x + wrap_width - bleed, self.template_height - bleed], 
                      outline=(100, 0, 100, 0), width=2)
        draw.text((wrap_x + 20, self.template_height//2), "WRAP", fill=(100, 0, 100, 0))
    ...
    def create_actual_cover_wrap(self, output_dir, spine_width_px):
        ...
        # Create CMYK image
        img = Image.new('CMYK', (self.template_width, self.template_height), (80, 50, 30, 0))
        draw = ImageDraw.Draw(img)
        ...
        if cover_source.exists():
            ...
            source_img = source_img.resize((front_width, front_height), Image.Resampling.LANCZOS)
