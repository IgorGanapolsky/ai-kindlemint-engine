...
class PaperbackToHardcoverConverter:
    ...
    def __init__(self):
        ...
        print(f"   Source: 8.5×11 inches ({self.source_width:.0f}×{self.source_height:.0f} points)")
        print(f"   Target: 6×9 inches ({self.target_width:.0f}×{self.target_height:.0f} points)")
    ...
    def convert_pdf(self, input_path, output_path):
        ...
        try:
            ...
            for page_num in range(page_count):
                ...
                new_page = output_doc.new_page(width=self.target_width, height=self.target_height)
...
def main():
    parser = argparse.ArgumentParser(description='Convert paperback PDF to hardcover format')
