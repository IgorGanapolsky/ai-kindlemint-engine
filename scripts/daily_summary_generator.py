...
class DailySummaryGenerator:
    ...
    def _get_market_insights(self) -> Dict:
        ...
        if market_dir.exists():
            ...
            today_file = market_dir / f"reddit_insights_{datetime.now().strftime('%Y%m%d')}.json"
    ...
    def _calculate_revenue(self) -> Dict:
        ...
        revenue["breakdown"]["paperback"] = formats["paperback"] * daily_sales * revenue["assumptions"]["paperback_profit"]
        revenue["breakdown"]["hardcover"] = formats["hardcover"] * daily_sales * revenue["assumptions"]["hardcover_profit"]
        revenue["breakdown"]["kindle"] = formats["kindle"] * daily_sales * revenue["assumptions"]["kindle_profit"]
    ...
    def _check_automation(self) -> Dict:
        ...
        status["github_actions"] = "Configured" if (self.base_dir / ".github" / "workflows").exists() else "Not configured"
        
        # Check market research
        if (self.base_dir / "data" / "market-insights" / f"reddit_insights_{datetime.now().strftime('%Y%m%d')}.json").exists():
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if prod["total_books"] < 5:
            recommendations.append("📚 Generate more volumes to build a series (target: 5-10 books)")
            
        if prod["formats"]["hardcover"] < prod["formats"]["paperback"]:
            recommendations.append("📖 Create hardcover editions for existing paperbacks (higher profit margin)")
            
        # Quality recommendations
        if quality["qa_pass_rate"] < 80:
            recommendations.append("⚠️ Fix QA issues before publishing - current pass rate: {:.0f}%".format(quality["qa_pass_rate"]))
            
        # Market recommendations
        if market["trending_keywords"]:
            recommendations.append(f"🔥 Create content for trending keyword: {market['trending_keywords'][0]}")
        ...
        if prod["total_books"] > 0 and not recommendations:
            recommendations.append("✅ All systems operational - focus on marketing and sales")
...
"""
    ...
    def run(self):
        ...
        # Print key metrics
        print(f"\n📚 Total Books: {summary['production']['total_books']}")
        print(f"💰 Monthly Revenue Estimate: ${summary['revenue_estimate']['monthly_estimate']:,.2f}")
