...
class CrosswordContentValidator:
        ...
        with open(pdf_path, 'rb') as f:
            ...
            for page_num, page in enumerate(reader.pages):
                ...
                if current_puzzle:
                    ...
                    if "ACROSS" in text:
                        across_section = text.split("ACROSS")[1].split("DOWN")[0] if "DOWN" in text else text.split("ACROSS")[1]
    ...
    def _validate_uniqueness(self, puzzles: List[Dict]) -> Dict[str, Any]:
        ...
        for puzzle in puzzles:
            ...
            if sig_hash in puzzle_hashes:
                ...
                self.issues.append(f"Puzzle {puzzle['number']} is duplicate of puzzle {puzzle_hashes[sig_hash]}")
            else:
        ...
        if not passed:
            self.issues.append(f"Only {unique_count}/{len(puzzles)} puzzles are unique ({uniqueness_ratio*100:.1f}%)")
    ...
    def _validate_completeness(self, puzzles: List[Dict]) -> Dict[str, Any]:
        ...
        for puzzle in puzzles:
            ...
            if len(puzzle["across_clues"]) < 5:
                issues.append(f"Only {len(puzzle['across_clues'])} ACROSS clues (minimum 5)")
            
            if len(puzzle["down_clues"]) < 5:
                issues.append(f"Only {len(puzzle['down_clues'])} DOWN clues (minimum 5)")
    ...
    def _validate_content_quality(self, puzzles: List[Dict]) -> Dict[str, Any]:
        ...
        for puzzle in puzzles:
            ...
            all_clues = list(puzzle["across_clues"].values()) + list(puzzle["down_clues"].values())
            
            for clue in all_clues:
                # Check for generic/placeholder clues
                if any(phrase in clue.lower() for phrase in ["letter word", "puzzle", "crossword entry"]):
            ...
            if issues:
                ...
                self.warnings.extend([f"Puzzle {puzzle['number']}: {issue}" for issue in issues[:2]])
        
        passed = len(quality_issues) == 0
        
        return {
            "passed": passed,
            "quality_score": (len(puzzles) - len(quality_issues)) / len(puzzles) if puzzles else 0,
    ...
    def _validate_solutions(self, puzzles: List[Dict]) -> Dict[str, Any]:
        ...
        for puzzle in puzzles:
            ...
            if issues:
                ...
                self.issues.extend([f"Puzzle {puzzle['number']}: {issue}" for issue in issues[:2]])
        ...
        for puzzle in puzzles:
            ...
            if total > 0:
                ...
                if across_ratio > 0.7 or down_ratio > 0.7:
                    ...
                    self.warnings.append(f"Puzzle {puzzle['number']}: Imbalanced clues ({across_count} across, {down_count} down)")
...
def main():
    ...
    # Save results
    output_path = Path(pdf_path).parent / f"content_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
