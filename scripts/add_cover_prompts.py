...
def get_cover_prompt(title: str, series_name: str, book_type: str) -> str:
    ...
    base_style = "Professional book cover design, clean modern layout, high contrast typography"
    
    if "Sudoku" in title:
        theme = "sudoku puzzle grids in background, mathematical symbols, number patterns"
    ...
    for file_path in sorted(all_files):
        try:
            ...
            if 'cover_design' in data:
                print(f"✓ Already has cover design: {os.path.relpath(file_path, base_dir)}")
                continue
