...
class IntelligentQAChecker:
    ...
    def check_solution_validity(self, book_path: Path) -> Tuple[List[QAIssue], Dict]:
        ...
        pdf_files = list(book_path.glob("*interior*.pdf")) + list(book_path.glob("*manuscript*.pdf"))
    ...
    def _generate_recommendations(self, issues: List[QAIssue], metrics: Dict) -> List[str]:
