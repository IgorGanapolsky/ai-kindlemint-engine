...
class SentryErrorHandler:
    ...
    async def _fix_epub_generation(self, error_data: Dict) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                ["python", "-c",
                    "import ebooklib; print('✅ ebooklib available')"],
                capture_output=True,
                text=True,
                cwd=Path.cwd(),
            )
    ...
    async def _fix_qa_issues(self, error_data: Dict) -> Dict[str, Any]:
        ...
        try:
            subprocess.run(
                ["python", "-c", "print('🔍 Running QA diagnostics...')"], check=True
            )
    ...
    async def process_sentry_errors(
        self, mock_errors: List[Dict] = None
    ) -> Dict[str, Any]:
    ...
    async def run_automated_fix_cycle(self) -> Dict[str, Any]:
        ...
        print(
            f"   • Success rate: {(results['fixed_errors']/results['processed_errors']*100):.1f}%"
        )
        print()
        ...
        if results["remediation_results"]:
            ...
            for fix in results["remediation_results"]:
                ...
                print(
                    f"   {status_icon} {fix['error_type']}: {fix.get('fixes_applied', [])}"
                )
