...
def generate_back_cover_prompt(metadata: Dict[str, Any]) -> str:
    ...
    is_puzzle_book = any(
        word in title.lower() for word in ["crossword", "sudoku", "puzzle"]
    )
...
def add_back_cover_to_metadata(file_path: str) -> bool:
    ...
    try:
        ...
        if isinstance(format_info, str):
            ...
            is_physical = format_info.get("type", "").lower() in [
                "paperback",
                "hardcover",
            ]
        ...
        if not is_physical and binding:
            ...
            is_physical = any(
                term in binding_lower
                for term in ["paperback", "perfect", "hardcover", "case"]
            )
        ...
        if "dalle_prompt" not in data["cover_design"]:
            ...
            data["cover_design"][
                "dalle_prompt"
            ] = f"Professional book cover for '{data.get('title', 'Book')}' - NEEDS PROPER PROMPT"
