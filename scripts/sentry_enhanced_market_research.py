...
def run_sentry_enhanced_research():
    ...
    with SentryKDPOperation("market_research_full_cycle") as transaction:
        ...
        with SentryKDPOperation("amazon_competitor_analysis", {"api": "serpapi"}) as sub_transaction:
            try:
                ...
                add_breadcrumb("Making SerpApi request", category="api", data={"params": params})
                
                response = requests.get('https://serpapi.com/search', params=params, timeout=30)
                
                if response.status_code != 200:
                    raise requests.HTTPError(f"SerpApi returned {response.status_code}: {response.text}")
                
                data = response.json()
                
                if 'organic_results' not in data:
                    raise ValueError(f"No organic_results in SerpApi response: {list(data.keys())}")
                ...
                add_breadcrumb(f"SerpApi success: {product_count} products found", 
                             category="api", data={"product_count": product_count})
                ...
                if product_count > 0:
                    ...
                    add_breadcrumb("Top competitor analyzed", 
        ...
        with SentryKDPOperation("slack_notification", {"integration": "webhook"}) as sub_transaction:
            try:
                add_breadcrumb("Initializing Slack notification", category="notification")
                ...
                api_count = len([api for api in results['apis_tested'] if 'SUCCESS' in api])
                ...
                message = {
                    ...
                    'blocks': [
                        {
                            'type': 'header',
                            'text': {'type': 'plain_text', 'text': '🎯 Seer AI Market Intelligence'}
                        },
                        {
                            'type': 'section',
                            'fields': [
                                {'type': 'mrkdwn', 'text': f'*APIs Working:* {api_count}'},
                                {'type': 'mrkdwn', 'text': f'*Products Found:* {results.get("amazon_products", 0)}'},
                                {'type': 'mrkdwn', 'text': f'*Sentry Tracking:* ✅ Active'},
                        ...
                        },
                        {
                            'type': 'context',
                            'elements': [
                                {'type': 'mrkdwn', 'text': f'🤖 Zero-Touch Debugging Active - {datetime.now().strftime("%Y-%m-%d %H:%M")}'}
                            ]
                        }
                    ]
                }
                
                add_breadcrumb("Sending Slack notification", category="notification", 
                             data={"message_blocks": len(message['blocks'])})
                
                response = requests.post(webhook_url, json=message, timeout=10)
                
                if response.status_code != 200:
                    raise requests.HTTPError(f"Slack webhook returned {response.status_code}: {response.text}")
                
                results['apis_tested'].append('Slack - SUCCESS')
                results['operations_tracked'].append('slack_notification')
                
                add_breadcrumb("Slack notification sent successfully", category="notification")
        ...
        try:
            ...
            add_breadcrumb("Research results saved", category="file_operation", 
            ...
            add_breadcrumb("Market research completed successfully", 
                         category="automation", 
