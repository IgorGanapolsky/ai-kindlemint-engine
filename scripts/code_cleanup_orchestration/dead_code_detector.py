...
class DeadCodeDetector:
    ...
    def build_dependency_graph(self) -> DependencyGraph:
        ...
        for file_path in nodes:
            ...
            if any(file_name == ep or file_name.startswith('main') for ep in self.entry_points):
    ...
    def _find_reachable_files(self, entry_points: Set[str], edges: Dict[str, Set[str]]) -> Set[str]:
    ...
    def _is_import_used(self, import_name: str, analyzer: ASTAnalyzer) -> bool:
        ...
        if base_name in analyzer.function_calls or base_name in analyzer.attribute_accesses:
            return True
    ...
    def detect_unused_functions(self) -> List[DeadCodeCandidate]:
        ...
        for file_path, analyzer in self.file_ast_data.items():
            for func_name, line_number in analyzer.function_definitions.items():
                ...
                if func_name in ['main', 'run', 'execute'] or file_path in self.dependency_graph.entry_points:
                ...
                if confidence > self.config.get('min_confidence_threshold', 0.7):
                    candidate = DeadCodeCandidate(
                        ...
                        dependencies=[],
                        last_modified=self._get_file_modified_time(file_path),
                        size_impact=self._estimate_function_size(file_path, line_number)
                    )
                    candidates.append(candidate)
    ...
    def detect_unused_classes(self) -> List[DeadCodeCandidate]:
        ...
        for analyzer in self.file_ast_data.values():
            ...
            all_class_references.update(analyzer.attribute_accesses.keys())  # Class.method calls
    ...
    def correlate_with_coverage(self, coverage_file: Optional[str] = None) -> List[DeadCodeCandidate]:
        ...
        if not coverage_file:
            self.logger.warning("No coverage file found. Skipping coverage correlation.")
            return []
        ...
        self.logger.info(f"Coverage correlation with {coverage_file} not yet implemented")
        ...
        try:
            ...
            for i in range(line_number, len(lines)):
                ...
                if base_indent is not None and indent > base_indent:
                    size += 1
                elif base_indent is not None and indent <= base_indent and i > line_number:
                    break
    ...
    def run_analysis(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'summary': {
                ...
                'high_confidence_candidates': len([c for c in self.candidates if c.confidence > 0.8]),
                'potential_lines_saved': sum(c.size_impact for c in self.candidates),
                'orphaned_files': len(self.dependency_graph.orphaned_files)
            },
            'candidates': [asdict(c) for c in self.candidates[:50]],  # Top 50 candidates
            'dependency_graph': {
        ...
        self.logger.info(f"Analysis complete. Found {len(self.candidates)} dead code candidates")
        return report
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Dead Code Detector for AI-KindleMint Engine')
    ...
    print(f"Potential lines of code that could be removed: {report['summary']['potential_lines_saved']}")
