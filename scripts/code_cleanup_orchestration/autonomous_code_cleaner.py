...
class AutonomousCodeCleaner:
            ...
            for file_path in self.repo_path.glob(pattern):
                if file_path.exists():
                    try:
                        ...
                        self.cleanup_results['files_removed'].append(str(file_path.relative_to(self.repo_path)))
                        removed_count += 1
                        bytes_saved += size
            ...
            if archive_path.exists() and archive_path.is_dir():
                try:
                    ...
                    if files_removed > 0:
                        ...
                        print(f"   ✓ Cleaned {archive_dir}: {files_removed} files removed")
                    ...
                    if ('archive' in str(remove_file) or 
                        ...
                        try:
                            ...
                            self.cleanup_results['duplicates_removed'].append({
                                'removed': str(remove_file.relative_to(self.repo_path)),
                                'kept': str(keep_file.relative_to(self.repo_path)),
                                'size': size
                            })
                            self.cleanup_results['bytes_saved'] += size
                            duplicates_removed += 1
                            print(f"   ✓ Removed duplicate: {remove_file.relative_to(self.repo_path)}")
    ...
    def remove_empty_directories(self):
        ...
        for dir_path in all_dirs:
            if (dir_path.exists() and 
                dir_path.is_dir() and 
                not self.is_important_directory(dir_path) and
                not list(dir_path.iterdir())):  # Empty directory
                try:
                    dir_path.rmdir()
                    self.cleanup_results['files_removed'].append(str(dir_path.relative_to(self.repo_path)) + '/')
                    removed_count += 1
    ...
    def is_important_directory(self, dir_path: Path) -> bool:
        ...
        important_dirs = {
            '.git', '.github', 'src', 'scripts', 'tests', 'lambda',
            'output', 'kindlemint', 'config', 'docs'
        }
    ...
    def generate_cleanup_report(self):
        ...
        report_path = self.repo_path / 'scripts/code_cleanup_orchestration/reports/cleanup_report.json'
        ...
        self.cleanup_results['summary'] = {
            ...
            'total_directories_cleaned': len(self.cleanup_results['directories_cleaned']),
