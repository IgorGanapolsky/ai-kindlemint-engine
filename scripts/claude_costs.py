...
Examples:
  ...
  claude_costs export costs.csv        # Export costs to CSV
    ...
    track_parser.add_argument("--model", default="claude-3-sonnet", 
    ...
    summary_parser.add_argument("--days", type=int, default=30, help="Number of days to summarize")
    
    # Details command
    details_parser = subparsers.add_parser("details", help="Show detailed commit costs")
    details_parser.add_argument("--last", type=int, default=10, help="Number of commits to show")
    ...
    if args.command == "init":
        ...
        # Repository analysis
        print(f"\nRepository Analysis:")
        print(f"  Full repo cost estimate: {format_currency(last_cost['full_repo_cost'])}")
        ...
        if "error" in summary:
            ...
            print(f"Average per commit: {format_currency(summary['average_cost_per_commit'])}")
        ...
        print("=" * 80)
        print(f"{'Hash':<10} {'Date':<20} {'Cost':<12} {'Files':<8} {'Tokens':<10} Message")
        print("-" * 80)
        
        for commit in commits:
            date = datetime.fromisoformat(commit['timestamp']).strftime("%Y-%m-%d %H:%M")
            cost = format_currency(commit['cost'])
            msg = commit['message'][:30] + "..." if len(commit['message']) > 30 else commit['message']
            
            print(f"{commit['hash']:<10} {date:<20} {cost:<12} {commit['files_changed']:<8} "
        ...
        if output_path.suffix == ".csv":
            ...
            with open(output_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["Hash", "Timestamp", "Cost", "Tokens", "Files", "Model", "Message"])
            ...
            print(f"✅ Exported {len(commit_costs.get('commits', []))} commits to {output_path}")
