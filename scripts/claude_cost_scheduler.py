...
class ClaudeCostScheduler:
    ...
    def setup_schedules(self) -> None:
        ...
        if self.config["daily_summary"]["enabled"]:
            ...
            logger.info(f"Daily summary scheduled at {self.config['daily_summary']['time']}")
        ...
        if self.config["efficiency_reports"]["enabled"]:
            ...
            if freq == "weekly":
                ...
                if day == "monday":
                    ...
                    schedule.every().tuesday.at(time_str).do(self._run_efficiency_report)
                elif day == "wednesday":
                    schedule.every().wednesday.at(time_str).do(self._run_efficiency_report)
                elif day == "thursday":
                    schedule.every().thursday.at(time_str).do(self._run_efficiency_report)
                elif day == "friday":
                    schedule.every().friday.at(time_str).do(self._run_efficiency_report)
                elif day == "saturday":
                    schedule.every().saturday.at(time_str).do(self._run_efficiency_report)
                elif day == "sunday":
                ...
                logger.info(f"Daily efficiency reports scheduled at {self.config['efficiency_reports']['time']}")
                ...
                if current_hash and current_hash != last_commit_hash:
                    ...
                    for commit in commit_costs.get("commits", []):
                        if commit["hash"] == current_hash[:8]:
                            cost = commit["cost"]
                            threshold = self.config["realtime_commits"]["min_cost_threshold"]
                            
                            if cost >= threshold:
                                logger.info(f"Commit cost ${cost:.4f} exceeds threshold ${threshold}")
    ...
    def test_notifications(self) -> None:
        ...
        tests = [
            ...
            ("Budget Alert (Daily)", lambda: self.notifier.send_budget_alert(1.00, "daily")),
        ]
...
"""
    ...
    plist_path = Path.home() / "Library/LaunchAgents/com.claudeflow.cost-scheduler.plist"
