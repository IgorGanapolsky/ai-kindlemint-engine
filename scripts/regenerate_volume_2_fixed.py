...
def create_puzzle_image_fixed(grid, output_path, is_solution=False, initial_grid=None):
        ...
        for col in range(9):
            ...
            if number != 0:  # ONLY draw if not zero
                ...
                try:
                    ...
                    text_width, text_height = draw.textsize(text, font=use_font)
                
                text_x = x - text_width // 2
                text_y = y - text_height // 2
                
                draw.text((text_x, text_y), text, fill=text_color, font=use_font)
            else:
                empty_count += 1
    ...
    print(f"  → Generated {'solution' if is_solution else 'puzzle'} image: {filled_count} filled, {empty_count} empty cells")
...
def regenerate_all_puzzle_images(base_dir):
    ...
    for puzzle_id in collection["puzzles"]:
        ...
        if empty != expected_empty:
            print(f"  ❌ ERROR: Expected {expected_empty} empty cells, got {empty}")
...
if __name__ == "__main__":
    ...
    if generate_volume_2_pdf(base_dir):
        ...
        print(f"Fixed PDF: {base_dir}/paperback/Large_Print_Sudoku_Masters_Volume_2_Interior.pdf")
    else:
