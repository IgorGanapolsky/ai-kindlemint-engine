...
class ProductionQAValidator:
    ...
    def validate_pdf(self, pdf_path):
        ...
        try:
            with open(pdf_path, 'rb') as file:
                ...
                if page_count != 156:
                    self.critical_issues.append(f"Wrong page count: {page_count} (expected 156)")
                else:
                ...
                if blank_pages:
                    self.critical_issues.append(f"Nearly blank pages found: {blank_pages}")
                else:
                ...
                if grids_found < 5:
                    self.critical_issues.append("Crossword grids appear to be missing or improperly rendered")
                else:
                    self.passed_checks.append(f"✅ Found {grids_found} crossword grids in sample")
                ...
                if not answer_key_found:
                    ...
                    self.critical_issues.append(f"Empty solution grids for puzzles: {empty_solutions[:10]}...")
                else:
                    self.passed_checks.append("✅ Answer key found with filled solutions")
                ...
                for page_num in [6, 10, 14, 18, 22]:  # Sample puzzle pages
                    if page_num < page_count:
                        ...
                        if hasattr(page, '_contents'):
                            ...
                            if ' 0 g' in contents_str or '0 0 0 rg' in contents_str:  # Black color commands
                                black_squares_found = True
                                break
                
                if not black_squares_found:
                    self.warnings.append("⚠️  Black squares may not be rendering properly")
                else:
                ...
                if file_size_mb > 400:
                    self.critical_issues.append(f"File too large: {file_size_mb:.2f} MB (KDP limit ~400MB)")
                elif file_size_mb < 0.1:
                    self.critical_issues.append(f"File suspiciously small: {file_size_mb:.2f} MB")
                else:
    ...
    def generate_report(self):
        ...
        # Calculate score
        total_checks = len(self.passed_checks) + len(self.critical_issues) + len(self.warnings)
        if total_checks == 0:
            score = 0
        else:
            # Critical issues = -20 points each, warnings = -5 points each
            score = max(0, 100 - (len(self.critical_issues) * 20) - (len(self.warnings) * 5))
