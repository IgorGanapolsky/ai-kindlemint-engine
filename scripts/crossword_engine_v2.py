...
class CrosswordEngineV2:
                    ...
                    if self._can_place_down(grid, row, col, len(answer)):
                        ...
                        for i, letter in enumerate(answer):
                            if (
                                ...
                                and filled_grid[row + i][col] != letter
                            ):
                                conflict = True
                                break
        ...
        for row in range(self.grid_size):
            for col in range(self.grid_size):
                ...
                if grid[row][col] == "#":
                    ...
                    if (
                        needs_number and not is_solution
                    ):  # Don't add numbers to solution grids
                        draw.text(
    ...
    def generate_puzzles(self):
        ...
        for i in range(self.puzzle_count):
            ...
            solution_path, _ = self.create_grid_image(
                grid, puzzle_id, is_solution=True, filled_grid=filled_grid
            )
