...
def main():
    base_dir = Path("/Users/igorganapolsky/workspace/git/ai/ai-kindlemint-engine/books/active_production/Large_Print_Crossword_Masters")
    ...
    for volume in range(1, 5):
        ...
        if hardcover_dir.exists():
            for pdf_file in hardcover_dir.glob("*.pdf"):
                if ("FINAL" in pdf_file.name or "interior" in pdf_file.name) and "cover" not in pdf_file.name.lower():
                    pages = check_pdf_page_count(pdf_file)
    ...
    for volume, formats in results.items():
        ...
        for format_type, info in formats.items():
            if isinstance(info, dict):
                print(f"  {format_type.title()}: {info['pages']} pages ({info['file']})")
