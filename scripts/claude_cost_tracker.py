...
class ClaudeCostTracker:
    ...
    def calculate_file_cost(self, file_analysis: Dict, model: str = "claude-3-sonnet") -> float:
    ...
    def analyze_commit_cost(self, changed_files: List[str], model: str = "claude-3-sonnet") -> Dict:
    ...
    def update_commit_costs(self, commit_analysis: Dict, commit_message: str = "") -> None:
    ...
    def update_last_commit_cost(self, commit_analysis: Dict) -> None:
        ...
        for file_path in all_files[:1000]:  # Limit to first 1000 files for performance
            if file_path and file_path.endswith(('.py', '.js', '.ts', '.java', '.cpp', '.md')):
                analysis = self.analyze_file_complexity(file_path)
                full_repo_tokens += analysis["tokens"]
        
        full_repo_cost = (full_repo_tokens * 18.0) / 1_000_000  # Average pricing estimate
    ...
    def track_commit(self, commit_message: str = "", model: str = "claude-3-sonnet") -> Dict:
    ...
    def get_cost_summary(self, days: int = 30) -> Dict:
        ...
        recent_commits = [
            c for c in commit_costs["commits"] 
        ...
        return {
            ...
            "most_expensive_commit": max(recent_commits, key=lambda c: c["cost"]) if recent_commits else None
        }
