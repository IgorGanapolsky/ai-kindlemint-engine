...
class SudokuValidator:
    ...
    def validate_puzzle(self, puzzle: List[List[int]], solution: List[List[int]]) -> Dict[str, any]:
        """
        ...
        if clue_count < 17:
            results["valid"] = False
            results["errors"].append(f"Puzzle has only {clue_count} clues (minimum is 17)")
        ...
        if symmetry_type == "none":
            results["warnings"].append("Puzzle lacks symmetry (not required but often preferred)")
    ...
    def _validate_grid_structure(self, grid: List[List[int]], name: str, results: Dict):
        ...
        if len(grid) != self.grid_size:
            results["valid"] = False
            results["errors"].append(f"{name} has {len(grid)} rows (expected {self.grid_size})")
            return
        ...
        for i, row in enumerate(grid):
            ...
            if len(row) != self.grid_size:
                results["valid"] = False
                results["errors"].append(f"{name} row {i} has {len(row)} columns (expected {self.grid_size})")
    
    def _validate_values(self, grid: List[List[int]], name: str, allow_zeros: bool, results: Dict):
        ...
        for i, row in enumerate(grid):
            for j, cell in enumerate(row):
                if not isinstance(cell, int):
                    results["valid"] = False
                    results["errors"].append(f"{name}[{i}][{j}] is not an integer: {type(cell)}")
                elif cell not in valid_range:
                    results["valid"] = False
                    results["errors"].append(f"{name}[{i}][{j}] has invalid value: {cell}")
    ...
    def _puzzle_matches_solution(self, puzzle: List[List[int]], solution: List[List[int]]) -> bool:
    ...
    def _check_minimality_sample(self, puzzle: List[List[int]], sample_size: int = 5) -> bool:
        """
        ...
        import random
        test_positions = random.sample(clue_positions, min(sample_size, len(clue_positions)))
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Sudoku Validator - Quality Assurance Tool')
        ...
        for puzzle_result in results['puzzle_results']:
            ...
            if puzzle_result['valid']:
                ...
                if 'stats' in puzzle_result:
                    ...
                    print(f"  📈 Difficulty: {stats.get('estimated_difficulty', 'N/A')}")
            else:
                print(f"  ❌ Invalid")
