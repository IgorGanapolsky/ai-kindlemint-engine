        ...
        if persona_type == PersonaType.BUSY_PARENT:
            return Persona(
                ...
                pain_points=[
                    "limited free time",
                    "need mental breaks",
                    "finding quality family activities",
                ],
                buying_behavior={
                    "frequency": "monthly",
                    "max_price": 12.99,
...
def __init__(self):
    ...
    def _generate_summary(
        self, book_spec: Dict[str, Any], viability: str, evaluations: List[Dict]
    ) -> str:
    ...
    def generate_market_report(self, output_path: Optional[Path] = None) -> Path:
        ...
        if not output_path:
            output_path = (
                Path("research")
                / f"market_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
...
def main():
    ...
    parser.add_argument(
        "--type", required=True, choices=["crossword", "sudoku", "word_search"]
    )
    parser.add_argument(
        "--difficulty", default="medium", choices=["easy", "medium", "hard", "mixed"]
    )
    parser.add_argument("--theme", help="Book theme")
    parser.add_argument("--price", type=float, default=9.99, help="Planned price")
    parser.add_argument(
        "--format", default="paperback", choices=["paperback", "hardcover", "kindle"]
    )
    parser.add_argument(
        "--target-personas", nargs="+", help="Specific personas to target"
    )
