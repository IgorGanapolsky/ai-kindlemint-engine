...
*KindleMint AI Issue Management System*"""
    ...
    async def cleanup_duplicate_reports(self) -> Dict[str, int]:
        ...
        for issue in issues:
            if self.is_automation_author(issue["author"]) and self.is_duplicate_report(
                issue["title"]
            ):
        ...
        for report_type, report_issues in report_groups.items():
            if len(report_issues) > self.max_reports_per_type:
                ...
                if issues_to_close:
                    ...
                    self.logger.info(
                        f"🧹 Cleaned {len(issues_to_close)} duplicate '{report_type}' reports"
                    )
    ...
    async def run_full_cleanup(self) -> Dict[str, any]:
        ...
        automation_issues = [
            i for i in final_issues if self.is_automation_author(i["author"])
        ]
        ...
        print(
            f"   • Report types cleaned: {results.get('report_types_cleaned', 0)}")
        print(
            f"   • Stale issues closed: {results.get('stale_issues_closed', 0)}")
        print(
            f"   • Remaining automation issues: {results.get('remaining_automation_issues', 0)}"
        )
