...
class IntegratedCrosswordQA:
    ...
    def validate_pdf(self, pdf_path: str) -> QAReport:
        ...
        try:
            ...
            weighted_score = sum(score * weight / 100 for score, weight in scores.values())
    ...
    def _parse_clues_from_text(self, text: str) -> Tuple[Dict[int, str], Dict[int, str]]:
    ...
    def _validate_uniqueness(self, content: Dict, report: QAReport) -> int:
        ...
        for puzzle in puzzles:
            ...
            if sig in puzzle_signatures:
                duplicates.append({
                    ...
                    'duplicate_of': puzzle_signatures[sig]
                })
            else:
        ...
        overused = [(clue, count) for clue, count in clue_usage.items() 
    ...
    def _validate_solvability(self, content: Dict, report: QAReport) -> int:
        ...
        puzzles_with_solutions = sum(1 for p in content['puzzles'] 
                                   if p.get('has_solution'))
    ...
    def _generate_recommendations(self, report: QAReport):
        ...
        if report.status == 'CRITICAL_FAIL':
            report.recommendations.insert(0, 
                "⚠️  DO NOT PUBLISH - Critical issues must be fixed first!"
            )
