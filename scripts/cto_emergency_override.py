...
class CTOOverrideSystem:
    ...
    def emergency_override_all(self):
        ...
        for pr in prs:
            try:
                ...
                if approved:
                    logger.info(
                        f"Force merging approved PR #{pr.number}: {pr.title}")
                    pr.merge(
                        merge_method="squash",
    ...
    def _handle_bot_pr(self, pr, hours_old):
        ...
        if bot_name in self.config["trusted_bots"]:
            timeout = self.config["override_rules"]["auto_merge_timeout_hours"][
                "trusted_bots"
            ]

            if hours_old > timeout:
                logger.info(
                    f"Auto-merging trusted bot PR #{pr.number} from {bot_name}")
                try:
                    pr.merge(
                        merge_method="squash", commit_title=f"{pr.title} [Auto-merged]"
                    )
            ...
            # Untrusted bot
            close_timeout = self.config["override_rules"]["auto_close_after_hours"][
                "untrusted_bots"
            ]

            if hours_old > close_timeout:
                logger.info(
                    f"Closing untrusted bot PR #{pr.number} from {bot_name}")
                pr.edit(state="closed")
                pr.create_issue_comment(
                    "Auto-closed: Untrusted bot PR with no human interaction"
                )
    ...
    def _handle_human_pr(self, pr, hours_old):
        ...
        if approved:
            ...
            if hours_since_update > timeout:
                logger.info(
                    f"Force merging stuck approved PR #{pr.number}: {pr.title}")
                self._try_admin_merge(pr)
    ...
    def _try_admin_merge(self, pr):
        ...
        try:
            ...
            logger.warning(
                f"PR #{pr.number} requires admin merge - escalating to CTO")
            pr.create_issue_comment(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="CTO Emergency Override System")
    parser.add_argument(
        "--emergency",
        action="store_true",
