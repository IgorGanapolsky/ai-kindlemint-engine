...
class WordSearchGenerator:
    ...
    def __init__(self, output_dir, puzzle_count=50, grid_size=15, words_file=None):
        ...
        if words_file:
            ...
            self.words = [w.strip().upper() for w in wf.read_text().splitlines() if w.strip()]
        else:
    ...
    def _generate_grid(self):
        ...
        return [[random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZ') for _ in range(self.grid_size)]
    ...
    def _create_image(self, grid, puzzle_id):
        ...
        for i in range(self.grid_size + 1):
            lw = 2
            # vertical
            draw.line([(margin + i*cell_size, margin), (margin + i*cell_size, img_size - margin)], fill='black', width=lw)
            # horizontal
            draw.line([(margin, margin + i*cell_size), (img_size - margin, margin + i*cell_size)], fill='black', width=lw)
...
def main():
    parser = argparse.ArgumentParser(description="Word Search Generator - Generate puzzles for KindleMint Engine")
    parser.add_argument('--output', required=True, help='Output directory')
    parser.add_argument('--count', type=int, default=50, help='Number of puzzles to generate')
    parser.add_argument('--grid-size', type=int, default=15, help='Grid dimension (NxN)')
    parser.add_argument('--words-file', help='Path to newline-separated words file')
