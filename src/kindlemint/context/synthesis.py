...
class MarketContextAnalyzer:
    ...
    async def get_relevant_context(self, intent_type: str, user_preferences: Optional[Dict] = None) -> MarketContext:
        ...
        # Get target keywords
        target_keywords = await self._generate_target_keywords(intent_type, user_preferences)
    ...
    def _should_refresh_cache(self) -> bool:
        ...
        hours_since_refresh = (datetime.now() - self.last_refresh).total_seconds() / 3600
    ...
    def _filter_trends_by_intent(self, intent_type: str) -> List[MarketTrend]:
        ...
        relevant_categories = intent_categories.get(intent_type, list(self.trend_cache.keys()))
        
        # Filter and sort by popularity
        relevant_trends = [
            trend for trend in self.trend_cache.values()
            if trend.category in relevant_categories or intent_type in ["explore_ideas"]
        ]
        
        return sorted(relevant_trends, key=lambda t: t.popularity_score, reverse=True)[:5]
    
    async def _generate_target_keywords(self, intent_type: str, user_preferences: Optional[Dict]) -> List[str]:
    ...
    def _identify_opportunity_gaps(self, trends: List[MarketTrend]) -> List[str]:
        ...
        all_categories = {"mystery", "romance", "fantasy", "business", "self_help", "science_fiction"}
    ...
    async def _analyze_audience_preferences(self, trends: List[MarketTrend]) -> Dict[str, any]:
        ...
        primary_demographics = sorted(demo_counts.items(), key=lambda x: x[1], reverse=True)[:3]
        ...
        return {
            ...
            "content_preferences": {
                ...
                "popular_themes": ["personal_growth", "problem_solving", "entertainment"]
            }
        }
    ...
    async def _get_seasonal_opportunities(self) -> Dict[str, any]:
        ...
        current_season = seasonal_calendar.get(current_month, {"themes": ["general"], "boost": 1.0})
...
class CreativeContextLibrary:
    ...
    def _initialize_creative_patterns(self) -> List[CreativePattern]:
        ...
        patterns.append(CreativePattern(
            ...
            character_archetypes=["amateur_sleuth", "quirky_sidekick", "suspicious_locals"],
            plot_devices=["red_herrings", "locked_room", "hidden_motive"],
            theme_elements=["justice", "community", "truth"],
            style_guidelines={"tone": "light", "violence": "minimal", "setting": "small_community"},
            success_rate=0.75
        ))
        ...
        patterns.append(CreativePattern(
            ...
            character_archetypes=["stubborn_protagonist", "equally_matched_love_interest"],
            plot_devices=["forced_proximity", "misunderstanding", "grand_gesture"],
            theme_elements=["love_conquers_all", "personal_growth", "vulnerability"],
            style_guidelines={"tone": "passionate", "steam_level": "variable", "pov": "dual"},
            success_rate=0.82
        ))
    ...
    def _find_matching_patterns(self, indicators: Dict[str, any]) -> List[CreativePattern]:
        ...
        for pattern in self.patterns:
            ...
            if ("light" in style_tone and "light_hearted" in indicators["mood_indicators"]) or \
               ("serious" in style_tone and "serious_tone" in indicators["mood_indicators"]):
                match_score += 2
    ...
    def _get_genre_conventions(self, voice_input: VoiceInput) -> Dict[str, any]:
        ...
        return {
            ...
            "romance": {
                "required_elements": ["romantic_relationship", "emotional_journey", "hea_hfn"],
    ...
    def _get_creative_constraints(self, voice_input: VoiceInput) -> Dict[str, any]:
        ...
        return {
            ...
            "content_rating": {"level": "pg13", "restrictions": ["minimal_violence", "fade_to_black"]}
        }
...
class PublishingContextEngine:
    ...
    def _get_target_formats(self, intent_type: str) -> List[PublishingFormat]:
        ...
        formats.append(PublishingFormat(
            format_type="ebook",
            platform="amazon_kdp",
            specifications={
                "file_format": "epub",
    ...
    def _get_metadata_templates(self) -> Dict[str, any]:
        ...
        return {
            ...
            "romance": {
                ...
                "keywords": ["enemies to lovers", "second chance", "small town romance"]
            }
        }
...
class ContextSynthesisEngine:
    ...
    async def synthesize_context(self, user_id: str, voice_input: VoiceInput) -> SynthesizedContext:
        ...
        try:
            ...
            author_ctx, market_ctx, creative_ctx, publishing_ctx = await asyncio.gather(*context_tasks)
            
            # Calculate synthesis weights based on intent and context
            synthesis_weights = self._calculate_synthesis_weights(voice_input, author_ctx)
            ...
            synthesized_context.quality_score = await self._calculate_quality_score(synthesized_context)
            synthesized_context.coherence_score = await self._calculate_coherence_score(synthesized_context)
            
            # Generate optimization suggestions
            synthesized_context.optimization_suggestions = await self._generate_optimization_suggestions(synthesized_context)
            
            # Store synthesis for future reference
            await self._store_synthesis_record(synthesized_context)
            
            self.logger.info(f"Synthesized context for user {user_id} with quality score {synthesized_context.quality_score:.2f}")
    ...
    def _calculate_synthesis_weights(self, voice_input: VoiceInput, author_ctx: AuthorContext) -> ContextSynthesisWeights:
    ...
    async def _calculate_quality_score(self, context: SynthesizedContext) -> float:
        ...
        quality_factors.append(author_completeness * context.synthesis_weights.author_weight)
        
        # Market context quality
        market_relevance = self._assess_market_relevance(context.market)
        quality_factors.append(market_relevance * context.synthesis_weights.market_weight)
        
        # Creative context quality
        creative_richness = self._assess_creative_richness(context.creative)
        quality_factors.append(creative_richness * context.synthesis_weights.creative_weight)
        
        # Publishing context quality
        publishing_readiness = self._assess_publishing_readiness(context.publishing)
        quality_factors.append(publishing_readiness * context.synthesis_weights.publishing_weight)
    ...
    async def _calculate_coherence_score(self, context: SynthesizedContext) -> float:
        ...
        author_creative_coherence = self._check_author_creative_coherence(context.author, context.creative)
        coherence_factors.append(author_creative_coherence)
        
        # Check market-publishing coherence
        market_publishing_coherence = self._check_market_publishing_coherence(context.market, context.publishing)
        coherence_factors.append(market_publishing_coherence)
        
        # Check author-market coherence
        author_market_coherence = self._check_author_market_coherence(context.author, context.market)
    ...
    def _check_author_creative_coherence(self, author_ctx: AuthorContext, creative_ctx: CreativeContext) -> float:
    ...
    def _check_market_publishing_coherence(self, market_ctx: MarketContext, publishing_ctx: PublishingContext) -> float:
    ...
    def _check_author_market_coherence(self, author_ctx: AuthorContext, market_ctx: MarketContext) -> float:
        ...
        if "bestseller" in author_goals and market_ctx.current_trends:
            ...
            trending_categories = {trend.category for trend in market_ctx.current_trends}
            author_genre_names = {g.value.replace('_', ' ') for g in author_ctx.writing_style.genre_preferences}
    ...
    async def _generate_optimization_suggestions(self, context: SynthesizedContext) -> List[str]:
        """Generate optimization suggestions based on context analysis"""
        suggestions = []
        
        # Quality-based suggestions
        if context.quality_score < 0.7:
            suggestions.append("Consider providing more information about your writing preferences to improve personalization")
        
        # Coherence-based suggestions
        if context.coherence_score < 0.6:
            suggestions.append("Your creative preferences and market targets could be better aligned for success")
        
        # Author context suggestions
        if context.author.total_sessions < 3:
            suggestions.append("Continue using the system to build a more complete author profile")
        
        # Market context suggestions
        if not context.market.current_trends:
            suggestions.append("Consider market research to identify trending topics in your genre")
        
        # Creative context suggestions
        if not context.creative.relevant_patterns:
            suggestions.append("Explore different creative patterns to find your unique voice")
