...
class WritingStyleAnalyzer:
    ...
    def __init__(self):
        ...
        self.complexity_indicators = {
            "simple": {"avg_words_per_sentence": (1, 10), "syllables_per_word": (1.0, 1.5)},
            "accessible": {"avg_words_per_sentence": (10, 20), "syllables_per_word": (1.5, 2.0)},
            "advanced": {"avg_words_per_sentence": (20, 30), "syllables_per_word": (2.0, 2.5)},
            "academic": {"avg_words_per_sentence": (30, 50), "syllables_per_word": (2.5, 3.5)}
        }
    ...
    async def analyze_voice_input(self, voice_input: VoiceInput) -> Dict[str, any]:
        ...
        dominant_tone = max(tone_scores, key=tone_scores.get) if tone_scores else "conversational"
    ...
    def _determine_complexity(self, avg_words: float, avg_syllables: float) -> str:
        ...
        for level, ranges in self.complexity_indicators.items():
            ...
            if (word_range[0] <= avg_words <= word_range[1] and 
                syllable_range[0] <= avg_syllables <= syllable_range[1]):
                return level
        
        return "accessible"  # default
    
    def _extract_voice_style_indicators(self, voice_input: VoiceInput) -> Dict[str, any]:
        ...
        return {
            ...
            "pace_preference": "fast" if voice_chars.pace > 1.2 else "slow" if voice_chars.pace < 0.8 else "medium",
    ...
    def _extract_characteristic_phrases(self, text: str) -> List[str]:
        ...
        transitions = ["however", "meanwhile", "in addition", "furthermore", "on the other hand"]
    ...
    def _identify_creative_markers(self, text: str) -> List[str]:
        ...
        for category, words in markers.items():
            ...
            if found_words:
                creative_markers.extend([f"{category}:{word}" for word in found_words[:2]])
...
class PreferenceEngine:
    ...
    def __init__(self):
        self.genre_keywords = {
            BookGenre.MYSTERY: ["mystery", "detective", "clue", "solve", "investigation"],
            BookGenre.ROMANCE: ["love", "romance", "relationship", "heart", "passion"],
            BookGenre.FANTASY: ["magic", "dragon", "quest", "realm", "wizard"],
            BookGenre.SCIENCE_FICTION: ["future", "space", "technology", "alien", "robot"],
            ...
            BookGenre.BUSINESS: ["strategy", "marketing", "profit", "business", "entrepreneur"]
        }
    
    async def update_preferences(self, current_prefs: UserPreferences, voice_input: VoiceInput, 
                               session_context: Dict[str, any]) -> UserPreferences:
        ...
        collaboration_style = self._determine_collaboration_style(voice_input, session_context)
        ...
        updated_prefs = UserPreferences(
            ...
            publishing_goals=list(set(current_prefs.publishing_goals + publishing_goals)),
    ...
    def _analyze_genre_preferences(self, text: str) -> List[BookGenre]:
        ...
        return [genre for genre, score in sorted(genre_scores.items(), key=lambda x: x[1], reverse=True)]
    ...
    def _determine_collaboration_style(self, voice_input: VoiceInput, session_context: Dict[str, any]) -> str:
        """Determine preferred collaboration style"""
        text = voice_input.text.lower()
        
        if any(phrase in text for phrase in ["help me", "guide me", "suggestions"]):
            return "guided"
        elif any(phrase in text for phrase in ["work together", "collaborate", "partner"]):
    ...
    def _determine_feedback_frequency(self, voice_input: VoiceInput) -> str:
        ...
        if any(phrase in text for phrase in ["check often", "frequent feedback", "keep checking"]):
            return "frequent"
        elif any(phrase in text for phrase in ["minimal feedback", "less interruption", "minimal"]):
            return "minimal"
    ...
    def _determine_quality_focus(self, voice_input: VoiceInput) -> str:
        ...
        if any(phrase in text for phrase in ["perfect", "high quality", "best possible"]):
...
class SuccessPatternsAnalyzer:
    ...
    async def analyze_success_patterns(self, past_works: List[WorkProfile], 
                                     recent_sessions: List[Dict[str, any]]) -> SuccessPatterns:
    ...
    def _analyze_effective_genres(self, past_works: List[WorkProfile]) -> List[BookGenre]:
        ...
        return [genre for genre, avg in sorted(genre_averages.items(), key=lambda x: x[1], reverse=True)]
    ...
    def _analyze_successful_themes(self, past_works: List[WorkProfile]) -> List[str]:
        ...
        return [theme for theme, avg in sorted(theme_averages.items(), key=lambda x: x[1], reverse=True)[:10]]
    
    def _analyze_optimal_writing_times(self, recent_sessions: List[Dict[str, any]]) -> List[str]:
        ...
        return [slot for slot, avg in sorted(slot_averages.items(), key=lambda x: x[1], reverse=True)]
    
    def _analyze_productive_moods(self, recent_sessions: List[Dict[str, any]]) -> List[CreativeMood]:
        ...
        return [mood for mood, avg in sorted(mood_averages.items(), key=lambda x: x[1], reverse=True)]
    ...
    def _calculate_quality_indicators(self, past_works: List[WorkProfile]) -> Dict[str, float]:
        """Calculate quality indicators based on past performance"""
        if not past_works:
            return {}
        
        total_works = len(past_works)
        successful_works = sum(1 for work in past_works if work.success_metrics.get("overall_score", 0) > 0.7)
        
        avg_length = sum(work.length for work in past_works) / total_works
        avg_success = sum(work.success_metrics.get("overall_score", 0) for work in past_works) / total_works
        
        return {
            "success_rate": successful_works / total_works,
            "average_length": avg_length,
            "average_success_score": avg_success,
            "consistency_score": 1.0 - (max(w.success_metrics.get("overall_score", 0) for w in past_works) - 
...
class AuthorContextBuilder:
    ...
    async def build_context(self, user_id: str, voice_input: VoiceInput) -> AuthorContext:
        ...
        try:
            ...
            updated_style = await self._update_writing_style(
                existing_context.writing_style if existing_context else WritingStyleProfile(),
                style_analysis
            )
            ...
            recent_sessions = await self.memory_store.get_recent_sessions(user_id, days=30)
            
            # Update success patterns
            past_works = existing_context.past_works if existing_context else []
            updated_success_patterns = await self.success_analyzer.analyze_success_patterns(
            ...
            author_context = AuthorContext(
                ...
                total_sessions=(existing_context.total_sessions + 1) if existing_context else 1,
                total_words_created=(existing_context.total_words_created + len(voice_input.text.split())) 
                                  if existing_context else len(voice_input.text.split())
            )
            
            # Store updated context
            await self.memory_store.store_author_context(user_id, author_context)
            
            self.logger.info(f"Built author context for user {user_id} with mood {current_mood.value}")
    ...
    async def _update_writing_style(self, current_style: WritingStyleProfile, 
                                  style_analysis: Dict[str, any]) -> WritingStyleProfile:
    ...
    def _blend_complexity(self, current: float, new: float, weight: float = 0.3) -> float:
    ...
    def _determine_creative_mood(self, voice_input: VoiceInput) -> CreativeMood:
        ...
        elif "explore" in voice_input.text.lower() or "experiment" in voice_input.text.lower():
        ...
        elif "think" in voice_input.text.lower() or "consider" in voice_input.text.lower():
