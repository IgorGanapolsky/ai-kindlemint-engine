...
def __init__(self):
    ...
    async def classify_intent(
        self, text: str, voice_characteristics: VoiceCharacteristics
    ) -> Tuple[Intent, float]:
        ...
        intent_scores = self._adjust_scores_by_voice(
            intent_scores, voice_characteristics
        )
    ...
    def _adjust_scores_by_voice(
        self, scores: Dict[Intent, float], voice_chars: VoiceCharacteristics
    ) -> Dict[Intent, float]:
...
def __init__(self):
    ...
    def _analyze_voice_emotions(
        self, voice_chars: VoiceCharacteristics
    ) -> Dict[str, float]:
    ...
    def _determine_creative_mood(
        self, text: str, voice_chars: VoiceCharacteristics
    ) -> CreativeMood:
        ...
        if voice_chars.energy_level > 0.8:
            mood_scores[CreativeMood.ENERGETIC] = (
                mood_scores.get(CreativeMood.ENERGETIC, 0) + 0.3
            )
        if voice_chars.pace > 1.2:
            mood_scores[CreativeMood.INSPIRED] = (
                mood_scores.get(CreativeMood.INSPIRED, 0) + 0.2
            )
        if voice_chars.confidence_level > 0.8:
            mood_scores[CreativeMood.PASSIONATE] = (
                mood_scores.get(CreativeMood.PASSIONATE, 0) + 0.2
            )
    ...
    def _determine_primary_emotion(
        self, text_emotions: Dict[str, float], voice_emotions: Dict[str, float]
    ) -> str:
    ...
    def _get_secondary_emotions(
        ...
        sorted_emotions = sorted(
            combined_emotions.items(), key=lambda x: x[1], reverse=True
        )
        return [emotion for emotion, score in sorted_emotions[1:4] if score > 0.1]

    def _calculate_energy_level(
        self, voice_chars: VoiceCharacteristics, text_emotions: Dict[str, float]
    ) -> float:
    ...
    def _calculate_enthusiasm(
        self, text_emotions: Dict[str, float], voice_chars: VoiceCharacteristics
    ) -> float:
        """Calculate enthusiasm score"""
        # Text-based enthusiasm
        enthusiasm_emotions = ["excited", "passionate", "thrilled", "amazing"]
        text_enthusiasm = sum(
            text_emotions.get(emotion, 0) for emotion in enthusiasm_emotions
        )

        # Voice-based enthusiasm (pace + confidence)
        voice_enthusiasm = (
            voice_chars.pace - 1.0
        ) * 0.5 + voice_chars.confidence_level * 0.5
...
def __init__(self):
    ...
    async def extract_characteristics(
        self, audio_analysis: Dict[str, any], text: str
    ) -> VoiceCharacteristics:
    ...
    def _extract_emphasis_patterns(
        self, audio_analysis: Dict[str, any], text: str
    ) -> List[str]:
    ...
    def _assess_collaborative_language(self, text: str) -> float:
        ...
        collaborative_count = sum(
            1 for word in collaborative_words if word in text_lower
        )
    ...
    async def process_voice_input(
        ...
        try:
            ...
            voice_characteristics = await self.voice_extractor.extract_characteristics(
                transcription_result["audio_analysis"], transcription_result["text"]
            )

            # Step 3: Classify intent
            intent, intent_confidence = await self.intent_classifier.classify_intent(
                transcription_result["text"], voice_characteristics
            )
            ...
            voice_input = VoiceInput(
                ...
                processing_metadata={
                    ...
                    "transcription_method": transcription_result.get(
                        "method", "whisper"
                    ),
            ...
            self.logger.error(
                f"Failed to process voice input for session {session_id}: {e}"
            )
    ...
    async def process_text_input(self, text: str, session_id: str) -> VoiceInput:
        ...
        try:
            ...
            default_voice_chars = VoiceCharacteristics(
                tone="neutral", pace=1.0, clarity_score=1.0, confidence_level=0.8
            )

            # Classify intent
            intent, intent_confidence = await self.intent_classifier.classify_intent(
                text, default_voice_chars
            )
            ...
            self.logger.info(
                f"Processed text input for session {session_id}: intent={intent.value}"
            )
            ...
            self.logger.error(
                f"Failed to process text input for session {session_id}: {e}"
            )
