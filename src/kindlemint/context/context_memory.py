...
class ContextMemoryStore:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("CREATE INDEX IF NOT EXISTS idx_sessions_user_id ON vibecode_sessions (user_id)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_voice_inputs_session ON voice_inputs (session_id)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_synthesis_session ON context_synthesis_history (session_id)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_metrics_user ON success_metrics (user_id)")
    ...
    async def get_author_context(self, user_id: str) -> Optional[AuthorContext]:
        ...
        try:
            ...
            self.logger.error(f"Failed to retrieve author context for user {user_id}: {e}")
            return None
    ...
    async def store_vibecode_session(self, session: VibecodeSession) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to store vibecode session {session.session_id}: {e}")
            return False
    ...
    async def store_voice_input(self, voice_input: VoiceInput) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to store voice input {voice_input.input_id}: {e}")
            return False
    
    async def get_recent_sessions(self, user_id: str, days: int = 30) -> List[Dict[str, any]]:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    sessions.append({
                        ...
                        "mood": session_dict.get("session_metadata", {}).get("mood", "focused"),
                        "energy_level": session_dict.get("session_metadata", {}).get("energy_level", 0.5),
                        "success_score": session_dict.get("session_metadata", {}).get("success_score", 0.0)
                    })
                
                return sessions
                
        except Exception as e:
            self.logger.error(f"Failed to retrieve recent sessions for user {user_id}: {e}")
            return []
    
    async def store_success_metric(self, user_id: str, session_id: Optional[str],
                                 metric_type: str, metric_data: Dict[str, any]) -> bool:
    ...
    async def cleanup_old_data(self, days_to_keep: int = 90) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                self.logger.info(f"Cleaned up old data: {voice_deleted} voice inputs, "
    ...
    def _serialize_author_context(self, context: AuthorContext) -> str:
        ...
        serializable_context = {
            ...
            "writing_style": {
                ...
                "genre_preferences": [g.value for g in context.writing_style.genre_preferences],
            ...
            "success_patterns": {
                "effective_genres": [g.value for g in context.success_patterns.effective_genres],
                ...
                "productive_moods": [m.value for m in context.success_patterns.productive_moods],
    ...
    def _deserialize_author_context(self, json_str: str) -> AuthorContext:
        ...
        context.writing_style.sentence_length_preference = style_data.get("sentence_length_preference", "medium")
        context.writing_style.vocabulary_level = style_data.get("vocabulary_level", "accessible")
        context.writing_style.narrative_voice = style_data.get("narrative_voice", "third_person")
        context.writing_style.dialogue_style = style_data.get("dialogue_style", "natural")
        context.writing_style.pacing_preference = style_data.get("pacing_preference", "balanced")
        context.writing_style.genre_preferences = [BookGenre(g) for g in style_data.get("genre_preferences", [])]
        ...
        context.preferences.preferred_length = prefs_data.get("preferred_length", "medium")
        context.preferences.target_audience = prefs_data.get("target_audience", "general")
        ...
        context.preferences.collaboration_style = prefs_data.get("collaboration_style", "guided")
        context.preferences.feedback_frequency = prefs_data.get("feedback_frequency", "regular")
