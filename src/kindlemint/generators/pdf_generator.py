...
class PDFGenerator:
    ...
    async def create_puzzle_book(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(f"🚀 Creating puzzle book PDF: {request.get('book_title', 'Untitled')}")
            ...
            total_pages = puzzle_pages + solution_pages + cover_pages + 2  # + title page + credits
    ...
    async def validate_pdf_layout(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(puzzles) > 200:
                return {
                    "valid": False,
                    "error": "Too many puzzles (max 200)"
                }
            ...
            if not (10 <= font_size <= 24):
                return {
                    "valid": False,
                    "error": "Font size must be between 10 and 24"
                }
            ...
            return {
                "valid": True,
                "message": "PDF layout parameters are valid"
            }
    ...
    async def optimize_pdf_size(self, pdf_path: str, target_size_mb: Optional[float] = None) -> Dict[str, Any]:
    ...
    async def _generate_puzzle_pages(self, puzzles: List[Dict], request: Dict[str, Any]) -> str:
        """Generate puzzle pages PDF"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        pdf_path = f"output/pdfs/puzzles_only_{timestamp}.pdf"
        return pdf_path
    
    async def _generate_solution_pages(self, puzzles: List[Dict], request: Dict[str, Any]) -> str:
