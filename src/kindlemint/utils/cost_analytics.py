...
class CostAnalytics:
    ...
    def _calculate_trends(self, periods: Dict) -> Dict[str, float]:
        ...
        if periods["last_month"]["cost"] > 0:
            mom_change = ((periods["month"]["cost"] - periods["last_month"]["cost"]) 
                         / periods["last_month"]["cost"]) * 100
        ...
        if periods["month"]["commits"] > 14:  # Need enough data
            ...
            if monthly_avg_weekly > 0:
                wow_change = ((weekly_avg - monthly_avg_weekly) / monthly_avg_weekly) * 100
    ...
    def generate_badges(self, analytics: Optional[Dict] = None) -> Dict[str, str]:
        ...
        badges["monthly"] = self._generate_badge(
            ...
            self._get_budget_color(analytics["periods"]["month"]["cost"], 50.0),  # $50 budget
            style="for-the-badge"
        )
        ...
        if analytics["last_commit"]:
            badges["last_commit"] = self._generate_badge(
                "Last Commit",
                analytics["last_commit"]["cost"],
                "blue",
                style="flat"
            )
        
        # Budget status badge
        budget_percent = (analytics["periods"]["month"]["cost"] / 50.0) * 100  # $50 budget
        budget_color = self._get_budget_color(analytics["periods"]["month"]["cost"], 50.0)
    ...
    def _generate_badge(self, label: str, value: float | str, color: str, 
                       style: str = "flat", is_currency: bool = True) -> str:
        ...
        base_url = f"https://img.shields.io/badge/{encoded_label}-{encoded_message}-{color}"
    ...
    def generate_cost_report(self) -> str:
        ...
        report.append(f"- **Monthly Cost**: ${analytics['periods']['month']['cost']:.2f}")
        report.append(f"- **Daily Average**: ${analytics['daily_average']:.2f}")
        report.append(f"- **Projected Monthly**: ${analytics['monthly_projection']:.2f}")
        report.append(f"- **Budget Status**: {(analytics['periods']['month']['cost'] / 50.0 * 100):.0f}% of $50")
        
        # Trends
        trend_mom = analytics['trends']['month_over_month']
        trend_symbol = "📈" if trend_mom > 0 else "📉" if trend_mom < 0 else "➡️"
        report.append(f"- **Month-over-Month**: {trend_symbol} {trend_mom:+.1f}%\n")
