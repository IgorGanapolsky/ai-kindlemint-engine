...
from scripts.api_manager import APIManager, APIProvider, APIConfig
from scripts.sentry_agent_monitoring import (
    get_agent_monitor, AgentContext, PromptTrace, 
    monitor_ai_agent
)
...
class EnhancedAPIManager(APIManager):
    ...
    def generate_text(self, 
        ...
        context = AgentContext(
            ...
            task_name=task_name,
            model=model or self.configs.get(provider, APIConfig(provider=provider)).model,
    ...
    def _call_openai(self, prompt: str, model: Optional[str], 
    ...
    def _call_gemini(self, prompt: str, model: Optional[str],
                    temperature: float, max_tokens: Optional[int],
                    system_prompt: Optional[str], **kwargs) -> Dict[str, Any]:
    ...
    def generate_image(self, 
        ...
        context = AgentContext(
            ...
            agent_type="dalle",
            task_name=task_name,
            model=self.configs.get(APIProvider.DALLE, APIConfig(provider=APIProvider.DALLE)).model,
            metadata={
                "size": size,
                "quality": quality,
                "count": n
    ...
    def batch_generate(self,
        ...
        batch_context = AgentContext(
            ...
            task_name=task_name,
            model=kwargs.get('model', self.configs.get(provider, APIConfig(provider=provider)).model),
            metadata={
