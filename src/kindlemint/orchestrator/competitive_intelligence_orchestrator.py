...
class CompetitiveIntelligenceOrchestrator:
    ...
    async def orchestrate_competitive_intelligence(self) -> Dict:
        ...
        if self.config["automation"]["auto_respond_to_threats"]:
            intelligence_report = await self._auto_execute_responses(
                intelligence_report
            )
    ...
    async def _generate_tactical_countermeasures(self) -> Dict:
        ...
        implementation_roadmap = await self._create_tactical_roadmap(
            response_strategies
        )
    ...
    async def _score_market_opportunity(
        self, gap: MarketOpportunity
    ) -> MarketOpportunity:
        ...
        gap.strategic_fit = min(
            gap.strategic_fit * 1.1, 1.0
        )  # Boost if analysis is positive
    ...
    def _detect_significant_changes(self, activities: Dict) -> List[Dict]:
        ...
        return [
            {"competitor": "Dover", "change": "pricing_strategy", "significance": 0.8}
        ]
    ...
    def _calculate_opportunity_score(
        ...
        total_score = sum(
            opp.strategic_fit * opp.profit_potential for opp in opportunities
        )
        return total_score / len(opportunities)

    def _calculate_overall_threat_level(self, threat_assessment: Dict) -> float:
        """Calculate overall threat level"""
        threat_levels = {"critical": 1.0,
                         "high": 0.8, "moderate": 0.5, "low": 0.2}
    ...
    async def _create_implementation_plans(
        ...
        return [
            {"opportunity": opp["niche"], "plan": "3-phase implementation"}
