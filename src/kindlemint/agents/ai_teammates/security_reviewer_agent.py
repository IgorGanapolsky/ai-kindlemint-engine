...
class AISecurityReviewerAgent:
    ...
    async def perform_security_review(self, repo_name: str, pr_number: int) -> SecurityAssessment:
        """
        ...
        try:
            ...
            overall_risk = self._determine_overall_risk(vulnerabilities, compliance_issues)
            ...
            blocking_issues = self._identify_blocking_issues(vulnerabilities, compliance_issues)
        ...
        for file in pr.get_files():
            ...
            for category, patterns in self.vulnerability_patterns.items():
                for pattern_def in patterns:
                    matches = re.finditer(pattern_def["pattern"], file.patch, re.MULTILINE | re.IGNORECASE)
                    
                    for match in matches:
                        # Calculate line number in diff
                        line_num = self._get_line_number_from_patch(file.patch, match.start())
                        ...
                        vulnerability = SecurityVulnerability(
                            ...
                            remediation=self._get_remediation_advice(category, pattern_def),
                            confidence=0.9
                        )
    ...
    async def _scan_secrets(self, pr) -> List[SecurityVulnerability]:
        ...
        for file in pr.get_files():
            ...
            for secret_pattern in self.secrets_patterns:
                matches = re.finditer(secret_pattern["pattern"], file.patch, re.MULTILINE)
    ...
    async def _check_compliance(self, pr) -> List[Dict]:
        ...
        for file in pr.get_files():
            ...
            for standard, rules in self.compliance_rules.items():
                for rule in rules:
                    matches = re.finditer(rule["pattern"], file.patch, re.MULTILINE | re.IGNORECASE)
    ...
    async def _perform_threat_modeling(self, pr, vulnerabilities: List[SecurityVulnerability]) -> Dict:
        ...
        attack_surfaces = {
            ...
            "data_processing": any("data" in f or "process" in f for f in files_changed),
            ...
            "external_integrations": any("integration" in f or "api" in f for f in files_changed)
        }
        ...
        threats = {
            ...
            "repudiation": self._assess_repudiation_threats(files_changed, vulnerabilities),
            "information_disclosure": self._assess_disclosure_threats(files_changed, vulnerabilities),
            "denial_of_service": self._assess_dos_threats(files_changed, vulnerabilities),
            "elevation_of_privilege": self._assess_privilege_threats(files_changed, vulnerabilities)
        }
    ...
    def _assess_spoofing_threats(self, files_changed: List[str], vulns: List[SecurityVulnerability]) -> Dict:
    ...
    def _assess_tampering_threats(self, files_changed: List[str], vulns: List[SecurityVulnerability]) -> Dict:
        ...
        return {
            ...
            "affected_components": [f for f in files_changed if "data" in f or "input" in f]
        }
    
    def _assess_repudiation_threats(self, files_changed: List[str], vulns: List[SecurityVulnerability]) -> Dict:
        """Assess repudiation threats"""
        logging_changes = any("log" in f or "audit" in f for f in files_changed)
        ...
        return {
            ...
            "affected_components": [f for f in files_changed if "log" in f or "audit" in f]
        }
    
    def _assess_disclosure_threats(self, files_changed: List[str], vulns: List[SecurityVulnerability]) -> Dict:
    ...
    def _assess_dos_threats(self, files_changed: List[str], vulns: List[SecurityVulnerability]) -> Dict:
        ...
        return {
            ...
            "affected_components": [f for f in files_changed if "api" in f or "endpoint" in f]
        }
    
    def _assess_privilege_threats(self, files_changed: List[str], vulns: List[SecurityVulnerability]) -> Dict:
        """Assess elevation of privilege threats"""
        auth_changes = any("auth" in f or "permission" in f for f in files_changed)
        auth_vulns = [v for v in vulns if v.category == "auth"]
        ...
        return {
            ...
            "affected_components": [f for f in files_changed if "auth" in f or "permission" in f]
        }
    ...
    def _prioritize_mitigations(self, threats: Dict) -> List[str]:
        ...
        for threat_type, threat_data in threats.items():
            if threat_data["risk_level"] == "high":
                mitigations.append(f"HIGH PRIORITY: Address {threat_type} threats")
    ...
    def _calculate_security_score(self, vulnerabilities: List[SecurityVulnerability], 
                                compliance_issues: List[Dict], files_count: int) -> float:
    ...
    def _determine_overall_risk(self, vulnerabilities: List[SecurityVulnerability], 
                              compliance_issues: List[Dict]) -> str:
    ...
    def _generate_security_recommendations(self, vulnerabilities: List[SecurityVulnerability],
                                         compliance_issues: List[Dict], ai_insights: Dict) -> List[str]:
        ...
        critical_vulns = [v for v in vulnerabilities if v.severity == "critical"]
        if critical_vulns:
            recommendations.append("URGENT: Fix all critical vulnerabilities before merge")
        ...
        if compliance_issues:
            recommendations.append("Address compliance requirements before deployment")
    ...
    def _identify_blocking_issues(self, vulnerabilities: List[SecurityVulnerability],
        ...
        for vuln in critical_vulns:
            blocking.append(f"CRITICAL: {vuln.description} in {vuln.file_path}")
        ...
        if len(high_vulns) > self.security_thresholds["max_high_vulns"]:
            blocking.append(f"TOO MANY HIGH-SEVERITY ISSUES: {len(high_vulns)} found")
        
        return blocking
    
    async def _generate_security_insights(self, pr, vulnerabilities: List[SecurityVulnerability],
                                        compliance_issues: List[Dict], threat_model: Dict) -> Dict:
        """Generate AI-powered security insights"""
        ...
        try:
            ...
            insights = {
                ...
                "mitigation_strategies": self._extract_mitigation_strategies(response.content[0].text)
            }
    ...
    def _extract_key_risks(self, ai_response: str) -> List[str]:
        ...
        for line in lines:
            if any(keyword in line.lower() for keyword in ['risk', 'threat', 'vulnerability', 'exploit']):
    ...
    def _extract_mitigation_strategies(self, ai_response: str) -> List[str]:
        ...
        lines = ai_response.split('\n')
        for line in lines:
            if any(keyword in line.lower() for keyword in ['mitigate', 'fix', 'secure', 'protect']):
...
"""
        ...
        if assessment.overall_risk in ["critical", "high"]:
            comment += "❌ **MERGE BLOCKED** - Critical security issues must be resolved\n"
