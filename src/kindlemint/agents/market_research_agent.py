...
class MarketResearchAgent(BaseAgent):
    ...
    def __init__(self, 
                 ...
                 research_interval: int = 86400):  # 24 hours default
        super().__init__(
    ...
    async def _analyze_competitors(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            competitive_landscape = await self._generate_competitive_landscape(detailed_analysis)
            ...
            results = {
                ...
                "recommendations": await self._generate_competitive_recommendations(competitive_landscape)
            }
    ...
    async def _identify_book_strengths(self, book_data: Dict[str, Any]) -> List[str]:
        ...
        if book_data.get('review_count', 0) >= 100:
            strengths.append(f"Strong review count ({book_data['review_count']} reviews)")
    ...
    async def _identify_book_weaknesses(self, book_data: Dict[str, Any]) -> List[str]:
        ...
        if book_data.get('rating', 5) < 3.5:
            weaknesses.append(f"Below average rating ({book_data['rating']} stars)")
            
        # Few reviews
        if book_data.get('review_count', 0) < 10:
            weaknesses.append(f"Limited reviews ({book_data['review_count']} reviews)")
    ...
    async def _estimate_market_size(self, niche: str) -> Dict[str, Any]:
        ...
        try:
            ...
            books_with_reviews = sum(1 for book in competitor_books if book.get('review_count', 0) > 0)
            avg_reviews = sum(book.get('review_count', 0) for book in competitor_books) / max(total_books, 1)
            avg_rating = sum(book.get('rating', 0) for book in competitor_books if book.get('rating')) / max(books_with_reviews, 1)
            ...
            return {
                ...
                "market_saturation": self._calculate_market_saturation(total_books, avg_reviews),
    ...
    async def _generate_competitive_landscape(self, detailed_analysis: List[Dict]) -> Dict[str, Any]:
        """Generate competitive landscape overview"""
        if not detailed_analysis:
            return {"error": "No competitor data available"}
            
        # Calculate competitive metrics
        ratings = [book.get('rating', 0) for book in detailed_analysis if book.get('rating')]
        review_counts = [book.get('review_count', 0) for book in detailed_analysis if book.get('review_count')]
        ...
        return {
            ...
            "top_performers": sorted(detailed_analysis, key=lambda x: x.get('rating', 0) * x.get('review_count', 0), reverse=True)[:5],
            "market_gaps": await self._identify_market_gaps(detailed_analysis),
            "competitive_intensity": self._assess_competitive_intensity(detailed_analysis)
        }
    ...
    async def _identify_market_gaps(self, competitor_analysis: List[Dict]) -> List[str]:
        ...
        low_rated_books = sum(1 for book in competitor_analysis if book.get('rating', 5) < 4.0)
        ...
        if prices:
            ...
            if avg_price > 15:
                gaps.append("Price gap - opportunity for more affordable options")
            elif avg_price < 5:
                gaps.append("Premium gap - opportunity for higher-quality offerings")
