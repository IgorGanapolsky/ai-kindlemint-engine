...
class MarketResearchAgent(BaseAgent):
    ...
    async def _analyze_competitors(self, task: Task) -> TaskResult:
        ...
        if not niche:
            return TaskResult(
                success=False, error="Niche is required for competitor analysis"
            )
        ...
        try:
            ...
            return TaskResult(
                success=True, data={"analysis": results, "file_path": str(filepath)}
            )
                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        return {
                            ...
                            "keywords_found": self._extract_keywords_from_description(
                                soup
                            ),
                        }
                    else:
    ...
    def _extract_bsr_info(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        if details_section:
            ...
            if overall_match:
                bsr_info["overall_rank"] = overall_match.group(
                    1).replace(",", "")
