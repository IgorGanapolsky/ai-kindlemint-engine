...
class HealthStatus:
    ...
    def calculate_health_level(self) -> HealthLevel:
        ...
        # Critical conditions
        if (
            self.health_checks.get("responsive", True) == False or
            self.metrics.error_rate > 50 or
...
class HealthMonitor:
    ...
    async def update_agent_health(self, agent_id: str, health_status: HealthStatus) -> None:
        ...
        if previous_level != health_status.health_level:
            await self._handle_health_level_change(agent_id, previous_level, health_status.health_level)
    ...
    async def _cleanup_stale_agents(self) -> None:
        ...
        stale_agents = [
            agent_id for agent_id, last_seen in self.agent_last_seen.items()
    ...
    async def _check_alert_thresholds(self, agent_id: str, health: HealthStatus) -> None:
        """Check if any metrics exceed alert thresholds"""
        alerts = []
        
        # CPU usage alerts
        if health.metrics.cpu_usage > self.alert_thresholds["cpu_usage_critical"]:
            alerts.append(("cpu_critical", f"CPU usage {health.metrics.cpu_usage:.1f}%"))
        ...
        if health.metrics.memory_usage > self.alert_thresholds["memory_usage_critical"]:
            alerts.append(("memory_critical", f"Memory usage {health.metrics.memory_usage:.1f}%"))
        elif health.metrics.memory_usage > self.alert_thresholds["memory_usage_warning"]:
            alerts.append(("memory_warning", f"Memory usage {health.metrics.memory_usage:.1f}%"))
        
        # Error rate alerts
        if health.metrics.error_rate > self.alert_thresholds["error_rate_critical"]:
            alerts.append(("error_critical", f"Error rate {health.metrics.error_rate:.1f}%"))
        elif health.metrics.error_rate > self.alert_thresholds["error_rate_warning"]:
            alerts.append(("error_warning", f"Error rate {health.metrics.error_rate:.1f}%"))
        
        # Success rate alerts
        if health.success_rate < self.alert_thresholds["success_rate_critical"]:
            alerts.append(("success_critical", f"Success rate {health.success_rate:.1f}%"))
        elif health.success_rate < self.alert_thresholds["success_rate_warning"]:
            alerts.append(("success_warning", f"Success rate {health.success_rate:.1f}%"))
