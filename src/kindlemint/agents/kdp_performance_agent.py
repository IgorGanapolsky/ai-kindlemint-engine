...
class KDPPerformanceAgent(BaseAgent):
    ...
    async def _monitor_single_book(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            if book_id in self.active_books:
                self.active_books[book_id]["last_monitored"] = datetime.now().isoformat()
    ...
    async def _collect_book_metrics(self, asin: str, book_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Collected metrics for book {book_id} (ASIN: {asin})")
    ...
    async def _store_performance_metrics(self, book_id: str, metrics: Dict[str, Any]) -> None:
    ...
    async def _generate_performance_report(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            # Save report
            report_file = self.data_storage_path / f"performance_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        for book_id, book_data in self.active_books.items():
            ...
            if recent_metrics:
                book_report = {
                    ...
                    "performance_summary": self._summarize_book_performance(recent_metrics)
                }
    ...
    async def _load_active_books(self) -> None:
        ...
        if active_books_file.exists():
            try:
                ...
                self.logger.info(f"Loaded {len(self.active_books)} active books")
            except Exception as e:
    ...
    async def _auto_discover_books(self) -> None:
            ...
            if series_dir.is_dir() and not series_dir.name.startswith('.'):
                for volume_dir in series_dir.iterdir():
                    if volume_dir.is_dir() and volume_dir.name.startswith('volume_'):
                        ...
                        metadata_files = list(volume_dir.rglob("amazon_kdp_metadata.json"))
        ...
        self.logger.info(f"Auto-discovered {len(discovered_books)} books for monitoring")
    ...
    def get_book_performance_summary(self, book_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "first_monitored": metrics_history[0]["timestamp"] if metrics_history else None,
            "last_monitored": metrics_history[-1]["timestamp"] if metrics_history else None
        }
    ...
    def get_all_books_summary(self) -> Dict[str, Any]:
        ...
        return {
            "total_books": len(self.active_books),
            "monitoring_enabled": sum(1 for book in self.active_books.values() if book.get("monitoring_enabled", True)),
