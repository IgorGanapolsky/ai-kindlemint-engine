...
from .atomizer import ContentAtomizer, AtomicContent
from .platforms import (
    LinkedInOptimizer, TwitterOptimizer, InstagramOptimizer, 
    FacebookOptimizer, TikTokOptimizer, YouTubeOptimizer,
    PlatformType, OptimizedPost
)
...
class SocialMediaMarketingEngine:
    ...
    def __init__(self, author_name: str, brand_name: str, 
                 expertise_area: str, target_audience: str, website_url: str):
        ...
        self.authority_builder = AuthorityBuilder(author_name, expertise_area, target_audience)
        self.engagement_bot = CommunityEngagementBot(author_name, brand_voice="professional")
    ...
    def create_complete_marketing_campaign(self, book_content: Dict[str, str]) -> Dict[str, any]:
        """
        ...
        lead_magnets = self.lead_generator.create_lead_magnets_from_content(atomic_content, max_magnets=5)
        
        # Step 3: Generate authority content
        print("👑 Building authority content...")
        authority_content = []
        for content in atomic_content[:20]:  # Top 20 pieces
            thought_leadership = self.authority_builder.generate_thought_leadership_content(content)
        ...
        for platform, optimizer in self.platform_optimizers.items():
            ...
            for content in atomic_content[:50]:  # Top 50 pieces per platform
                try:
                    ...
                    print(f"Warning: Failed to optimize content for {platform.value}: {e}")
                    continue
        ...
        campaign_data = {
            ...
            "content_library": {
                ...
                "total_optimized_posts": sum(len(posts) for posts in optimized_content.values())
            },
            "lead_magnets": [
                {
                    "id": magnet.id,
            ...
            "schedules": {
                ...
                "engagement_activities": sum(len(activities) for activities in engagement_schedule.values()),
    ...
    def _create_30_day_calendar(self, book_content: Dict[str, str]) -> Dict[str, any]:
        ...
        for day in range(1, 31):
            ...
            for platform in [PlatformType.LINKEDIN, PlatformType.INSTAGRAM, PlatformType.TWITTER]:
                ...
                if content_index < len(atomic_content):
                    ...
                    try:
                        ...
                        daily_content[platform.value] = {"content": "Content optimization pending"}
    ...
    def get_campaign_status(self, campaign_id: str) -> Dict[str, any]:
        ...
        return {
            ...
            "dashboard_summary": dashboard["summary_cards"] if "summary_cards" in dashboard else {},
            "authority_status": {
                "positioning_complete": len(positioning) > 0,
                "thought_leadership_posts": dashboard.get("content_leaderboard", [])[:5]
            },
            "community_status": {
    ...
    def optimize_campaign(self, campaign_id: str, performance_data: Dict[str, any]) -> Dict[str, any]:
        ...
        if "content_performance" in performance_data:
            content_optimizations = self._optimize_content_strategy(performance_data["content_performance"])
            optimization_report["optimizations_applied"].extend(content_optimizations)
        
        # Platform optimization
        if "platform_performance" in performance_data:
            platform_optimizations = self._optimize_platform_strategy(performance_data["platform_performance"])
            optimization_report["optimizations_applied"].extend(platform_optimizations)
        
        # Lead generation optimization
        if "lead_performance" in performance_data:
            lead_optimizations = self._optimize_lead_generation(performance_data["lead_performance"])
    ...
    def _optimize_platform_strategy(self, platform_performance: Dict) -> List[str]:
        ...
        best_platform = max(platform_performance.items(), key=lambda x: x[1].get("engagement_rate", 0))
    ...
    def export_campaign_data(self, campaign_id: str, filename: str) -> None:
        ...
        campaign_export = {
            ...
            "metadata": {
                ...
                "campaign_duration_days": (datetime.now() - self.analytics.tracking_start_date).days
            }
        }
...
def demo_marketing_engine():
    ...
    print(f"📊 Total content pieces: {campaign['content_library']['total_optimized_posts']}")
    print(f"🧲 Lead magnets created: {len(campaign['lead_magnets'])}")
    print(f"📅 30-day calendar: {len(campaign['30_day_specifics']['content_calendar'])} days planned")
