...
class SudokuValidator(PuzzleValidator):
    ...
    def check_solvability(
        ...
        if (
            solution_count == 1
            and "solution" in puzzle_data
            and isinstance(puzzle_data["solution"], list)
        ):
            ...
            # Compare solutions
            if len(provided_solution) == 9 and all(
                len(row) == 9 for row in provided_solution
            ):
                for i in range(9):
                    for j in range(9):
    ...
    def _is_valid(self, grid: List[List[int]], row: int, col: int, num: int) -> bool:
        ...
        for x_var in range(9):
            if grid[row][x_var] == num:
                return False

        # Check column
        for x_var in range(9):
            if grid[x_var][col] == num:
                return False
