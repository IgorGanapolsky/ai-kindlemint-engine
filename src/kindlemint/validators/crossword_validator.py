...
class CrosswordValidator(PuzzleValidator):
    ...
    def check_content(self, puzzle_data: Dict[str, Any], puzzle_id: Union[int, str]) -> List[ValidationIssue]:
        ...
        if "clue_positions" in puzzle_data and isinstance(puzzle_data["clue_positions"], dict):
            clue_positions = puzzle_data["clue_positions"]
            position_numbers = set(int(pos_num) for pos_num in clue_positions.values() if isinstance(pos_num, (int, str)) and str(pos_num).isdigit())
            ...
            for direction in ["across", "down"]:
                if direction in clues and isinstance(clues[direction], list):
                    for clue in clues[direction]:
                        if isinstance(clue, list) and len(clue) >= 1 and isinstance(clue[0], int):
