...
class WordSearchValidator(PuzzleValidator):
        ...
        if "grid" in puzzle_data:
            ...
            if not isinstance(grid, list):
                ...
                if grid_height == 0:
                    ...
                    grid_width = len(grid[0]) if len(grid) > 0 and isinstance(grid[0], list) else 0
    ...
    def _find_word_in_grid(self, grid: List[List[str]], word: str, direction: Tuple[int, int]) -> bool:
        """
        ...
        for row in range(grid_height):
            for col in range(grid_width):
                ...
                if (0 <= row + row_delta * (len(word) - 1) < grid_height and 
                    0 <= col + col_delta * (len(word) - 1) < grid_width):
