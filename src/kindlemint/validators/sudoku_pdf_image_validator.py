    ...
    def validate_pdf_images(self, pdf_path: Path) -> Dict:
        ...
        try:
            ...
            for page_num in range(total_pages):
                ...
                if "Puzzle #" in text and page_num < 110:  # First ~100 pages are puzzles
                    if images:
                        puzzle_pages_with_images += 1
                        self.passed_checks.append(f"✓ Page {page_num + 1}: Puzzle has image")
                    else:
                        # Check for text fallback
                        if ". . ." in text or "[ Sudoku Grid ]" in text:
                            pages_with_text_fallback += 1
                            self.errors.append(f"❌ Page {page_num + 1}: Puzzle using text fallback!")
                        else:
                            self.warnings.append(f"⚠️ Page {page_num + 1}: No puzzle image found")
                
                # Check if this is a solution page
                elif "Solution to Puzzle" in text and page_num > 100:
                    if images:
                        solution_pages_with_images += 1
                        self.passed_checks.append(f"✓ Page {page_num + 1}: Solution has image")
                    else:
                        # Check for text fallback
                        if "1 2 3" in text or "[ Solution Grid ]" in text:
                            pages_with_text_fallback += 1
                            self.errors.append(f"❌ Page {page_num + 1}: Solution using text fallback!")
                        else:
                            self.warnings.append(f"⚠️ Page {page_num + 1}: No solution image found")
            
            # Validate counts
            if puzzle_pages_with_images < expected_puzzle_pages * 0.9:  # Allow 10% margin
                self.errors.append(
            ...
            for page_num in range(len(doc)):
                ...
                for img_index, img in enumerate(page.get_images()):
                    try:
                        ...
                        self.warnings.append(f"⚠️ Could not check image on page {page_num + 1}: {str(e)}")
        except Exception as e:
