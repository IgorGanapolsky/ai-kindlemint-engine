...
class SudokuBookQAValidator:
            ...
            with open(pdf_path, "rb") as file:
                ...
                if num_pages < 200:
                    self.warnings.append(
                        f"PDF has only {num_pages} pages (expected 200+)"
                    )
                else:
                    self.passed_checks.append(
                        f"✓ PDF has correct page count: {num_pages}"
                    )
                ...
                if num_pages > 10:
                    ...
                    if self._is_complete_grid_in_text(text):
                        self.errors.append(
                            "❌ CRITICAL: Puzzle pages appear to contain complete solutions!"
                        )
                    else:
                        self.passed_checks.append(
                            "✓ Puzzle pages appear to have blanks (not complete solutions)"
                        )
    ...
    def _validate_puzzle_directory(self, puzzle_dir: Path):
        ...
        if metadata_dir.exists():
            ...
            if issues["missing_images"] > 0:
                self.errors.append(
                    f"❌ {issues['missing_images']} puzzle images are missing"
                )

            # Add summary for clue validation
            if issues["wrong_clues"] == 0:
                self.passed_checks.append(
                    "✓ All checked puzzles have appropriate clue counts for their difficulty"
                )
        else:
    ...
    def _validate_puzzle_image(self, image_path: Path, puzzle_id: int) -> int:
        ...
        try:
            ...
            if white_ratio < 0.5:
                self.errors.append(
                    f"❌ Puzzle {puzzle_id} image appears to be completely filled"
                )
    ...
    def _validate_pdf_visual_rendering(self, pdf_path: Path):
        ...
        try:
            ...
            if time_diff.total_seconds() < 3600:  # Less than 1 hour old
                self.passed_checks.append(
                    "✓ PDF was recently regenerated with clue rendering fix"
                )
            else:
                self.warnings.append(
                    "⚠️ PDF may need regeneration to fix clue rendering"
                )
