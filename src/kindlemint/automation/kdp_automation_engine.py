...
class KDPBrowserBot:
    ...
    def login_to_kdp(self, email: str, password: str) -> bool:
        ...
        try:
            ...
            # Wait for dashboard
            self.wait.until(EC.presence_of_element_located((By.CLASS_NAME, "kdp-dashboard")))
            return True
    ...
    def upload_book(self, pdf_path: str, metadata: BookMetadata) -> bool:
        ...
        try:
            ...
            create_btn = self.wait.until(
                EC.element_to_be_clickable((By.XPATH, "//button[contains(text(), 'Create New Title')]"))
            )
            create_btn.click()
            
            # Select Paperback
            paperback_btn = self.wait.until(
                EC.element_to_be_clickable((By.XPATH, "//span[text()='Paperback']"))
            )
            paperback_btn.click()
    ...
    def _fill_book_details(self, metadata: BookMetadata):
        ...
        # Keywords
        for i, keyword in enumerate(metadata.keywords[:7]):  # KDP allows 7 keywords
    ...
    def _upload_content(self, pdf_path: str):
        """Upload PDF content"""
        upload_input = self.wait.until(EC.presence_of_element_located((By.INPUT, "file")))
    ...
    def _set_pricing(self, price: float):
        """Set book pricing"""
        price_field = self.wait.until(EC.presence_of_element_located((By.NAME, "list_price")))
        price_field.clear()
...
class KDPAutomationEngine:
    ...
    def __init__(self, config: Dict[str, str]):
        ...
        self.jungle_scout_api = JungleScoutAPI(config.get("jungle_scout_api_key"))
    ...
    async def find_profitable_niches(self, seed_keywords: List[str] = None) -> List[NicheOpportunity]:
        ...
        for keyword in seed_keywords:
            ...
            opportunity = self._calculate_opportunity(keyword, amazon_results, h10_data)
            if opportunity:
    ...
    def _calculate_opportunity(self, keyword: str, amazon_data: List[Dict], h10_data: Dict) -> Optional[NicheOpportunity]:
        """Calculate opportunity score for a keyword"""
        if not amazon_data:
            return None
            
        # Extract metrics
        avg_bsr = sum(book.get("bsr", 100000) for book in amazon_data) / len(amazon_data)
        avg_price = sum(book.get("price", 0) for book in amazon_data) / len(amazon_data)
        avg_reviews = sum(book.get("reviews", 0) for book in amazon_data) / len(amazon_data)
    ...
    def generate_book_metadata(self, niche: NicheOpportunity) -> BookMetadata:
        ...
        return BookMetadata(
            ...
            description=enhanced_metadata.get("description", f"Premium {niche.keyword} puzzle book"),
            keywords=enhanced_metadata.get("keywords", [niche.keyword, "puzzles", "large print"]),
            categories=enhanced_metadata.get("categories", ["Games & Activities"]),
    ...
    async def auto_generate_and_publish(self, niche: NicheOpportunity) -> bool:
        ...
        try:
            ...
            login_success = self.browser_bot.login_to_kdp(kdp_email, kdp_password)
            ...
            if upload_success:
                self.logger.info(f"✅ Book published successfully: {metadata.title}")
                return True
            else:
    ...
    async def run_full_automation(self, max_books: int = 3) -> List[Dict]:
        ...
        """
        self.logger.info(f"🎯 Starting full KDP automation pipeline (max {max_books} books)")
        ...
        for i, niche in enumerate(niches[:max_books]):
            self.logger.info(f"📚 Processing niche {i+1}/{max_books}: {niche.keyword}")
        ...
        self.logger.info(f"🎉 Automation complete! Success rate: {success_rate:.1f}%")
