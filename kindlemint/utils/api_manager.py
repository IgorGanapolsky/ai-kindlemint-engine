...
from typing import Dict, Any, Optional # List unused
...
class APIManager:
    ...
    def __init__(self):
        ...
        self.providers = {
            AIProvider.OPENAI: {
                ...
                'rate_limits': { 'requests_per_minute': 3000, 'tokens_per_minute': 150000, 'requests_per_day': 10000 },
                'costs': {
                    'gpt-4': {'input': 0.03, 'output': 0.06},
                    'gpt-3.5-turbo': {'input': 0.0005, 'output': 0.0015}, # Updated example cost
                    'dall-e-3': 0.040  # Cost per image (This is a flat rate, not token based for cost)
                }
            },
            AIProvider.GEMINI: {
                'api_key': os.getenv('GEMINI_API_KEY'),
                'models': { 'text': 'gemini-1.5-flash', 'cheap_text': 'gemini-1.5-flash' }, # Example, could be gemini-pro for quality text
                'rate_limits': { 'requests_per_minute': 60, 'requests_per_day': 1500 },
                'costs': {
                    'gemini-1.5-flash': {'input': 0.000125, 'output': 0.000375} # Example for Flash per 1K tokens
                    # 'gemini-pro': {'input': 0.0025, 'output': 0.005} # Example for Pro per 1K tokens
                }
            },
            AIProvider.GROK: {
                'api_key': os.getenv('GROK_API_KEY'),
                'models': { 'text': 'grok-beta', 'cheap_text': 'grok-beta' }, # Placeholder
                'rate_limits': { 'requests_per_minute': 1000, 'requests_per_day': 10000 },
                'costs': { 'grok-beta': {'input': 0.005, 'output': 0.015} } # Example pricing per 1K tokens
            }
        }
        ...
        if self.providers[AIProvider.OPENAI]['api_key'] and openai:
            try:
                self.openai_client = openai.OpenAI(api_key=self.providers[AIProvider.OPENAI]['api_key'])
    ...
    def _load_usage_data(self) -> Dict:
        ...
        if self.usage_file.exists():
            try:
                with open(self.usage_file, 'r', encoding='utf-8') as f: # Added encoding
                    return json.load(f)
            except json.JSONDecodeError: # More specific error
                self.logger.error(f"Error decoding JSON from {self.usage_file}. Initializing new usage data.")
            except Exception as e: # Catch other potential errors
                self.logger.error(f"Error loading usage data from {self.usage_file}: {e}. Initializing new usage data.")
    ...
    def get_best_provider_for_task(self, task_type: str, priority: str = 'balanced') -> AIProvider:
        ...
        for provider_enum, config in self.providers.items(): # Corrected iteration variable name
            if not config['api_key']:
                self.logger.debug(f"Provider {provider_enum.value} skipped: No API key.")
                continue

            # Check client/library readiness
            if provider_enum == AIProvider.OPENAI and not self.openai_client:
                self.logger.debug(f"Provider {provider_enum.value} skipped: OpenAI client not initialized.")
                continue
            if provider_enum == AIProvider.GEMINI and not (genai and self.providers[AIProvider.GEMINI]['api_key']): # Check genai lib and configured API key
                 self.logger.debug(f"Provider {provider_enum.value} skipped: Gemini library or API key not configured.")
                 continue
            if provider_enum == AIProvider.GROK and not (self.grok_client or os.getenv('GROK_API_KEY')): # Assuming self.grok_client would be set, or check key
                 self.logger.debug(f"Provider {provider_enum.value} skipped: Grok API key or client not available.")
                 continue
            
            # Check daily limits
            daily_usage = self.usage_data['daily_usage'].get(today, {}).get(provider_enum.value, {})
            ...
            if requests_today < daily_limit:
                ...
                self.logger.warning(f"Provider {provider_enum.value} at daily request limit ({requests_today}/{daily_limit}).")
        
        if not eligible_providers:
            self.logger.warning("⚠️ All configured and ready providers are at daily limits or unavailable! Defaulting to OpenAI if configured, else first provider.")
            # Fallback to OpenAI if its client is ready, otherwise just the first in the main list (which might be empty)
            return AIProvider.OPENAI if self.openai_client else (self.providers.keys()[0] if self.providers.keys() else AIProvider.OPENAI)


        # Route based on task type and priority
        if task_type == 'image_generation':
            if AIProvider.OPENAI in eligible_providers: # Image gen currently tied to OpenAI
                return AIProvider.OPENAI
            else:
                self.logger.warning("⚠️ OpenAI not eligible for image generation, though it's the only option. Returning first eligible provider.")
        ...
        order = []
        if priority == 'cost':
            cost_priority_order = [AIProvider.GEMINI, AIProvider.GROK, AIProvider.OPENAI]
            for p in cost_priority_order:
                if p in eligible_providers and self.providers[p]['models'].get(model_key_suffix):
                    order.append(p)
        # Quality-driven selection
        elif priority == 'quality':
            quality_priority_order = [AIProvider.OPENAI, AIProvider.GROK, AIProvider.GEMINI]
        ...
        self.logger.warning(f"No provider found for task '{task_type}' with priority '{priority}' among eligible. Falling back to first eligible provider.")
        return eligible_providers[0]

    def generate_text(self, prompt: str, task_type: str = 'text_generation', 
                     priority: str = 'balanced', max_tokens: int = 1500,
                     model_preference: Optional[str] = None) -> Optional[str]:
        ...
        if provider and model_preference:
            ...
            is_valid_preference = any(model_preference == m_name for m_name in self.providers[provider]['models'].values())
            if not is_valid_preference:
                self.logger.warning(f"Model preference '{model_preference}' not found for provider {provider.value}. Using provider's default for task type '{model_key_for_task}'.")
                selected_model_name = self.providers[provider]['models'].get(model_key_for_task)
        elif provider: # No preference, use provider's default for task type
            selected_model_name = self.providers[provider]['models'].get(model_key_for_task)

        if not selected_model_name: # If still no model (e.g. provider has no model for 'cheap_text')
            if provider and self.providers[provider]['models'].get('text'): # Fallback to provider's main 'text' model
                selected_model_name = self.providers[provider]['models']['text']
                self.logger.info(f"No model for '{model_key_for_task}' for {provider.value}, falling back to main text model: {selected_model_name}")
            else:
                self.logger.error(f"Could not determine any model for provider {provider.value if provider else 'N/A'}, task_type {task_type}, preference {model_preference}.")
                return None

        self.logger.info(f"Attempting text generation with {provider.value} using model {selected_model_name}.")

        try:
            if provider == AIProvider.OPENAI:
                if not self.openai_client: raise Exception("OpenAI client not initialized.")
                return self._generate_with_openai(prompt, max_tokens, model_name=selected_model_name)
            elif provider == AIProvider.GEMINI:
                if not (genai and self.providers[AIProvider.GEMINI]['api_key']): raise Exception("Gemini library/API key not configured.")
                return self._generate_with_gemini(prompt, max_tokens, model_name=selected_model_name)
            elif provider == AIProvider.GROK:
                # Assumes grok_client or key check is done in _generate_with_grok
                return self._generate_with_grok(prompt, max_tokens, model_name=selected_model_name) # This will raise NotImplementedError
            else: # Should ideally not be reached if get_best_provider_for_task works
                self.logger.error(f"Unsupported or uninitialized provider determined: {provider}")
                return None
        except Exception as e:
            self.logger.error(f"Text generation failed with {provider.value} (model: {selected_model_name}): {e}")
            
            # Simplified fallback: try OpenAI if it wasn't the primary, then Gemini
            fallback_attempts = []
            if provider != AIProvider.OPENAI and self.openai_client:
                fallback_attempts.append({'provider': AIProvider.OPENAI, 'model_key': 'text'})
            if provider != AIProvider.GEMINI and (genai and self.providers[AIProvider.GEMINI]['api_key']):
                fallback_attempts.append({'provider': AIProvider.GEMINI, 'model_key': 'text'})

            for attempt in fallback_attempts:
                fb_provider = attempt['provider']
                fb_model_name = self.providers[fb_provider]['models'].get(attempt['model_key'])
                if not fb_model_name: continue

                try:
                    self.logger.info(f"🔄 Trying fallback: {fb_provider.value} with model {fb_model_name}")
                    if fb_provider == AIProvider.OPENAI:
                        return self._generate_with_openai(prompt, max_tokens, model_name=fb_model_name)
                    elif fb_provider == AIProvider.GEMINI:
                         return self._generate_with_gemini(prompt, max_tokens, model_name=fb_model_name)
                except Exception as fe:
                    self.logger.warning(f"Fallback {fb_provider.value} (model: {fb_model_name}) also failed: {fe}")
            
            self.logger.error(f"❌ All text generation attempts failed for prompt (first 50 chars): {prompt[:50]}...")
            return None
    ...
    def generate_image(self, prompt: str, size: str = "1024x1024") -> Optional[str]:
        ...
        if not self.openai_client: # Check if OpenAI client is initialized
            self.logger.warning("⚠️ OpenAI client not initialized. Cannot generate image.")
            return None
        try:
            ...
            return self._generate_image_with_dalle(prompt, size, model_name=dalle_model_name)
        except Exception as e:
            self.logger.error(f"Image generation failed: {e}")
            return None

    def _generate_with_openai(self, prompt: str, max_tokens: int, model_name: str) -> str:
        ...
        self._track_usage(AIProvider.OPENAI, 'text', model_name=model_name,
                          input_tokens=input_tokens, output_tokens=output_tokens)
        return response.choices[0].message.content

    def _generate_with_gemini(self, prompt: str, max_tokens: int, model_name: str) -> str:
        ...
        if not (genai and self.providers[AIProvider.GEMINI]['api_key']):
             self.logger.error("Gemini library or API key not configured when _generate_with_gemini called.")
        ...
        try:
            ...
            if hasattr(response, 'usage_metadata') and response.usage_metadata and hasattr(response.usage_metadata, 'candidates_token_count'):
                 output_tokens = response.usage_metadata.candidates_token_count
            elif hasattr(response, 'text'): # Fallback to counting tokens from response.text if usage_metadata is not helpful
                 output_tokens = active_gemini_model.count_tokens(response.text).total_tokens
            else: # Further fallback if response.text is also not available (should not happen for successful calls)
                self.logger.warning(f"Could not determine output tokens for Gemini model {model_name} from response structure.")
        except Exception as e:
            self.logger.warning(f"Could not get exact token counts for Gemini model {model_name}: {e}. Estimating.")
            input_tokens = len(prompt) // 4 # Rough estimate
            output_tokens = len(response.text) // 4 if hasattr(response, 'text') else 0 # Rough estimate
        
        self._track_usage(AIProvider.GEMINI, 'text', model_name=model_name,
                           input_tokens=input_tokens, output_tokens=output_tokens)
        return response.text

    def _generate_with_grok(self, prompt: str, max_tokens: int, model_name: str) -> str:
        """Generate text with Grok (X.AI). Placeholder."""
        if not (self.grok_client or os.getenv('GROK_API_KEY')): # Check if client or at least key is available
             self.logger.error("Grok client/API key accessed but not available.")
             raise Exception("Grok client/API key not available.")
        self.logger.warning(f"⚠️ Grok API integration for model {model_name} not yet implemented. Prompt (start): {prompt[:50]}...")
        ...
        # return response.choices[0].message.content
        raise NotImplementedError(f"Grok API integration for model {model_name} pending")

    def _generate_image_with_dalle(self, prompt: str, size: str, model_name: str) -> str:
        """Generate image with DALL-E using the pre-initialized client."""
        if not self.openai_client:
            self.logger.error("OpenAI client (for DALL-E) accessed but not initialized.")
        ...
        self._track_usage(AIProvider.OPENAI, 'image', model_name=model_name, images_generated=1)
        return image_url

    def _track_usage(self, provider: AIProvider, task_type: str, model_name: str, **kwargs):
        ...
        if model_name not in usage_today_provider['model_usage']:
            usage_today_provider['model_usage'][model_name] = {
                'requests': 0, 'tokens_input': 0, 'tokens_output': 0,
                'images_generated':0, 'cost': 0.0 # Added images_generated here for per-model image tracking
            }
        ...
        current_cost = 0.0
        provider_config_block = self.providers.get(provider) # Get the whole config for the provider
        ...
        if task_type == 'text':
            ...
            if provider_config_block and model_name in provider_config_block['costs']:
                cost_config_for_model = provider_config_block['costs'][model_name] # e.g. {'input': 0.03, 'output': 0.06}
                if isinstance(cost_config_for_model, dict): # Standard input/output cost structure
                    current_cost += (input_tokens / 1000) * cost_config_for_model.get('input', 0)
                    current_cost += (output_tokens / 1000) * cost_config_for_model.get('output', 0)
                else: # Should not happen if config is correct for text models
                    self.logger.warning(f"Cost config for text model {model_name} of {provider_value} is not a dict: {cost_config_for_model}")
            else:
                 self.logger.warning(f"No cost config found for text model {model_name} of {provider_value}")

        elif task_type == 'image' and 'images_generated' in kwargs:
            images_generated = kwargs['images_generated']
            usage_today_provider['images'] += images_generated # Overall images by provider
            model_usage_stats['images_generated'] += images_generated # Images by this specific model (e.g. dall-e-3)

            if provider_config_block and model_name in provider_config_block['costs']:
                 cost_per_image = provider_config_block['costs'][model_name] # e.g. dall-e-3: 0.040
                 if isinstance(cost_per_image, (float, int)): # Flat rate for image
                    ...
                    self.logger.warning(f"Cost config for image model {model_name} of {provider_value} is not a flat rate: {cost_per_image}")
            else:
                self.logger.warning(f"No cost config found for image model {model_name} of {provider_value}")


        usage_today_provider['cost'] = round(usage_today_provider.get('cost', 0.0) + current_cost, 6) # Accumulate and round
        model_usage_stats['cost'] = round(model_usage_stats.get('cost', 0.0) + current_cost, 6) # Accumulate and round
    ...
    def get_usage_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'yesterday_by_provider': self.usage_data['daily_usage'].get(yesterday_str, {}), # Renamed for clarity
        ...
        for p_enum, config in self.providers.items():
            ...
            if api_key_present:
                if p_enum == AIProvider.OPENAI:
                    ...
                    status_detail = "OpenAI Client Initialized" if client_ready else "OpenAI Client Failed/Not Initialized"
                elif p_enum == AIProvider.GEMINI:
                    client_ready = bool(genai and config.get('api_key')) # Genai lib loaded and key was present for configure()
                    status_detail = "Gemini Library Configured" if client_ready else "Gemini Library/API Key Not Configured"
                elif p_enum == AIProvider.GROK:
                    # Assuming self.grok_client would be set if successfully initialized
                    client_ready = bool(self.grok_client or os.getenv('GROK_API_KEY')) # Looser check if client object not used yet
                    status_detail = "Grok Configured/Client Ready" if client_ready else "Grok Not Fully Initialized/Client Missing"
                else:
        ...
        num_fully_available_providers = sum(1 for s in summary['provider_status'].values() if s['is_fully_available'])
        num_configured_providers = sum(1 for s in summary['provider_status'].values() if s['api_key_set'])
        ...
        for p_name in [AIProvider.OPENAI.value, AIProvider.GEMINI.value]:
            if p_name in summary['provider_status'] and not summary['provider_status'][p_name]['is_fully_available']:
                reason = "at limit" if not summary['provider_status'][p_name]['within_daily_request_limit'] else \
                         "client/config issue" if not summary['provider_status'][p_name]['client_library_ready'] else "unknown reason"
                primary_down_messages.append(f"{p_name} ({reason})")

        if len(primary_down_messages) == 2: # Both primary are down
             summary['recommendations'].append(f"🚨 CRITICAL: Both OpenAI & Gemini are down/limited ({'; '.join(primary_down_messages)}). Severe impact likely.")
        elif primary_down_messages:
            summary['recommendations'].append(f"⚠️ Primary provider issue: {'; '.join(primary_down_messages)} unavailable/limited.")
