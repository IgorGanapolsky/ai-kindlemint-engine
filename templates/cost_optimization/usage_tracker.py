...
class ClaudeCodeUsageTracker:
    ...
    def end_session(self, lines_changed: int, files_modified: int, 
                   git_commits: int, estimated_cost: float, notes: str = ""):
    ...
    def get_usage_report(self, days: int = 30) -> Dict:
        ...
        expensive_sessions = sorted(recent_sessions, 
                                  key=lambda s: s.estimated_cost, reverse=True)[:5]
    ...
    def print_usage_report(self, days: int = 30):
        ...
        for task_type, data in report['task_breakdown'].items():
            ...
            print(f"  {task_type:20} {data['count']:3} sessions  "
        ...
        for i, session in enumerate(report['most_expensive_sessions'], 1):
            print(f"  {i}. {session['task_type']} - ${session['cost']:.2f} "
    ...
    def suggest_optimizations(self) -> List[str]:
        ...
        expensive_short = [s for s in report['most_expensive_sessions'] 
